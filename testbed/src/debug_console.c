...
void debug_console_create()
{
    if (!state_ptr)
    {
        state_ptr = ballocate(sizeof(debug_console_state), MEMORY_TAG_GAME);
...
void debug_console_update()
{
    if (state_ptr && state_ptr->dirty)
    {
        ...
        u32 max_lines = BMIN(state_ptr->line_display_count, BMAX(line_count, state_ptr->line_display_count));

        // Calculate min line first, taking into account line offset as well
        u32 min_line = BMAX(line_count - max_lines - state_ptr->line_offset, 0);
...
void debug_console_move_up()
{
    if (state_ptr)
    {
        ...
        state_ptr->line_offset = BMIN(state_ptr->line_offset, line_count - state_ptr->line_display_count);
    }
}
...
void debug_console_move_down()
{
    if (state_ptr)
    {
        ...
        state_ptr->line_offset--;
        state_ptr->line_offset = BMAX(state_ptr->line_offset, 0);
    }
}
