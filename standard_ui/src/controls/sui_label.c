...
static void regenerate_label_geometry(sui_control* self)
{
    ...
    if (needs_realloc)
    {
        // Realloc from vertex buffer
        if (typed_data->max_text_length > 0)
        {
            if (!renderer_renderbuffer_free(vertex_buffer, prev_vertex_buffer_size, typed_data->vertex_buffer_offset))
                BERROR("Failed to free from renderer vertex buffer: size=%u, offset=%u", vertex_buffer_size, typed_data->vertex_buffer_offset);
        }
        ...
        // Realloc from index buffer
        if (typed_data->max_text_length > 0)
        {
            if (!renderer_renderbuffer_free(index_buffer, prev_index_buffer_size, typed_data->index_buffer_offset))
                BERROR("Failed to free from renderer index buffer: size=%u, offset=%u", index_buffer_size, typed_data->index_buffer_offset);
        }
    ...
    // Update max length if the string is longer
