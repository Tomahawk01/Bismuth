...
{
    // Internal ogg vorbis file handle, if the file is ogg
    stb_vorbis* vorbis;
    // Internal mp3 file handle
...
static u64 audio_file_load_samples(struct audio_file* audio, u32 chunk_size, i32 count)
{
    ...
    else if (audio->internal_data->mp3_info.buffer)
    {
        ...
        return BMIN(audio->total_samples_left, chunk_size);
    }
    BERROR("Error loading samples: Unknown file type");
...
static void* audio_file_stream_buffer_data(struct audio_file* audio)
{
    ...
    {
        BERROR("Error streaming audio dta: Unknown file type. Null is returned")
        return 0;
    }
}
...
void audio_file_rewind(struct audio_file* audio)
{
    if (audio)
    {
        ...
        {
            BERROR("Error rewinding audio file: unknown type");
            return;
        }
    }
}
...
static b8 audio_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource)
{
    ...
    audio_file* resource_data = ballocate(sizeof(audio_file), MEMORY_TAG_RESOURCE);
    // Keep a pointer to this to free it later
    resource_data->audio_resource = out_resource;
    resource_data->type = typed_params->type;
    resource_data->internal_data = ballocate(sizeof(audio_file_internal), MEMORY_TAG_RESOURCE);
    if (string_index_of_str(".ogg", full_file_path) != -1)
    {
        BTRACE("Processing OGG music file '%s'...", full_file_path);
        i32 ogg_error = 0;
        ...
        if (!resource_data->internal_data->vorbis)
        {
            ...
            BERROR("Failed to load vorbis file with error: %u", ogg_error);
            return false;
        }
        ...
        if (resource_data->type == AUDIO_FILE_TYPE_MUSIC_STREAM)
        {
            ...
            resource_data->internal_data->pcm = ballocate(buffer_length, MEMORY_TAG_AUDIO);
        ...
        else
        {
            ...
            // Since whole thing is being read into a buffer at once, just use an inline array for the data
            resource_data->internal_data->pcm = ballocate(buffer_length, MEMORY_TAG_AUDIO);
            resource_data->internal_data->pcm_size = buffer_length;
            ...
            if (read_samples != (i64)length_samples)
                BWARN("Read/length mismatch while reading ogg file. This might cause playback issues");
    ...
    {
        BTRACE("Processing MP3 file '%s'...", full_file_path);
        ...
        BDEBUG("mp3 freq: %dHz, avg kbit/s rate: %u", info->hz, info->avg_bitrate_kbps);
        resource_data->channels = info->channels;
    ...
    {
        BERROR("Unsupported audio file type");
        return false;
    }
...
static void audio_loader_unload(struct resource_loader* self, resource* resource)
{
    if (resource->data)
    {
        ...
        if (resource_data->internal_data)
        {
            ...
            bfree(resource_data->internal_data, sizeof(audio_file_internal), MEMORY_TAG_RESOURCE);

            if (resource_data->internal_data->pcm)
            {
                bfree(resource_data->internal_data->pcm, resource_data->internal_data->pcm_size, MEMORY_TAG_AUDIO);
