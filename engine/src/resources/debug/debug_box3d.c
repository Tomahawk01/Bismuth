...
b8 debug_box3d_initialize(debug_box3d *box)
{
    ...
    box->vertex_count = 2 * 12;  // 12 lines to make cube
    box->vertices = ballocate(sizeof(color_vertex_3d) * box->vertex_count, MEMORY_TAG_ARRAY);
    ...
    if (box)
    {
        if (box->vertex_count && box->vertices)
        {
            for (u32 i = 0; i < box->vertex_count; ++i)
                box->vertices[i].color = box->color;
        }
    }
}
