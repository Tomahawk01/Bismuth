...
b8 simple_scene_update(simple_scene *scene, const struct frame_data *p_frame_data)
{
    ...
    if (scene->state >= SIMPLE_SCENE_STATE_LOADED)
    {
        ...
        if (scene->dir_light && scene->dir_light->debug_data)
        {
            ...
            {
                // Update color
                // NOTE: doing this every frame might be expensive if we have to reload the geometry all the time
...
b8 simple_scene_directional_light_add(simple_scene *scene, const char *name, struct directional_light *light)
{
    ...
    if (scene->dir_light)
    {
        ...
        {
            BERROR("simple_scene_add_directional_light - failed to add directional light to light system");
            return false;
        }
...
b8 simple_scene_point_light_remove(simple_scene *scene, const char *name)
{
    ...
    BERROR("Cannot remove a point light from a scene of which it is not a part");
    return false;
}
...
b8 simple_scene_mesh_remove(simple_scene *scene, const char *name)
{
    ...
    BERROR("Cannot remove a mesh from a scene of which it is not a part");
    return false;
}
...
b8 simple_scene_terrain_remove(simple_scene *scene, const char *name)
{
    ...
    BERROR("Cannot remove a terrain from a scene of which it is not a part");
    return false;
}
    ...
    for (u32 i = 0; i < mesh_count; ++i)
    {
        ...
        if (m->generation != INVALID_ID_U8)
        {
            ...
            for (u32 j = 0; j < m->geometry_count; ++j)
            {
                ...
                // TODO: Distance-from-line detection per object
