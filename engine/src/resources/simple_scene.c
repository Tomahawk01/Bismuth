...
b8 simple_scene_add_directional_light(simple_scene* scene, struct directional_light* light)
{
    ...
    if (scene->dir_light)
    {
        // TODO: resource unloading required
...
static void simple_scene_actual_unload(simple_scene* scene)
{
    ...
    {
        // TODO: If there are resource to unload, that should be done before this next line
        // Always kill first one on the list each time since entries are popped from the array
        if (!simple_scene_remove_point_light(scene, scene->point_lights[0]))
            BERROR("Failed to unload/remove point light");
    }
