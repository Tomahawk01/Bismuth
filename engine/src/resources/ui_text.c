...
b8 ui_text_create(const char* name, ui_text_type type, const char* font_name, u16 font_size, const char* text_content, ui_text* out_text)
{
    ...
    {
        BERROR("ui_text_create requires a valid pointer to font_name, text_content and out_text");
        return false;
    }

    bzero_memory(out_text, sizeof(ui_text));

    // Assign type first
    out_text->type = type;

    // Acquire font of correct type and assign its internal data.
    ...
    // In case of an empty string, cannot create an empty buffer so just create enough to hold one
    if (text_length < 1)
        text_length = 1;
    ...
    {
        BERROR("ui_text_create failed to bind vertex renderbuffer");
        return false;
    }
    ...
    {
        BERROR("ui_text_create failed to bind index renderbuffer");
        return false;
    }

    // Verify atlas has needed glyphs
...
void ui_text_destroy(ui_text* text)
{
    if (text)
    {
        ...
        if(text->name)
        {
            ...
            bfree(text->name, sizeof(char) * text_length + 1, MEMORY_TAG_STRING);
            text->name = 0;
        }
    ...
    bzero_memory(text, sizeof(ui_text));
}
...
static void regenerate_geometry(ui_text* text)
{
    ...
    vertex_2d* vertex_buffer_data = ballocate(vertex_buffer_size, MEMORY_TAG_ARRAY);
    u32* index_buffer_data = ballocate(index_buffer_size, MEMORY_TAG_ARRAY);
    ...
    // Clean up
    bfree(vertex_buffer_data, vertex_buffer_size, MEMORY_TAG_ARRAY);
    bfree(index_buffer_data, index_buffer_size, MEMORY_TAG_ARRAY);

    // Verify results
    if (!vertex_load_result)
        BERROR("regenerate_geometry failed to load data into vertex buffer range");
    if (!index_load_result)
        BERROR("regenerate_geometry failed to load data into index buffer range");
}
