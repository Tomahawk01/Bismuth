...
void linear_allocator_create(u64 total_size, void* memory, linear_allocator* out_allocator)
{
    if (out_allocator)
    {
        ...
        {
            out_allocator->memory = ballocate(total_size, MEMORY_TAG_LINEAR_ALLOCATOR);
        }
    }    
}
...
void linear_allocator_destroy(linear_allocator* allocator)
{
    if (allocator)
    {
        ...
        {
            bfree(allocator->memory, allocator->total_size, MEMORY_TAG_LINEAR_ALLOCATOR);
        }
        allocator->memory = 0;
...
void* linear_allocator_allocate(linear_allocator* allocator, u64 size)
{
    if (allocator && allocator->memory)
    {
        if (allocator->allocated + size > allocator->total_size)
        {
            ...
            BERROR("linear_allocator_allocate - Tried to allocate %lluB, only %lluB remaining", size, remaining);
            return 0;
        }
    ...
    BERROR("linear_allocator_allocate - Provided allocator not initialized");
    return 0;
}
...
void linear_allocator_free_all(linear_allocator* allocator, b8 clear)
{
    if (allocator && allocator->memory)
    {
        allocator->allocated = 0;
        if (clear)
            bzero_memory(allocator->memory, allocator->total_size);
    }
}
