...
void* dynamic_allocator_allocate(dynamic_allocator* allocator, u64 size)
{
    if (allocator && size)
    {
        ...
        {
            // Use that offset against base memory block to get the block
            void* block = (void*)(state->memory_block + offset);
            return block;
        }
        else
        {
            BERROR("dynamic_allocator_allocate no blocks of memory large enough to allocate from");
    ...
    BERROR("dynamic_allocator_allocate requires a valid allocator and size");
    return 0;
}
...
b8 dynamic_allocator_free(dynamic_allocator* allocator, void* block, u64 size)
{
    if (!allocator || !block)
    {
        BERROR("dynamic_allocator_free requires both a valid allocator (0x%p) and a block (0x%p) to be freed", allocator, block);
        return false;
    }
    ...
    if (block < state->memory_block || block > state->memory_block + state->total_size)
    {
        ...
        BERROR("dynamic_allocator_free trying to release block (0x%p) outside of allocator range (0x%p)-(0x%p)", block, state->memory_block, end_of_block);
        return false;
    }
    u64 offset = (block - state->memory_block);
    if (!freelist_free_block(&state->list, size, offset))
    {
        BERROR("dynamic_allocator_free failed");
        return false;
    }
