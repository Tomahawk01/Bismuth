...
material* material_system_acquire(const char* name)
{
    ...
    string_format(full_file_path, format_str, name, "bmt");
    if (!load_configuration_file(full_file_path, &config))
    {
        BERROR("Failed to load material file: '%s'. Null pointer will be returned", full_file_path);
        return 0;
    }
...
b8 create_default_material(material_system_state* state)
{
    ...
    {
        BFATAL("Failed to acquire renderer resources for default texture. Application cannot continue");
        return false;
    }
...
b8 load_configuration_file(const char* path, material_config* out_config)
{
    ...
    {
        BERROR("load_configuration_file - unable to open material file for reading: '%s'", path);
        return false;
    }
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length))
    {
        ...
        if (equal_index == -1)
        {
            BWARN("Potential formatting issue found in file '%s': '=' token not found. Skipping line %ui", path, line_number);
            line_number++;
            continue;
        }
        ...
        // Assume a max of 511-65 (446) for the max length of the value to account for the variable name and '='
        char raw_value[446];
        ...
        else if (strings_equali(trimmed_var_name, "diffuse_color"))
        {
            ...
            {
                BWARN("Error parsing diffuse_color in file '%s'. Using default of white instead", path);
