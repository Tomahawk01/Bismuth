...
material* material_system_acquire_from_config(material_config config)
{
    ...
    BERROR("material_system_acquire_from_config failed to acquire material '%s'. Null pointer will be returned", config.name);
    return 0;
}
...
b8 load_material(material_config config, material* m)
{
    ...
    if (string_length(config.diffuse_map_name) > 0)
    {
        ...
        {
            BWARN("Unable to load texture '%s' for material '%s', using default", config.diffuse_map_name, m->name);
    ...
    {
        BERROR("Unable to acquire resources for diffuse texture map");
        return false;
    }
    ...
    if (string_length(config.specular_map_name) > 0)
    {
        ...
        {
            BWARN("Unable to load specular texture '%s' for material '%s', using default", config.specular_map_name, m->name);
    ...
    {
        BERROR("Unable to acquire resources for specular texture map");
        return false;
    }
    ...
    if (string_length(config.normal_map_name) > 0)
    {
        ...
        {
            BWARN("Unable to load normal texture '%s' for material '%s', using default", config.normal_map_name, m->name);
    ...
    {
        BERROR("Unable to acquire resources for normal texture map");
        return false;
    }

    // TODO: other maps

    // Send it off to renderer to acquire resources
    ...
    // Gather list of pointers to texture maps
