...
typedef struct material_system_state
{
    ...
    // Known locations for material shader
    ...
    // Known locations for PBR shader
...
static b8 material_system_on_event(u16 code, void* sender, void* listener_inst, event_context context)
{
    if (code == EVENT_CODE_BVAR_CHANGED)
    {
        ...
        {
            bvar_int_get("use_pcf", &state_ptr->use_pcf);
            return true;
        }
    }
...
b8 material_system_initialize(u64* memory_requirement, void* state, void* config)
{
    ...
    *memory_requirement = struct_requirement + array_requirement + hashtable_requirement;
    ...
    state_ptr->pbr_locations.ambient_color = INVALID_ID_U16;
    ...
    state_ptr->terrain_locations.ambient_color = INVALID_ID_U16;
    ...
    {
        BFATAL("Failed to create default material. Application cannot continue");
        return false;
    }
    ...
    state_ptr->pbr_locations.ambient_color = shader_system_uniform_index(s, "ambient_color");
    ...
    state_ptr->terrain_locations.ambient_color = shader_system_uniform_index(s, "ambient_color");
    ...
    // NOTE: Add bvar to track PCF filtering enabled/disabled
    bvar_int_create("use_pcf", 1);  // On by default
    bvar_int_get("use_pcf", &state_ptr->use_pcf);

    event_register(EVENT_CODE_BVAR_CHANGED, 0, material_system_on_event);
...
void material_system_shutdown(void* state)
{
    ...
    if (s)
    {
        event_unregister(EVENT_CODE_BVAR_CHANGED, 0, material_system_on_event);
...
material* material_system_acquire_terrain_material(const char* material_name, u32 material_count, const char** material_names, b8 auto_release)
{
    ...
    if (needs_creation)
    {
        ...
        material** materials = ballocate(sizeof(material*) * material_count, MEMORY_TAG_ARRAY);
        ...
        // 5 maps per material for PBR. Allocate enough slots for all materials. Also 1 for irradiance map
        ...
        // Use default material for unassigned slots
        ...
        for (u32 material_idx = 0; material_idx < TERRAIN_MAX_MATERIAL_COUNT; ++material_idx)
        {
            ...
            mat_props->diffuse_color = props->diffuse_color;
            ...
            for (u32 map_idx = 0; map_idx < TERRAIN_PER_MATERIAL_SAMP_COUNT; ++map_idx)
            {
                ...
                {
                    BERROR("Failed to assign '%s' texture map for terrain material index %u", map_names[map_idx], material_idx);
                    return false;
                }
            }
        }
        ...
        texture_map** maps = ballocate(sizeof(texture_map*) * TERRAIN_PER_MATERIAL_SAMP_COUNT, MEMORY_TAG_ARRAY);
        // Assign material maps
        for (u32 i = 0; i < TERRAIN_PER_MATERIAL_SAMP_COUNT; ++i)
            maps[i] = &m->maps[i];

        b8 result = renderer_shader_instance_resources_acquire(s, TERRAIN_PER_MATERIAL_SAMP_COUNT, maps, &m->internal_id);
        if (!result)
            BERROR("Failed to acquire renderer resources for material '%s'.", m->name);

        if (maps)
            bfree(maps, sizeof(texture_map*) * TERRAIN_PER_MATERIAL_SAMP_COUNT, MEMORY_TAG_ARRAY);
...
material* material_system_get_default(void)
{
    ...
    BFATAL("material_system_get_default called before system is initialized");
    return 0;
}
...
#define MATERIAL_APPLY_OR_FAIL(expr)                  \
    if (!expr)                                        \
    {                                                 \
        BERROR("Failed to apply material: %s", expr); \
        return false;                                 \
    }
...
b8 material_system_apply_global(u32 shader_id, const struct frame_data* p_frame_data, const mat4* projection, const mat4* view, const vec4* ambient_color, const vec3* view_position, u32 render_mode)
{
    ...
    else if (shader_id == state_ptr->terrain_shader_id)
    {
        ...
        MATERIAL_APPLY_OR_FAIL(shader_system_uniform_set_by_index(state_ptr->terrain_locations.ambient_color, ambient_color));
    ...
    else if (shader_id == state_ptr->pbr_shader_id)
    {
        ...
        MATERIAL_APPLY_OR_FAIL(shader_system_uniform_set_by_index(state_ptr->pbr_locations.ambient_color, ambient_color));
    ...
    {
        BERROR("material_system_apply_global(): Unrecognized shader id '%d'", shader_id);
        return false;
    }
...
b8 material_system_apply_instance(material* m, struct frame_data* p_frame_data, b8 needs_update)
{
    ...
    if (needs_update)
    {
        ...
        else if (m->shader_id == state_ptr->terrain_shader_id)
        {
            ...
            MATERIAL_APPLY_OR_FAIL(shader_system_uniform_set_by_index(state_ptr->terrain_locations.ibl_cube_texture, &m->maps[SAMP_TERRAIN_IRRADIANCE_MAP]));

            // Apply properties
            shader_system_uniform_set_by_index(state_ptr->terrain_locations.properties, m->properties);

            // TODO: Duplicating above... move to its own function
        ...
        {
            BERROR("material_system_apply_instance(): Unrecognized shader id '%d' on shader '%s'", m->shader_id, m->name);
            return false;
        }
    }
...
b8 material_system_irradiance_set(texture* irradiance_cube_texture)
{
    if (irradiance_cube_texture)
    {
        ...
        {
            BWARN("material_system_irradiance_set requires parameter irradiance_cube_texture to be a cubemap type texture. Nothing was done");
            return false;
        }
...
static b8 load_material(material_config* config, material* m)
{
    ...
    {
        // NOTE: only one map and property, so just use first
        m->maps = darray_reserve(texture_map, 1);
        darray_length_set(m->maps, 1);
        m->property_struct_size = sizeof(material_ui_properties);
        m->properties = ballocate(sizeof(material_ui_properties), MEMORY_TAG_MATERIAL_INSTANCE);
        material_ui_properties* properties = m->properties;
        properties->diffuse_color = config->properties[0].value_v4;
    ...
    if (!s)
    {
        BERROR("Unable to load material because its shader was not found: '%s'. This is likely a problem with the material asset", config->shader_name);
        return false;
    }
    ...
    texture_map** maps = ballocate(sizeof(texture_map*) * map_count, MEMORY_TAG_ARRAY);
    for (u32 i = 0; i < map_count; ++i)
        maps[i] = &m->maps[i];
    ...
    if (maps)
        bfree(maps, sizeof(texture_map*) * map_count, MEMORY_TAG_ARRAY);
...
static b8 create_default_material(material_system_state* state)
{
    bzero_memory(&state->default_material, sizeof(material));
    ...
    state->default_material.properties = ballocate(sizeof(material_phong_properties), MEMORY_TAG_MATERIAL_INSTANCE);
...
static b8 create_default_pbr_material(material_system_state* state)
{
    ...
    texture_map* maps[PBR_MAP_COUNT] = {
        &state->default_pbr_material.maps[SAMP_ALBEDO],
        &state->default_pbr_material.maps[SAMP_NORMAL],
...
static b8 create_default_terrain_material(material_system_state* state)
{
    ...
    {
        BFATAL("Failed to acquire renderer resources for default terrain material. Application cannot continue");
        return false;
    }
