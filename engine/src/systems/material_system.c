...
b8 material_system_apply_global(u32 shader_id, const struct frame_data* p_frame_data, const mat4* projection, const mat4* view, const vec4* ambient_color, const vec3* view_position, u32 render_mode)
{
    ...
    else if (shader_id == state_ptr->terrain_shader_id)
    {
        ...
        MATERIAL_APPLY_OR_FAIL(shader_system_uniform_set_by_index(state_ptr->terrain_locations.cascade_splits, ambient_color));
    ...
    else if (shader_id == state_ptr->pbr_shader_id)
    {
        ...
        // TODO: set cascade splits like dir lights and shadow map, etc
        MATERIAL_APPLY_OR_FAIL(shader_system_uniform_set_by_index(state_ptr->pbr_locations.cascade_splits, ambient_color));
