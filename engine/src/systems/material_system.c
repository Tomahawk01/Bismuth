...
material* material_system_acquire_terrain_material(const char* material_name, u32 material_count, const char** material_names, b8 auto_release)
{
    ...
    if (needs_creation)
    {
        ...
        // NOTE: 4 materials * 5 maps will still be loaded in order (albedo/norm/met/rough/ao per mat)
...
b8 material_system_apply_global(u32 shader_id, struct frame_data* p_frame_data, const mat4* projection, const mat4* view, const vec4* ambient_color, const vec3* view_position, u32 render_mode)
{
    ...
    if (shader_id == state_ptr->terrain_shader_id)
    {
        ...
        MATERIAL_APPLY_OR_FAIL(shader_system_uniform_set_by_location(state_ptr->terrain_locations.cascade_splits, ambient_color));
    ...
    else if (shader_id == state_ptr->pbr_shader_id)
    {
        ...
        // TODO: set cascade splits like dir lights and shadow map, etc
        MATERIAL_APPLY_OR_FAIL(shader_system_uniform_set_by_location(state_ptr->pbr_locations.cascade_splits, ambient_color));
