...
{
    f32 volume;
    // Currently bound sound/stream
    struct audio_file* current;
    // Currently bound audio emitter
...
b8 audio_system_initialize(u64* memory_requirement, void* state, void* config)
{
    ...
    {
        BERROR("Audio system initialization requires valid pointers to memory_requirement and config");
        return false;
    }
    ...
    *memory_requirement = struct_requirement;
    ...
    plugin_config.max_sources = typed_config->audio_channel_count;
...
{
    audio_system_state* state = systems_manager_get_state(B_SYSTEM_TYPE_AUDIO);
...
void audio_system_master_volume_set(f32 volume)
{
    ...
    for (u32 i = 0; i < state->config.audio_channel_count; ++i)
    {
        ...
        {
            // Take emitter's volume into account if there is one
...
b8 audio_system_channel_volume_set(i8 channel_id, f32 volume)
{
    ...
    {
        BERROR("Channel id %u is outside the range of available channels. Nothing was done", channel_id);
        return false;
    }

    state->channels[channel_id].volume = BCLAMP(volume, 0.0f, 1.0f);
    // Apply channel volume, taking the master volume into account
...
b8 audio_system_channel_play(i8 channel_id, struct audio_file* file, b8 loop)
{
    ...
    // If -1 is passed, use first available channel
    if (channel_id == -1)
    {
        for (u32 i = 0; i < state->config.audio_channel_count; ++i)
        {
    ...
    if (channel_id == -1)
    {
        // This means all channels are playing something
        BWARN("No channel available for playback. Dropping sound");
        return false;
    }
    ...
    {
        // Set position to the listener position
        vec3 position;
...
b8 audio_system_channel_emitter_play(i8 channel_id, struct audio_emitter* emitter)
{
    ...
    {
        // This means all channels are playing something
        BWARN("No channel available for playback. Dropping emitter");
        return false;
    }
    audio_channel* channel = &state->channels[channel_id];
