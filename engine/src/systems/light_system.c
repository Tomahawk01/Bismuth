...
b8 light_system_add_point(point_light* light)
{
    ...
    BERROR("light_system_add_point already has the max of %u lights. Light not added", MAX_POINT_LIGHTS);
    return false;
}
...
b8 light_system_remove_point(point_light* light)
{
    ...
    BERROR("light_system_remove_point does not have a light that matches the one passed, it cannot be removed");
    return false;
}
