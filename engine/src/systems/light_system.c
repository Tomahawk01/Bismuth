...
b8 light_system_point_add(point_light* light)
{
    ...
    BERROR("light_system_point_add already has the max of %u lights. Light not added", MAX_POINT_LIGHTS);
    return false;
}
...
b8 light_system_point_remove(point_light* light)
{
    ...
    BERROR("light_system_point_remove does not have a light that matches the one passed, it cannot be removed");
    return false;
}
