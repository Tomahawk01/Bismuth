...
b8 shader_system_initialize(u64* memory_requirement, void* memory, shader_system_config config)
{
    ...
    if (config.max_shader_count < 512)
    {
        ...
        {
            BERROR("shader_system_initialize - config.max_shader_count must be greater than 0");
            return false;
        }
        else
        {
            // This is to help avoid hashtable collisions
            BWARN("shader_system_initialize - config.max_shader_count is recommended to be at least 512");
        }
    }
...
b8 shader_system_create(const shader_config* config)
{
    ...
    bzero_memory(out_shader->push_constant_ranges, sizeof(range) * 32);
    ...
    out_shader->hashtable_block = ballocate(element_size * element_count, MEMORY_TAG_UNKNOWN);
    ...
    {
        BERROR("Unable to find renderpass '%s'", config->renderpass_name);
        return false;
    }
...
void shader_destroy(shader* s)
{
    ...
    for (u32 i = 0; i < sampler_count; ++i)
        bfree(s->global_texture_maps[i], sizeof(texture_map), MEMORY_TAG_RENDERER);
...
{
    // Only perform the use if shader id is different
    if (state_ptr->current_shader_id != shader_id)
    {
        ...
        {
            BERROR("Failed to bind globals for shader '%s'", next_shader->name);
            return false;
        }
    }
    return true;
...
u16 shader_system_uniform_index(shader* s, const char* uniform_name)
{
    ...
    {
        BERROR("shader_system_uniform_location called with invalid shader");
...
b8 shader_system_uniform_set(const char* uniform_name, const void* value)
{
    ...
    {
        BERROR("shader_system_uniform_set called without a shader in use");
        return false;
    }
...
b8 shader_system_uniform_set_by_index(u16 index, const void* value)
{
    ...
    if (shader->bound_scope != uniform->scope)
    {
        ...
        {
            // NOTE: Nothing to do here for locals, just set uniform
        }
...
b8 add_sampler(shader* shader, shader_uniform_config* config)
{
    ...
    {
        BERROR("add_sampler cannot add an instance sampler for a shader that does not use instances");
        return false;
    }
    ...
    if (config->scope == SHADER_SCOPE_GLOBAL)
    {
        ...
        // NOTE: This allocation is onlu done for global texture maps
        texture_map* map = ballocate(sizeof(texture_map), MEMORY_TAG_RENDERER);
        *map = default_map;
...
b8 uniform_add(shader* shader, const char* uniform_name, u32 size, shader_uniform_type type, shader_scope scope, u32 set_location, b8 is_sampler)
{
    ...
    else
    {
        ...
        {
            BERROR("Cannot add a locally-scoped uniform for a shader that does not support locals");
            return false;
        }
        ...
        // Increase push constant's size by the total value
