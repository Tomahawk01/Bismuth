...
b8 render_view_system_initialize(u64* memory_requirement, void* state, void* config)
{
    ...
    *memory_requirement = struct_requirement + hashtable_requirement + array_requirement;
    ...
    // Fill array with invalid entries
...
b8 render_view_system_create(const render_view_config* config)
{
    if (!config)
    {
        BERROR("render_view_system_create requires a pointer to a valid config");
        return false;
    }
    ...
    {
        BERROR("render_view_system_create - A view named '%s' already exists. A new one will not be created", config->name);
        return false;
    }
    ...
    if (id == INVALID_ID_U16)
    {
        BERROR("render_view_system_create - No available space for a new view. Change system config to account for more");
        return false;
    }
    ...
    // Create renderpasses according to config
    for (u32 i = 0; i < view->renderpass_count; ++i)
    {
        if (!renderer_renderpass_create(&config->passes[i], &view->passes[i]))
        {
            BERROR("render_view_system_create - Failed to create renderpass '%s'", config->passes[i].name);
            return false;
        }
    }
    ...
    {
        view->on_packet_build = render_view_world_on_packet_build;      // For building packet
        view->on_packet_destroy = render_view_world_on_packet_destroy;  // For destroying packet
    ...
    {
        view->on_packet_build = render_view_ui_on_packet_build;         // For building packet
        view->on_packet_destroy = render_view_ui_on_packet_destroy;     // For destroying packet
    ...
    {
        view->on_packet_build = render_view_skybox_on_packet_build;     // For building packet
        view->on_packet_destroy = render_view_skybox_on_packet_destroy; // For destroying packet
        view->on_render = render_view_skybox_on_render;                 // For rendering packet
    ...
    {
        view->on_packet_build = render_view_pick_on_packet_build;      // For building packet
        view->on_packet_destroy = render_view_pick_on_packet_destroy;  // For destroying packet
        view->on_render = render_view_pick_on_render;                  // For rendering packet
