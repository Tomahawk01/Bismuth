...
b8 font_system_initialize(u64* memory_requirement, void* memory, font_system_config* config)
{
    ...
    {
        BFATAL("font_system_initialize - config.max_bitmap_font_count and config.max_system_font_count must be > 0");
        return false;
    }
...
b8 font_system_acquire(const char* font_name, u16 font_size, struct ui_text* text)
{
    if (text->type == UI_TEXT_TYPE_BITMAP)
    {
        ...
        // Assign data, increment reference
    ...
    BERROR("Unrecognized font type: %d", text->type);
    return false;
}
...
b8 rebuild_system_font_variant_atlas(system_font_lookup* lookup, font_data* variant)
{
    ...
    if (variant->kerning_count)
    {
        ...
        {
            BERROR("Kerning entry count mismatch: %i->%i", entry_count, variant->kerning_count);
            return false;
        }
