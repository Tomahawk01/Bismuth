...
static b8 create_default_cube_texture(texture* t, const char* name)
{
    ...
    for (u8 i = 0; i < 6; ++i)
    {
        if (!pixels)
        {
            ...
            pixels = ballocate(sizeof(u8) * image_size * 6, MEMORY_TAG_ARRAY);
        }

        // Copy to the relevant portion of the array
        bcopy_memory(pixels + image_size * i, cube_side_pixels, image_size);
    }

    // Acquire internal texture resources and upload to GPU
    renderer_texture_create(pixels, t);

    bfree(pixels, sizeof(u8) * image_size * 6, MEMORY_TAG_ARRAY);
    pixels = 0;
...
static b8 load_cube_textures(const char texture_names[6][TEXTURE_NAME_MAX_LENGTH], texture* t)
{
    ...
    for (u8 i = 0; i < 6; ++i)
    {
        ...
        {
            BERROR("load_cube_textures() - Failed to load image resource for texture '%s'", texture_names[i]);
            return false;
        }
        ...
        else
        {
            ...
            {
                BERROR("load_cube_textures - All textures must be the same resolution and bit depth");
                bfree(pixels, sizeof(u8) * image_size * 6, MEMORY_TAG_ARRAY);
                pixels = 0;
                return false;
            }
        ...
        bcopy_memory(pixels + image_size * i, resource_data->pixels, image_size);
...
static b8 texture_load_layered_job_start(void* params, void* result_data)
{
    ...
    for (; layer < load_params->layer_count; ++layer)
    {
        ...
        bcopy_memory(data_location, resource_data->pixels, layer_size);
