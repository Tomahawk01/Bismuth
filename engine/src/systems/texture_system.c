...
void texture_system_release(const char* name)
{
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref))
    {
        ...
        if (ref.reference_count == 0 && ref.auto_release)
        {
            ...
            // Reset array entry, ensure invalid ids are set
            ...
            BTRACE("Released texture '%s', Texture unloaded because reference count=0 and auto_release=true", name);
        }
        else
        {
            BTRACE("Released texture '%s', now has a reference count of '%i' (auto_release=%s)", name, ref.reference_count, ref.auto_release ? "true" : "false");
        }
...
b8 load_texture(const char* texture_name, texture* t)
{
    ...
    if (data)
    {
        ...
        if (stbi_failure_reason())
            BWARN("load_texture() failed to load file '%s': %s", full_file_path, stbi_failure_reason());
