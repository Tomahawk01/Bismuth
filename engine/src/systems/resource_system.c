...
b8 resource_system_initialize(u64* memory_requirement, void* state, void* config)
{
    ...
    {
        BFATAL("resource_system_initialize failed because typed_config->max_loader_count==0");
        return false;
    }
...
b8 resource_system_loader_register(resource_loader loader)
{
    if (state_ptr)
    {
        ...
        // Ensure no loaders for given type already exist
        for (u32 i = 0; i < count; ++i)
        {
            ...
            if (l->id != INVALID_ID)
            {
                ...
                {
                    BERROR("resource_system_loader_register - Loader of type %d already exists and will not be registered", loader.type);
                    return false;
                }
                else if (loader.custom_type && string_length(loader.custom_type) > 0 && strings_equali(l->custom_type, loader.custom_type))
                {
                    BERROR("resource_system_loader_register - Loader of custom type %s already exists and will not be registered", loader.custom_type);
                    return false;
                }
            }
        }
...
const char* resource_system_base_path(void)
{
    ...
    BERROR("resource_system_base_path called before initialization, returning empty string");
    return "";
}
