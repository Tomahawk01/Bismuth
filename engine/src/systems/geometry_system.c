...
geometry* geometry_system_get_default_2d(void)
{
    ...
    BFATAL("geometry_system_get_default_2d called before system was initialized. Returning nullptr");
    return 0;
}
...
static b8 create_default_geometries(geometry_system_state* state)
{
    ...
    vertex_2d verts2d[4];
    bzero_memory(verts2d, sizeof(vertex_2d) * 4);
    ...
    {
        BFATAL("Failed to create default 2d geometry. Application cannot continue");
        return false;
    }
    if (!renderer_geometry_upload(&state->default_2d_geometry))
    {
        BFATAL("Failed to upload default 2d geometry. Application cannot continue");
        return false;
    }
