...
typedef struct application_state
{
    ...
    // Unique identifier of currently hovered-over object
...
b8 event_on_debug_event(u16 code, void* sender, void* listener_inst, event_context data)
{
    if (code == EVENT_CODE_DEBUG0)
    {
        ...
        // Just swap out material on the first mesh if it exists
...
b8 application_create(game* game_inst)
{
    ...
    // Allocate game state
    game_inst->state = ballocate(game_inst->state_memory_requirement, MEMORY_TAG_GAME);

    // Stand up the application state
    game_inst->application_state = ballocate(sizeof(application_state), MEMORY_TAG_APPLICATION);
    ...
    skybox_passes[0].clear_color = (vec4){0.0f, 0.0f, 0.2f, 1.0f};
    skybox_passes[0].clear_flags = RENDERPASS_CLEAR_COLOR_BUFFER_FLAG;
    skybox_passes[0].depth = 1.0f;
    skybox_passes[0].stencil = 0;

    render_target_attachment_config skybox_target_attachment = {};
    // Color attachment
    skybox_target_attachment.type = RENDER_TARGET_ATTACHMENT_TYPE_COLOR;
    ...
    {
        BFATAL("Failed to create skybox view. Aborting application...");
        return false;
    }
    ...
    world_passes[0].clear_color = (vec4){0.0f, 0.0f, 0.2f, 1.0f};
    ...
    // Color attachment
    world_target_attachments[0].type = RENDER_TARGET_ATTACHMENT_TYPE_COLOR;
    ...
    {
        BFATAL("Failed to create world view. Aborting application...");
        return false;
    }
    ...
    ui_passes[0].clear_color = (vec4){0.0f, 0.0f, 0.2f, 1.0f};
    ...
    app_state->ui_meshes[0].geometries = ballocate(sizeof(geometry*), MEMORY_TAG_ARRAY);
...
b8 application_run()
{
    ...
    while (app_state->is_running)
    {
        ...
        if (!app_state->is_suspended)
        {
            ...
            // Perform similar rotation on second mesh
            transform_rotate(&app_state->meshes[1].transform, rotation);
            // Perform similar rotation on third mesh
...
b8 application_on_resized(u16 code, void* sender, void* listener_inst, event_context context)
{
    if (code == EVENT_CODE_RESIZED)
    {
        ...
        if (width != app_state->width || height != app_state->height)
        {
            ...
            // TODO: temp
            // Move debug text to bottom of the screen
            ui_text_set_position(&app_state->test_text, vec3_create(20, app_state->height - 75, 0));
            // TODO: end temp
        }
    }
