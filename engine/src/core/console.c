...
b8 console_initialize(u64* memory_requirement, void* memory, void* config)
{
    *memory_requirement = sizeof(console_state) + (sizeof(console_consumer) * MAX_CONSUMER_COUNT);
    ...
    bzero_memory(memory, *memory_requirement);
    state_ptr = memory;
...
void console_shutdown(void* state)
{
    if (state_ptr)
    {
        ...
        bzero_memory(state, sizeof(console_state) + (sizeof(console_consumer) * MAX_CONSUMER_COUNT));
    }
...
void console_consumer_register(void* inst, PFN_console_consumer_write callback, u8* out_consumer_id)
{
    if (state_ptr)
    {
        BASSERT_MSG(state_ptr->consumer_count + 1 < MAX_CONSUMER_COUNT, "Max console consumers reached");
...
void console_write_line(log_level level, const char* message)
{
    if (state_ptr)
    {
        // Notify each consumer that line has been added
...
b8 console_command_register(const char* command, u8 arg_count, PFN_console_command func)
{
    ...
    for (u32 i = 0; i < command_count; ++i)
    {
        ...
        {
            BERROR("Command already registered: %s", command);
            return false;
        }
    }
...
static void console_object_print(u8 indent, console_object* obj)
{
    ...
    switch (obj->type)
    {
        case CONSOLE_OBJECT_TYPE_INT32:
            BINFO("%s%i", indent_buffer, *((i32*)obj->block));
            break;
        case CONSOLE_OBJECT_TYPE_UINT32:
            BINFO("%s%u", indent_buffer, *((u32*)obj->block));
            break;
...
static b8 console_operator_type_from_token(const console_token token, const char* source, console_operator_type* out_type)
{
    ...
    {
        BERROR("console_operator_type_from_token requires a valid pointer to source and an operator type token");
        return false;
    }
    ...
    {
        BERROR("operators are always either 1 or 2 characters");
        return false;
    }
    ...
    *out_type = CONSOLE_OPERATOR_TYPE_UNKNOWN;
    ...
    if (!result)
        BERROR("Unexpected character '%c' at position %u", source[token.start_index + error_offset], token.start_index + error_offset);
...
static b8 tokenize_source(const char* source, stack* out_tokens)
{
    ...
    {
        BERROR("tokenize_source requires valid pointers to source and out_tokens");
        return false;
    }
    ...
    for (u32 i = 0; i < char_length; ++i)
    {
        ...
        {
            BERROR("Error processing expression - unable to decode codepoint at position: %u", i);
            return false;
        }
        ...
        switch (codepoint)
        {
            ...
            case '%':
            {
                ...
                else
                {
                    ...
                    {
                        BERROR("Invalid character at position %u", i);
                        return false;
                    }
                        ...
                        case '=':
                        {
                            ...
                            switch (prev)
                            {
                                ...
                                case '%':
                                    ...
                                    break;
                                default:
                                    BERROR("Unexpected character at position %u", i);
                                    return false;
                            }
                        }
                        case '+':
            ...
            default:
            {
                // Any other character should be treated as part of a string or an identifier, depending on what kind of token we are currently in
...
static b8 numeric_token_parse(const char* source, const console_token* token, console_object_type* out_type, console_numeric* out_result)
{
    ...
    if (token->type == CONSOLE_TOKEN_TYPE_NUMERIC_LITERAL)
    {
        ...
        char* value = ballocate(sizeof(char) * token->length + 1, MEMORY_TAG_STRING);
        if (string_index_of(value, '.'))
        {
            if (!string_to_f32(value, &out_result->f))
            {
                BERROR("Failed to parse token as float at position %i", token->start_index);
                result = false;
            }
    ...
    else if (token->type == CONSOLE_TOKEN_TYPE_IDENTIFIER)
    {
        ...
        else
        {
            switch (obj->type)
            {
                ...
                default:
                    // Wrong type of token
                    BERROR("Expected numeric token (i32, u32 or f32) at position %i", token->start_index);
                    result = false;
                    break;
            }
        }
        bfree(name, sizeof(char) * token->length + 1, MEMORY_TAG_STRING);
    }
...
static b8 evaluate_numerical_expression(const char* source, const console_token* a, const console_token* operator, const console_token* b, console_object_type* out_type, console_numeric* out_result)
{
    ...
    {
        BERROR("Failed to evaluate numerical expression due to invalid operator token");
        return false;
    }
    ...
    {
        BERROR("Failed to evaluate numerical expression. See logs for details");
        return false;
    }
    ...
    if (is_unary)
    {
        if (type_a == CONSOLE_OBJECT_TYPE_F32)
        {
            ...
            {
                BWARN("Incrementing on float at position %i. May yield unexpected results", a->start_index);
                out_result->f = numeric_a.f + 1;
            }
            else if (operator_type == CONSOLE_OPERATOR_TYPE_DECREMENT)
            {
                BWARN("Decrementing on float at position %i. May yield unexpected results", a->start_index);
                out_result->f = numeric_a.f - 1;
            }
            else
            {
                BERROR("Unexpected unary operator at position %i", a->start_index);
                return false;
            }
        }
        else
    ...
    else
    {
        ...
        if (type_a == CONSOLE_OBJECT_TYPE_F32 || type_b == CONSOLE_OBJECT_TYPE_F32)
        {
            *out_type = CONSOLE_OBJECT_TYPE_F32;
            ...
            {
                BERROR("Unsupported object type at position %i", a->start_index);
                return false;
            }
            ...
            {
                BERROR("Unsupported object type at position %i", b->start_index);
                return false;
            }
...
static b8 console_expression_parse2(const char* expression_str, console_object_type* out_type, void* out_value)
{
    ...
    // NOTE: keep track of the original pointer address since string_trim works by moving the pointer forward on the left side
...
static b8 console_expression_parse(const char* expression, console_object_type* out_type, void* out_value)
{
    ...
    if (!operator_found)
    {
        ...
        if (space_index != -1)
        {
            BERROR("Unexpected token at position %i", space_index);
            result = false;
        ...
        if (!identifier_found)
        {
            BERROR("Identifier is undefined: '%s'", expression_copy);
            result = false;
            goto console_expression_parse_cleanup;
        }
    }
...
b8 console_command_execute(const char* command)
{
    ...
    void* block = ballocate(sizeof(void*), MEMORY_TAG_ARRAY);
    if (console_expression_parse(command, &parsed_type, block))
    {
        // TODO: cast to appropriate type and use

        has_error = false;
        goto console_command_execute_cleanup;
    }

    // Write line back out to console for reference
    char temp[512] = {0};
    ...
    for (u32 i = 0; i < command_count; ++i)
    {
        ...
        if (strings_equali(cmd->name, parts[0]))
        {
            ...
            {
                BERROR("The console command '%s' requires %u arguments but %u were provided", cmd->name, cmd->arg_count, arg_count);
                has_error = true;
            }
            else
            {
                ...
                {
                    context.arguments = ballocate(sizeof(console_command_argument) * cmd->arg_count, MEMORY_TAG_ARRAY);
                ...
                if (context.arguments)
                    bfree(context.arguments, sizeof(console_command_argument) * cmd->arg_count, MEMORY_TAG_ARRAY);
            }
            break;
        }
    }
...
b8 console_object_register(const char* object_name, void* object, console_object_type type)
{
    ...
    {
        BERROR("console_object_register requires a valid pointer to object and object_name");
        return false;
    }
    ...
    for (u32 i = 0; i < object_count; ++i)
    {
        ...
        {
            BERROR("Console object already registered: '%s'", object_name);
            return false;
        }
    }
...
b8 console_object_unregister(const char* object_name)
{
    if (!object_name)
    {
        BERROR("console_object_register requires a valid pointer object_name");
        return false;
    }
...
b8 console_object_add_property(const char* object_name, const char* property_name, void* property, console_object_type type)
{
    ...
    {
        BERROR("console_object_add_property requires a valid pointer to property, property_name and object_name");
        return false;
    }
        ...
        if (strings_equali(state_ptr->registered_objects[i].name, object_name))
        {
            ...
            if (obj->properties)
            {
                ...
                for (u32 j = 0; j < property_count; ++j)
                {
                    ...
                    {
                        BERROR("Object '%s' already has a property named '%s'", object_name, property_name);
                        return false;
                    }
                }
            }
    ...
    BERROR("Console object not found: '%s'.", object_name);
    return false;
}
...
b8 console_object_remove_property(const char* object_name, const char* property_name)
{
    ...
    {
        BERROR("console_object_remove_property requires a valid pointer to property, property_name and object_name");
        return false;
    }
    ...
    for (u32 i = 0; i < object_count; ++i)
    {
        if (strings_equali(state_ptr->registered_objects[i].name, object_name))
        {
            ...
            BERROR("Property '%s' not found on console object '%s'", object_name, property_name);
            return false;
        }
    }
