...
BAPI void console_register_consumer(void* inst, PFN_console_consumer_write callback, u8* out_consumer_id);
BAPI void console_update_consumer(u8 consumer_id, void* inst, PFN_console_consumer_write callback);

void console_write_line(log_level level, const char* message);

BAPI b8 console_register_command(const char* command, u8 arg_count, PFN_console_command func);
BAPI b8 console_unregister_command(const char* command);

BAPI b8 console_execute_command(const char* command);
