    ...
    for (u32 i = 0; i < B_SYSTEM_TYPE_MAX_COUNT; ++i)
    {
        ...
        if (s->update)
        {
            if (!s->update(s->state, p_frame_data))
                BERROR("System update failed for type: %i", i);
        }
    }
    return true;
}
    ...
    void* config)
{
    ...
    // Call initialize, alloc memory, call initialize again with allocated block
    if (initialize)
    {
        sys->initialize = initialize;
        if (!sys->initialize(&sys->state_size, 0, config))
        {
            BERROR("Failed to register system - initialize call failed");
            return false;
        }
        ...
        {
            BERROR("Failed to register system - initialize call failed");
            bzero_memory(sys, sizeof(b_system));
            return false;
        }
    }
    else
    {
        if (type != B_SYSTEM_TYPE_MEMORY)
        {
            BERROR("Initialize is required for types except B_SYSTEM_TYPE_MEMORY");
            return false;
        }
    }
...
static b8 register_known_systems_pre_boot(systems_manager_state* state, application_config* app_config)
{
    ...
    {
        // Dedicate the first 2 threads to these things, pass general tasks to other threads
        job_thread_types[0] = JOB_TYPE_GPU_RESOURCE;
        job_thread_types[1] = JOB_TYPE_RESOURCE_LOAD;
    }
...
static void shutdown_known_systems(systems_manager_state* state)
{
    ...
    state->systems[B_SYSTEM_TYPE_RENDER_VIEW].shutdown(state->systems[B_SYSTEM_TYPE_RENDER_VIEW].state);
    state->systems[B_SYSTEM_TYPE_GEOMETRY].shutdown(state->systems[B_SYSTEM_TYPE_GEOMETRY].state);
    state->systems[B_SYSTEM_TYPE_MATERIAL].shutdown(state->systems[B_SYSTEM_TYPE_MATERIAL].state);
    state->systems[B_SYSTEM_TYPE_TEXTURE].shutdown(state->systems[B_SYSTEM_TYPE_TEXTURE].state);

    state->systems[B_SYSTEM_TYPE_JOB].shutdown(state->systems[B_SYSTEM_TYPE_JOB].state);
    state->systems[B_SYSTEM_TYPE_SHADER].shutdown(state->systems[B_SYSTEM_TYPE_SHADER].state);
    state->systems[B_SYSTEM_TYPE_RENDERER].shutdown(state->systems[B_SYSTEM_TYPE_RENDERER].state);

    state->systems[B_SYSTEM_TYPE_RESOURCE].shutdown(state->systems[B_SYSTEM_TYPE_RESOURCE].state);
    state->systems[B_SYSTEM_TYPE_PLATFORM].shutdown(state->systems[B_SYSTEM_TYPE_PLATFORM].state);
    state->systems[B_SYSTEM_TYPE_INPUT].shutdown(state->systems[B_SYSTEM_TYPE_INPUT].state);
    state->systems[B_SYSTEM_TYPE_LOGGING].shutdown(state->systems[B_SYSTEM_TYPE_LOGGING].state);
    state->systems[B_SYSTEM_TYPE_EVENT].shutdown(state->systems[B_SYSTEM_TYPE_EVENT].state);
    state->systems[B_SYSTEM_TYPE_BVAR].shutdown(state->systems[B_SYSTEM_TYPE_BVAR].state);
...
static b8 register_known_systems_post_boot(systems_manager_state* state, application_config* app_config)
{
    ...
    if (!systems_manager_register(state, B_SYSTEM_TYPE_RENDER_VIEW, render_view_system_initialize, render_view_system_shutdown, 0, &render_view_sys_config))
    {
        BERROR("Failed to register render view system");
        return false;
    }
    ...
    for (u32 v = 0; v < view_count; ++v)
    {
        ...
        {
            BFATAL("Failed to register view '%s'. Aborting application", view->name);
            return false;
        }
    }
