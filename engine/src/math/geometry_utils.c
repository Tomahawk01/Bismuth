...
void geometry_generate_normals(u32 vertex_count, vertex_3d *vertices, u32 index_count, u32 *indices)
{
    for (u32 i = 0; i < index_count; i += 3)
    {
        ...
        // NOTE: This generates a face normal. Smoothing out should be done in a separate pass
...
void terrain_geometry_generate_normals(u32 vertex_count, terrain_vertex *vertices, u32 index_count, u32 *indices)
{
    for (u32 i = 0; i < index_count; i += 3)
    {
        ...
        // NOTE: This generates a face normal. Smoothing out should be done in a separate pass if desired
        vertices[i0].normal = normal;
        vertices[i1].normal = normal;
        vertices[i2].normal = normal;
    }
...
void generate_quad_2d(const char *name, f32 width, f32 height, f32 tx_min, f32 tx_max, f32 ty_min, f32 ty_max, struct geometry_config *out_config)
{
    if (out_config)
    {
        bzero_memory(out_config, sizeof(geometry_config));
        ...
        bcopy_memory(out_config->vertices, uiverts, out_config->vertex_size * out_config->vertex_count);

        // Indices - counter-clockwise
        u32 uiindices[6] = {2, 1, 0, 3, 0, 1};
        bcopy_memory(out_config->indices, uiindices, out_config->index_size * out_config->index_count);
    }
}
