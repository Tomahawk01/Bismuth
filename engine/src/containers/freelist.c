...
void freelist_create(u64 total_size, u64* memory_requirement, void* memory, freelist* out_list)
{
    ...
    // Catch an edge case of having really small amount of memory to manage, and only having a
    ...
    *memory_requirement = sizeof(internal_state) + (sizeof(freelist_node) * max_entries);
    ...
    bzero_memory(out_list->memory, *memory_requirement);
...
void freelist_destroy(freelist* list)
{
    if (list && list->memory)
    {
        // Zero out memory before giving it back
        internal_state* state = list->memory;
        bzero_memory(list->memory, sizeof(internal_state) + sizeof(freelist_node) * state->max_entries);
        list->memory = 0;
    }
}
...
b8 freelist_free_block(freelist* list, u64 size, u64 offset)
{
    ...
    else
    {
        while (node)
        {
            ...
            else if (node->offset > offset)
            {
                ...
                // If there is a previous node, new node should be inserted between this and it
                if (previous)
                {
                ...
                {
                    // Otherwise, new node becomes the head
                    new_node->next = node;
                    state->head = new_node;
                }
    ...
    BWARN("Unable to find block to be freed. Corruption possible?");
    return false;
}
...
b8 freelist_resize(freelist* list, u64* memory_requirement, void* new_memory, u64 new_size, void** out_old_memory)
{
    ...
    *memory_requirement = sizeof(internal_state) + (sizeof(freelist_node) * max_entries);
    ...
    // Assign old memory pointer so it can be freed
    *out_old_memory = list->memory;
    ...
    // Block's layout is head* first, then array of available nodes
    bzero_memory(list->memory, *memory_requirement);
    ...
    if (!old_node)
    {
        // If there is no head, then entire list is allocated. In this case,
        // head should be set to the difference of space now available, and
    ...
    else
    {
        ...
        while (old_node)
        {
            ...
            else
            {
                ...
                // Check if it extends to the end of the block
