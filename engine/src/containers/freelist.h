 ...
 */
BAPI void freelist_create(u64 total_size, u64* memory_requirement, void* memory, freelist* out_list);

/**
 * @brief Destroys provided list.
 * 
 ...
 */
BAPI b8 freelist_allocate_block(freelist* list, u64 size, u64* out_offset);
 ...
 */
BAPI b8 freelist_free_block(freelist* list, u64 size, u64 offset);

/**
 * @brief Attempts to resize provided freelist to the given size. Internal data is copied to the new
 ...
 * @param new_memory New block of state memory. Set to 0 if only querying memory_requirement.
 * @param new_size New size. Must be greater than the size of the provided list.
 * @param out_old_memory A pointer to hold the old block of memory so that it may be freed after this call.
 * @return True on success; otherwise false.
 */
BAPI b8 freelist_resize(freelist* list, u64* memory_requirement, void* new_memory, u64 new_size, void** out_old_memory);
