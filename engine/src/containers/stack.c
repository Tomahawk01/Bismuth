...
b8 stack_create(stack* out_stack, u32 element_size)
{
    if (!out_stack)
    {
        BERROR("stack_create requires a pointer to a valid stack");
        return false;
    }

    bzero_memory(out_stack, sizeof(stack));
...
b8 stack_push(stack* s, void* element_data)
{
    if (!s)
    {
        BERROR("stack_push requires a pointer to a valid stack");
        return false;
    }
...
b8 stack_pop(stack* s, void* out_element_data)
{
    ...
    {
        BERROR("stack_pop requires a pointer to a valid stack and to hold element data output");
        return false;
    }

    if (s->element_count < 1)
    {
        BWARN("Can not pop from an empty stack");
        return false;
    }
