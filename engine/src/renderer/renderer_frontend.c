...
b8 renderer_system_initialize(u64* memory_requirement, void* state, void* config)
{
    ...
    *memory_requirement = sizeof(renderer_system_state);
    ...
    // Create vsync bvar
    bvar_int_create("vsync", (renderer_config.flags & RENDERER_CONFIG_FLAG_VSYNC_ENABLED_BIT) ? 1 : 0);
...
void renderer_begin_debug_label(const char* label_text, vec3 color)
{
    ...
    renderer_system_state* state_ptr = (renderer_system_state*)systems_manager_get_state(B_SYSTEM_TYPE_RENDERER);
    if (state_ptr)
        state_ptr->plugin.begin_debug_label(&state_ptr->plugin, label_text, color);
#endif
}
...
b8 renderer_frame_prepare(struct frame_data* p_frame_data)
{
    ...
    // Update frame data with renderer info
...
b8 renderer_present(struct frame_data* p_frame_data)
{
    ...
    if (!result)
        BERROR("renderer_present failed. Application shutting down...");
...
void renderer_render_target_destroy(render_target* target, b8 free_internal_memory)
{
    ...
    if (free_internal_memory)
        bzero_memory(target, sizeof(render_target));
}
...
b8 renderer_renderpass_create(const renderpass_config* config, renderpass* out_renderpass)
{
    ...
    if (!config)
    {
        BERROR("Renderpass config is required");
        return false;
    }

    if (config->render_target_count == 0)
    {
        BERROR("Cannot have a renderpass target count of 0");
        return false;
    }

    out_renderpass->render_target_count = config->render_target_count;
    out_renderpass->targets = ballocate(sizeof(render_target) * out_renderpass->render_target_count, MEMORY_TAG_ARRAY);
