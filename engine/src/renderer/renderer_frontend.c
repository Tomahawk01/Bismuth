...
b8 renderer_renderbuffer_allocate(renderbuffer* buffer, u64 size, u64* out_offset)
{
    ...
    {
        BERROR("vulkan_buffer_allocate requires valid buffer, a nonzero size and valid pointer to hold offset");
        return false;
    }

    if (buffer->freelist_memory_requirement == 0)
    {
        BWARN("vulkan_buffer_allocate called on a buffer not using freelists. Offset will not be valid. Call renderer_renderbuffer_load_range instead");
        *out_offset = 0;
        return true;
    }
...
b8 renderer_renderbuffer_free(renderbuffer* buffer, u64 size, u64 offset)
{
    if (!buffer || !size)
    {
        BERROR("vulkan_buffer_free requires valid buffer and a nonzero size");
        return false;
    }

    if (buffer->freelist_memory_requirement == 0)
    {
        BWARN("vulkan_buffer_allocate called on a buffer not using freelists. Nothing was done");
        return true;
    }
