...
{
    // Take copy of default texture pointer
    ...
    for (u32 i = 0; i < OBJECT_SHADER_STAGE_COUNT; ++i)
    {
        ...
        {
            BERROR("Unable to create %s shader module for '%s'", stage_type_strs[i], BUILTIN_SHADER_NAME_OBJECT);
            return false;
        }
    }
    ...
    bzero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
...
void vulkan_object_shader_update_object(vulkan_context* context, struct vulkan_object_shader* shader, geometry_render_data data)
{
    ...
    bzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_OBJECT_SHADER_DESCRIPTOR_COUNT);
    ...
    for (u32 sampler_index = 0; sampler_index < sampler_count; ++sampler_index)
    {
        ...
        if (t->generation == INVALID_ID)
        {
            ...
            // Reset descriptor generation if using the default texture
