...
b8 vulkan_ui_shader_create(vulkan_context* context, vulkan_ui_shader* out_shader)
{
    ...
    for (u32 i = 0; i < UI_SHADER_STAGE_COUNT; ++i)
    {
        ...
        {
            BERROR("Unable to create %s shader module for '%s'", stage_type_strs[i], BUILTIN_SHADER_NAME_UI);
            return false;
        }
    }
    ...
    bzero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
...
#define ATTRIBUTE_COUNT 2
    ...
    {
        BERROR("UI instance buffer creation failed for shader");
        return false;
    }
...
void vulkan_ui_shader_apply_material(vulkan_context* context, struct vulkan_ui_shader* shader, material* material)
{
    if (context && shader)
    {
        ...
        bzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
...
void vulkan_ui_shader_release_resources(vulkan_context* context, struct vulkan_ui_shader* shader, material* material)
{
    ...
    // TODO: add the object_id to free list
}
