...
b8 vulkan_material_shader_create(vulkan_context* context, vulkan_material_shader* out_shader)
{
    ...
    for (u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; ++i)
    {
        ...
        {
            BERROR("Unable to create %s shader module for '%s'", stage_type_strs[i], BUILTIN_SHADER_NAME_MATERIAL);
            return false;
        }
    }
    ...
    bzero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) * VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT);
...
#define ATTRIBUTE_COUNT 2
    ...
    bzero_memory(stage_create_infos, sizeof(stage_create_infos));
    for (u32 i = 0; i < MATERIAL_SHADER_STAGE_COUNT; ++i)
    {
    ...
    {
        BERROR("Material instance buffer creation failed for shader");
        return false;
    }
...
void vulkan_material_shader_update_object(vulkan_context* context, struct vulkan_material_shader* shader, geometry_render_data data)
{
    ...
    bzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * VULKAN_MATERIAL_SHADER_DESCRIPTOR_COUNT);
    ...
    // f32 s = (bsin(accumulator) + 1.0f) / 2.0f; // scale from -1, 1 to 0, 1
    ...
    for (u32 sampler_index = 0; sampler_index < sampler_count; ++sampler_index)
    {
        ...
        switch (use)
        {
            case TEXTURE_USE_MAP_DIFFUSE:
                ...
                break;
            default:
                BFATAL("Unable to bind sampler to unknown use");
                return;
        }
        ...
        // If texture hasn't been loaded yet, use default
        if (t->generation == INVALID_ID)
        {
            t = texture_system_get_default_texture();

            // Reset descriptor generation if using default texture
    ...
    // Bind the descriptor set to be updated, or in case shader changed
...
b8 vulkan_material_shader_acquire_resources(vulkan_context* context, struct vulkan_material_shader* shader, material* material)
{
    ...
    {
        BERROR("Error allocating descriptor sets in shader");
        return false;
    }
