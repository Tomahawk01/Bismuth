...
// Debugger
    ...
    *out_window_render_target_count = context.swapchain.image_count;
    ...
    // Renderpass table will be a lookup of array indices. Start every index with an invalid id
    context.renderpass_table_block = ballocate(sizeof(u32) * VULKAN_MAX_REGISTERED_RENDERPASSES, MEMORY_TAG_RENDERER);
    ...
    {
        // TODO: move to function for reusability
        // Make sure there are no collisions with name first
        u32 id = INVALID_ID;
        hashtable_get(&context.renderpass_table, config->pass_configs[i].name, &id);
        if (id != INVALID_ID)
        {
            BERROR("Collision with renderpass named '%s'. Initialization failed", config->pass_configs[i].name);
            return false;
        }
        // Snip up a new id
        ...
        if (id == INVALID_ID)
        {
            BERROR("No space was found for a new renderpass. Increase VULKAN_MAX_REGISTERED_RENDERPASSES. Initialization failed");
            return false;
        }
...
renderpass* vulkan_renderer_renderpass_get(const char* name)
{
    if (!name || name[0] == 0)
    {
        BERROR("vulkan_renderer_renderpass_get requires a name. Nothing will be returned");
        return 0;
    }
    ...
    if (id == INVALID_ID)
    {
        BWARN("There is no registered renderpass named '%s'", name);
        return 0;
    }
...
b8 recreate_swapchain(renderer_backend* backend)
{
    ...
    // Tell renderer that refresh is required
...
// Index of the UBO binding
const u32 BINDING_INDEX_UBO = 0;
// Index of the image sampler binding
...
{
    shader->internal_data = ballocate(sizeof(vulkan_shader), MEMORY_TAG_RENDERER);
    ...
    bzero_memory(out_shader->config.stages, sizeof(vulkan_shader_stage_config) * VULKAN_SHADER_MAX_STAGES);
    ...
    bzero_memory(out_shader->config.descriptor_sets, sizeof(vulkan_descriptor_set_config) * 2);

    // Attributes array
    bzero_memory(out_shader->config.attributes, sizeof(VkVertexInputAttributeDescription) * VULKAN_SHADER_MAX_ATTRIBUTES);
    ...
    {
        // If using instances, add second descriptor set
        vulkan_descriptor_set_config instance_descriptor_set_config = {};

        // Add UBO to it, as instances should always have one available
...
b8 vulkan_renderer_shader_initialize(shader* shader)
{
    ...
    bzero_memory(s->stages, sizeof(vulkan_shader_stage) * VULKAN_SHADER_MAX_STAGES);
    for (u32 i = 0; i < s->config.stage_count; ++i)
    {
        if (!create_module(s, s->config.stages[i], &s->stages[i]))
        {
            BERROR("Unable to create %s shader module for '%s'. Shader will be destroyed", s->config.stages[i].file_name, shader->name);
            return false;
        }
    }
    ...
    {
        // For samplers, descriptor bindings need to be updated.
        if (shader->uniforms[i].type == SHADER_UNIFORM_TYPE_SAMPLER)
        {
            ...
            if (set_config->binding_count < 2)
            {
                ...
                set_config->bindings[BINDING_INDEX_SAMPLER].descriptorCount = 1;              // Default to 1, will increase with each sampler added to appropriate level
    ...
    bzero_memory(s->descriptor_set_layouts, s->config.descriptor_set_count);
...
b8 vulkan_renderer_shader_apply_instance(shader* s, b8 needs_update)
{
    ...
    if (needs_update)
    {
        ...
        // Samplers will always be in the binding. If binding count is less than 2, there are no samplers
...
void vulkan_renderpass_create(renderpass* out_renderpass, f32 depth, u32 stencil, b8 has_prev_pass, b8 has_next_pass)
{
    ...
    {
        bzero_memory(&attachment_descriptions[attachment_description_count], sizeof(VkAttachmentDescription));
...
void vulkan_renderer_render_target_create(u8 attachment_count, texture** attachments, renderpass* pass, u32 width, u32 height, render_target* out_target)
{
    ...
    // Take copy of attachments and count
    out_target->attachment_count = attachment_count;
    if (!out_target->attachments)
        out_target->attachments = ballocate(sizeof(texture*) * attachment_count, MEMORY_TAG_ARRAY);
    bcopy_memory(out_target->attachments, attachments, sizeof(texture*) * attachment_count);
...
void vulkan_renderer_render_target_destroy(render_target* target, b8 free_internal_memory)
{
    if (target && target->internal_framebuffer)
    {
        ...
        {
            bfree(target->attachments, sizeof(texture*) * target->attachment_count, MEMORY_TAG_ARRAY);
...
texture* vulkan_renderer_window_attachment_get(u8 index)
{
    ...
    {
        BFATAL("Attempting to get attachment index out of range: %d. Attachment count: %d", index, context.swapchain.image_count);
        return 0;
    }
