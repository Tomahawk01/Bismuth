...
b8 track_initialize(track* trk)
 {
    ...
    // Generate geometry
    trk->geometry.type = BGEOMETRY_TYPE_3D_STATIC;
    trk->geometry.name = bname_create("track_base_collision");
    ...
    t->geometry.vertices = ballocate(sizeof(vertex_3d) * t->geometry.vertex_count, MEMORY_TAG_ARRAY);
    t->geometry.index_count = 6;
    t->geometry.indices = ballocate(sizeof(u32) * t->geometry.index_count, MEMORY_TAG_ARRAY);
    ...
    */
    ...
    trk->geometry.vertices = ballocate(sizeof(vertex_3d) * trk->geometry.vertex_count, MEMORY_TAG_ARRAY);
    trk->geometry.index_count = point_count * 12 * trk->segment_resolution; // 6 per face, 2 faces per segment Tessellation
    trk->geometry.indices = ballocate(sizeof(u32) * trk->geometry.index_count, MEMORY_TAG_ARRAY);
    ...
    for (u32 i = 0; i < point_count - 1; ++i)
    {
        ...
        for (u32 t = 0; t <= trk->segment_resolution; ++t, vi += 3)
        {
            ...
            f32 left_width = blerp(start->left_width, end->left_width, pct);
            f32 right_width = blerp(start->right_width, end->right_width, pct);
            f32 rotation_y = blerp(start->rotation_y, end->rotation_y, pct);
            ...
            // Generate index data per segment. This looks forward to the next tessellated face. So skip the last iteration
...
void track_destroy(track* t)
{
    if (t)
    {
        if (t->geometry.vertices && t->geometry.vertex_count)
            bfree(t->geometry.vertices, sizeof(vertex_3d) * t->geometry.vertex_count, MEMORY_TAG_ARRAY);
        if (t->geometry.indices && t->geometry.index_count)
            bfree(t->geometry.indices, sizeof(u32) * t->geometry.index_count, MEMORY_TAG_ARRAY);
...
static f32 compute_interpolated_width(vec3 point, vec3 segment_start, vec3 segment_end, f32 start_width, f32 end_width)
{
    ...
    return blerp(start_width, end_width, t);
}
...
static vec3 calculate_normal(vec3 segment_vector, f32 rotation_y)
{
    ...
    f32 cos_theta = bcos(rotation_y);
    f32 sin_theta = bsin(rotation_y);
...
vec3 track_constrain_object(vec3 object_position, track* t)
{
    ...
    for (u32 i = 0; i < point_count - 1; ++i)
    {
        ...
        f32 height_at_closest = blerp(height_left_at_closest, height_right_at_closest, t);
