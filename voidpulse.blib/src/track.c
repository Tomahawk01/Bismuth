...
b8 track_initialize(track* trk)
 {
    ...
    for (u32 i = 0; i < point_count - 1; ++i)
    {
        ...
        segment->geometry.vertices = ballocate(sizeof(vertex_3d) * segment->geometry.vertex_count, MEMORY_TAG_ARRAY);
        segment->geometry.index_count = 12 * trk->segment_resolution; // 6 per face, 2 faces per segment Tessellation
        segment->geometry.indices = ballocate(sizeof(u32) * segment->geometry.index_count, MEMORY_TAG_ARRAY);
