...
{
    vec3 direction = {bcos(rotation_y), 0.0f, bsin(rotation_y)};
...
b8 track_initialize(track* trk)
 {
    ...
    for (u32 i = 0; i < segment_count; ++i)
    {
        ...
        segment->geometry.name = bname_create("track_segment");
        ...
        segment->geometry.vertices = BALLOC_TYPE_CARRAY(vertex_3d, segment->geometry.vertex_count);
        segment->geometry.index_count = 24 * trk->segment_resolution; // 6 per face, 4 faces per segment Tessellation
        segment->geometry.indices = BALLOC_TYPE_CARRAY(u32, segment->geometry.index_count);

        // Triangle and adjacency data
        segment->triangle_count = 4 * trk->segment_resolution;
        segment->triangles = BALLOC_TYPE_CARRAY(triangle_with_adjacency, segment->triangle_count);
        ...
        vec3 start_direction = {bcos(segment->start->rotation_y), 0.0f, bsin(segment->start->rotation_y)};
        vec3 start_normal = vec3_cross(start_direction, vec3_up());
        vec3 end_direction = {bcos(segment->end->rotation_y), 0.0f, bsin(segment->end->rotation_y)};
        ...
        for (u32 t = 0; t <= trk->segment_resolution; ++t, vi += 5)
        {
            ...
            // Generate index data per segment. This looks forward to the next tessellated face. So skip the last iteration. Also generate triangle adjacency data
...
b8 track_load(track* t, bphysics_world* physics_world)
{
    if (!t)
        return false;

    // Get a material for the track. This is only needed when visualizing the track
    // HACK: hardcoded material for now. Should be able to vary by segment
    if (!material_system_acquire(engine_systems_get()->material_system, bname_create("testcube_mat"), &t->material))
    {
        BERROR("Failed to load material!");
    }

    t->physics_world = physics_world;
    ...
    for (u32 i = 0; i < segment_count; ++i)
    {
        ...
        {
            BERROR("Failed to upload geometry for track segment. Track load failed");
            return false;
        }
        ...
        if (!segment->geometry.tris)
        {
            ...
            {
                BERROR("Triangle generation failed for track segment geometry. See logs for details");
                return false;
            }
        }
        ...
        if (!bphysics_world_add_body(engine_systems_get()->physics_system, t->physics_world, segment->physics_body))
        {
            BERROR("Failed to add track segment to physics world. See logs for details");
            return false;
        }
    }
...
void track_unload(track* t)
{
    if (t)
    {
        ...
        for (u32 i = 0; i < segment_count; ++i)
        {
            ...
            bphysics_world_remove_body(engine_systems_get()->physics_system, t->physics_world, segment->physics_body);
            // Destroy physics body
            bphysics_body_destroy(engine_systems_get()->physics_system, &segment->physics_body);
    ...
    if (t)
    {
        ...
        for (u32 i = 0; i < segment_count; ++i)
        {
            ...
            if (segment->geometry.vertices && segment->geometry.vertex_count)
                bfree(segment->geometry.vertices, sizeof(vertex_3d) * segment->geometry.vertex_count, MEMORY_TAG_ARRAY);
            if (segment->geometry.indices && segment->geometry.index_count)
                bfree(segment->geometry.indices, sizeof(u32) * segment->geometry.index_count, MEMORY_TAG_ARRAY);
        }

        darray_destroy(t->segments);

        bzero_memory(t, sizeof(track));
    }
}
...
b8 is_point_inside_triangle(vec3 point, const triangle_3d* tri)
{
    ...
    if (babs(determinant) < 1e-6)
    {
        BTRACE("degenerate tri");
...
triangle_3d find_closest_triangle(vec3 point, const bgeometry* geometry)
{
    vertex_3d* verts = geometry->vertices;
    u32* indices = geometry->indices;
    // TODO: use a BVH or something here
    f32 closest_distance = B_FLOAT_MAX;
...
triangle_with_adjacency* find_closest_triangle_with_adjacency(vec3 point, u32 count, triangle_with_adjacency* tris)
{
    ...
    // BTRACE("closest tri is idx %u (point=%.2f,%2f,%.2f)", closest_triangle.index, point.x, point.y, point.z);
...
i32 constrain_to_track_segment(vec3 point, vec3 velocity, track* trk, track_segment* segment, vec3* out_position, vec3* out_surface_normal)
{
    ...
    *out_surface_normal = triangle_3d_get_normal(&closest_triangle->tri);
    ...
    if (is_point_inside_triangle(projected_position, &closest_triangle->tri))
    {
        ...
        BTRACE("point inside, applying velocity (tri idx=%u)", closest_triangle->index);
    ...
    else
    {
        ...
        f32 closest_edge_dist = B_FLOAT_MAX;
        ...
        // If the closest edge shares two vertices with the end (either on the left or the right), then need to transition to the next segment.
        // 2 is done so that transition only happens when on the triangle with a full side is done

        u32 shared_start_points = 0;
        shared_start_points += vec3_compare(closest_edge_start, segment->start->left, B_FLOAT_EPSILON);
        shared_start_points += vec3_compare(closest_edge_start, segment->start->position, B_FLOAT_EPSILON);
        shared_start_points += vec3_compare(closest_edge_start, segment->start->right, B_FLOAT_EPSILON);
        shared_start_points += vec3_compare(closest_edge_end, segment->start->left, B_FLOAT_EPSILON);
        shared_start_points += vec3_compare(closest_edge_end, segment->start->position, B_FLOAT_EPSILON);
        shared_start_points += vec3_compare(closest_edge_end, segment->start->right, B_FLOAT_EPSILON);

        u32 shared_end_points = 0;
        shared_end_points += vec3_compare(closest_edge_start, segment->end->left, B_FLOAT_EPSILON);
        shared_end_points += vec3_compare(closest_edge_start, segment->end->position, B_FLOAT_EPSILON);
        shared_end_points += vec3_compare(closest_edge_start, segment->end->right, B_FLOAT_EPSILON);
        shared_end_points += vec3_compare(closest_edge_end, segment->end->left, B_FLOAT_EPSILON);
        shared_end_points += vec3_compare(closest_edge_end, segment->end->position, B_FLOAT_EPSILON);
        shared_end_points += vec3_compare(closest_edge_end, segment->end->right, B_FLOAT_EPSILON);
...
vec3 constrain_to_track(vec3 vehicle_point, vec3 velocity, track* t, vec3* out_surface_normal)
{
    ...
    while (segment)
    {
        ...
        {
            BTRACE("Not contained in segment %u. Segment change needed", segment_index);
        ...
        {
            BWARN("No segments found that contain the point. Giving up");
            break;
        }
    }
