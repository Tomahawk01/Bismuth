...
i32 combine_texture_maps(i32 argc, char** argv)
{
    ...
    for (u32 i = 2; i < argc; ++i)
    {
        ...
        {
            BERROR("Unrecognized map type '%s'", parts[0]);
            return -5;
        }
    }
    ...
    {
        BERROR("parameter outfile is required. Usage: outfile=[filename]");
        return -4;
    }
    ...
    for (u32 i = 0; i < MAP_TYPE_MAX; ++i)
    {
        ...
        if (!maps[i].data)
        {
            BFATAL("Failed to load file '%s'", maps[i].file_path);
            return -6;
        }
    }
    ...
    for (u32 i = 0; i < MAP_TYPE_MAX; ++i)
    {
        if (maps[i].file_path)
        {
            ...
            else
            {
                ...
                {
                    BERROR("All texture maps must be the same width and height");
                    return -7;
                }
            }
        }
    ...
    if (width == -1 || height == -1)
    {
        BERROR("Unable to obtain width and height - no textures set?");
        return -8;
    }
    for (u32 i = 0; i < MAP_TYPE_MAX; ++i)
    {
    ...
    {
        BERROR("Error writing outfile");
        return -9;
    }

    BINFO("Successfully processed all maps");
    return 0;
}
