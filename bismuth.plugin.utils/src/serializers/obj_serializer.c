...
b8 obj_serializer_deserialize(const char* obj_file_text, obj_source_asset* out_source_asset)
{
    ...
    {
        BERROR("obj_serializer_deserialize requires valid pointers to obj_file_text and out_source_asset");
        return false;
    }
    ...
    out_source_asset->geometries = ballocate(sizeof(obj_source_geometry) * out_source_asset->geometry_count, MEMORY_TAG_ARRAY);
    bcopy_memory(out_source_asset->geometries, geometries_darray, sizeof(obj_source_geometry) * out_source_asset->geometry_count);
...
static void process_subobject(vec3* positions, vec3* normals, vec2* tex_coords, mesh_face_data* faces, obj_source_geometry* out_data)
{
    ...
    b8 extent_set = false;
    bzero_memory(&out_data->extents.min, sizeof(vec3));
    bzero_memory(&out_data->extents.max, sizeof(vec3));
    ...
    for (u64 f = 0; f < face_count; ++f)
    {
        ...
        for (u64 i = 0; i < 3; ++i)
        {
            ...
            // TODO: Color. Hardcoded to white for now
            vert.color = vec4_one();
    ...
    for (u8 i = 0; i < 3; ++i)
        out_data->center.elements[i] = (out_data->extents.min.elements[i] + out_data->extents.max.elements[i]) / 2.0f;
}
