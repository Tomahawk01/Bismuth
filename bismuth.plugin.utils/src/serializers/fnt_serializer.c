...
{
    BDEBUG("Importing source bitmap .fnt file ...");
    ...
    while (true)
    {
        ...
        case 'c': {
            ...
            if (line_buf[1] == 'o')
            {
                ...
                {
                    if (!out_asset->pages)
                        out_asset->pages = ballocate(sizeof(bitmap_font_page) * out_asset->page_count, MEMORY_TAG_ARRAY);
                }
                else
                {
            ...
            else if (line_buf[1] == 'h')
            {
                if (line_buf[4] == 's')
                {
                    ...
                    {
                        if (!out_asset->glyphs)
                            out_asset->glyphs = ballocate(sizeof(font_glyph) * out_asset->glyph_count, MEMORY_TAG_ARRAY);
                    }
                    else
                    {
        ...
        case 'k': {
            ...
            if (line_buf[7] == 's')
            {
                ...
                if (!out_asset->kernings)
                    out_asset->kernings = ballocate(sizeof(font_kerning) * out_asset->kerning_count, MEMORY_TAG_ARRAY);
            }
            else if (line_buf[7] == ' ')
            {
                // Kerning record
