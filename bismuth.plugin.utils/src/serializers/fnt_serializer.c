...
b8 fnt_serializer_deserialize(const char* fnt_file_text, fnt_source_asset* out_fnt_source_asset)
{
    ...
    {
        BERROR("fnt_serializer_deserialize requires valid pointers to fnt_file_text and out_fnt_source_asset");
        return false;
    }
...
{
    BDEBUG("Importing source bitmap font .fnt file ...");

    bzero_memory(out_asset, sizeof(fnt_source_asset));
    char* line = 0;
    ...
    while (true)
    {
        ...
        case 'i': {
            ...
            bzero_memory(face_name_buf, sizeof(char) * 512);
        ...
        case 'c': {
            ...
            if (line[1] == 'o')
            {
                ...
                {
                    if (!out_asset->pages)
                        out_asset->pages = BALLOC_TYPE_CARRAY(basset_bitmap_font_page, out_asset->page_count);
                }
                else
                {
            ...
            else if (line[1] == 'h')
            {
                if (line[4] == 's')
                {
                    ...
                    {
                        if (!out_asset->glyphs)
                            out_asset->glyphs = BALLOC_TYPE_CARRAY(basset_bitmap_font_glyph, out_asset->glyph_count);
                    }
                    else
                    {
                ...
                {
                    // Assume 'char' line
                    basset_bitmap_font_glyph* g = &out_asset->glyphs[glyphs_read];
        ...
        case 'p': {
            // 'page' line
            basset_bitmap_font_page* page = &out_asset->pages[pages_read];
            ...
            page->image_asset_name = bname_create(page_file_str);
        ...
        case 'k': {
            ...
            if (line[7] == 's')
            {
                ...
                if (!out_asset->kernings)
                    out_asset->kernings = BALLOC_TYPE_CARRAY(basset_bitmap_font_kerning, out_asset->kerning_count);
            }
            else if (line[7] == ' ')
            {
                // Kerning record
                basset_bitmap_font_kerning* k = &out_asset->kernings[kernings_read];
                i32 elements_read = sscanf(
                    line,
