...
b8 basset_importer_bitmap_font_fnt(const struct basset_importer* self, u64 data_size, const void* data, void* params, struct basset* out_asset)
{
    ...
    // Handle FNT file import
    {
        ...
        // Convert FNT asset to basset_bitmap_font
        ...
        BCOPY_TYPE_CARRAY(typed_asset->pages.data, fnt_asset.pages, basset_bitmap_font_page, fnt_asset.page_count);

        typed_asset->glyphs = array_basset_bitmap_font_glyph_create(fnt_asset.glyph_count);
        BCOPY_TYPE_CARRAY(typed_asset->glyphs.data, fnt_asset.glyphs, basset_bitmap_font_glyph, fnt_asset.glyph_count);

        if (fnt_asset.kerning_count)
        {
            typed_asset->kernings = array_basset_bitmap_font_kerning_create(fnt_asset.kerning_count);
            BCOPY_TYPE_CARRAY(typed_asset->kernings.data, fnt_asset.kernings, basset_bitmap_font_kerning, fnt_asset.kerning_count);
        }

        // Cleanup fnt asset
        BFREE_TYPE_CARRAY(fnt_asset.pages, basset_bitmap_font_page, fnt_asset.page_count);
        BFREE_TYPE_CARRAY(fnt_asset.glyphs, basset_bitmap_font_glyph, fnt_asset.glyph_count);
        BFREE_TYPE_CARRAY(fnt_asset.kernings, basset_bitmap_font_kerning, fnt_asset.kerning_count);
        string_free(fnt_asset.face_name);
    }

    // Serialize data and write out bbf file (binary Bismuth Bitmap Font)
    {
        u64 serialized_size = 0;
        void* serialized_data = basset_bitmap_font_serialize(out_asset, &serialized_size);
        ...
        // Write out .bbf file
        if (!vfs_asset_write(engine_systems_get()->vfs_system_state, out_asset, true, serialized_size, serialized_data))
        {
            BWARN("Failed to write .bbf (Bismuth Bitmap Font) file. See logs for details");
        }
    }
