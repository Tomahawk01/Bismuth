...
b8 render_view_editor_world_on_registered(struct render_view* self)
{
    if (self)
    {
        self->internal_data = ballocate(sizeof(render_view_editor_world_internal_data), MEMORY_TAG_RENDERER);
        ...
        // Get color3d shader uniform locations
        {
            ...
            data->s = shader_system_get(color3d_shader_name);
            if (!data->s)
            {
    ...
    BERROR("render_view_editor_world_on_create - Requires a valid pointer to a view");
    return false;
}
...
void render_view_editor_world_on_destroy(struct render_view* self)
{
    if (self && self->internal_data)
    {
        ...
        bfree(self->internal_data, sizeof(render_view_editor_world_internal_data), MEMORY_TAG_RENDERER);
...
b8 render_view_editor_world_on_packet_build(const struct render_view* self, struct frame_data* p_frame_data, struct viewport* v, struct camera* c, void* data, struct render_view_packet* out_packet)
{
    ...
    {
        BWARN("render_view_editor_world_on_build_packet requires valid pointer to view, packet and data");
        return false;
    }
    ...
    if (packet_data->gizmo)
    {
        ...
        // TODO: Should get this from camera/viewport
        ...
        // f32 fixed_size = 0.1f;  // TODO: Make this configurable option for gizmo size
        f32 scale_scalar = 1.0f; // ((2.0f * btan(fov * 0.5f)) * dist) * fixed_size;
        packet_data->gizmo->scale_scalar = scale_scalar;  // Keep copy of this for hit detection
...
b8 render_view_editor_world_on_render(const struct render_view* self, const struct render_view_packet* packet, struct frame_data* p_frame_data)
{
    ...
    for (u32 p = 0; p < self->renderpass_count; ++p)
    {
        ...
        {
            BERROR("render_view_editor_world_on_render pass index %u failed to start", p);
            return false;
        }
        ...
        {
            BERROR("render_view_editor_world_on_render pass index %u failed to end", p);
            return false;
        }
    }
