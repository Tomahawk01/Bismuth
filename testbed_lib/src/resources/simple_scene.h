...
/**
 * @brief Creates a new scene with the given config with default values. No resources are allocated. Config is not yet processed.
 * 
...
/**
 * @brief Performs initialization routines on the scene, including processing configuration (if provided) and scaffolding heirarchy.
 * 
 ...
 * @param viewport Pointer to the viewport to be used when populating the render packets.
 * @param p_frame_data A constant pointer to the current frame's data.
 * @param packet A pointer to the packet to populate.
 * @return True on success; otherwise false.
 */
BAPI b8 simple_scene_populate_render_packet(simple_scene* scene, struct camera* current_camera, struct viewport* v, struct frame_data* p_frame_data, struct render_packet* packet);
