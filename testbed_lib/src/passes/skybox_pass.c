...
b8 skybox_pass_create(struct rendergraph_pass* self)
{
    ...
    self->internal_data = ballocate(sizeof(skybox_pass_internal_data), MEMORY_TAG_RENDERER);
    self->pass_data.ext_data = ballocate(sizeof(skybox_pass_extended_data), MEMORY_TAG_RENDERER);
...
b8 skybox_pass_initialize(struct rendergraph_pass* self)
{
    ...
    skybox_pass_config.clear_color = (vec4){0.0f, 0.0f, 0.2f, 1.0f};
    skybox_pass_config.clear_flags = RENDERPASS_CLEAR_COLOR_BUFFER_FLAG;
    skybox_pass_config.depth = 1.0f;
    skybox_pass_config.stencil = 0;
    skybox_pass_config.target.attachment_count = 1;
    skybox_pass_config.target.attachments = ballocate(sizeof(render_target_attachment_config) * skybox_pass_config.target.attachment_count, MEMORY_TAG_ARRAY);
    skybox_pass_config.render_target_count = renderer_window_attachment_count_get();

    // Color attachment
    render_target_attachment_config* skybox_target_color = &skybox_pass_config.target.attachments[0];
    ...
    {
        BERROR("Skybox rendergraph pass - Failed to create skybox renderpass");
        return false;
    }
    ...
    {
        BERROR("Failed to load skybox shader resource");
        return false;
    }
    ...
    {
        BERROR("Failed to create skybox shader");
        return false;
    }
...
b8 skybox_pass_execute(struct rendergraph_pass* self, struct frame_data* p_frame_data)
{
    ...
    {
        BERROR("skybox pass failed to start");
        return false;
    }
    ...
    if (ext_data->sb)
    {
        ...
        // Get view matrix, but zero out position so the skybox stays put on screen
    ...
    {
        BERROR("skybox pass failed to end");
        return false;
    }
...
void skybox_pass_destroy(struct rendergraph_pass* self)
{
    if (self)
    {
        if (self->internal_data)
        {
            ...
            bfree(self->internal_data, sizeof(skybox_pass_internal_data), MEMORY_TAG_RENDERER);
