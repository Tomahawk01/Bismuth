...
b8 ui_pass_create(struct rendergraph_pass* self)
{
    ...
    self->pass_data.ext_data = ballocate(sizeof(ui_pass_extended_data), MEMORY_TAG_RENDERER);
...
b8 ui_pass_initialize(struct rendergraph_pass* self)
{
    ...
    ui_pass_config.clear_color = (vec4){0.0f, 0.0f, 0.2f, 1.0f};
    ...
    ui_pass_config.target.attachments = ballocate(sizeof(render_target_attachment_config) * ui_pass_config.target.attachment_count, MEMORY_TAG_ARRAY);
    ...
    {
        BERROR("Failed to create UI renderpass");
        return false;
    }
    ...
    {
        BERROR("Failed to load UI shader resource");
        return false;
    }
    ...
    {
        BERROR("Failed to create UI shader");
        return false;
    }
    resource_system_unload(&config_resource);

    // Get either the custom shader override or defined default
...
b8 ui_pass_execute(struct rendergraph_pass* self, struct frame_data* p_frame_data)
{
    ...
    {
        BERROR("UI renderpass failed to start");
        return false;
    }

    if (!shader_system_use_by_id(internal_data->s->id))
    {
        BERROR("Failed to use shader. Render frame failed");
        return false;
    }
    ...
    {
        BERROR("Failed to use apply globals for shader. Render frame failed");
        return false;
    }
    ...
    for (u32 i = 0; i < ext_data->ui_text_count; ++i)
    {
        ...
        {
            BERROR("Failed to apply bitmap font diffuse map uniform");
            return false;
        }

        // TODO: font color
        static vec4 white_color = (vec4){1.0f, 1.0f, 1.0f, 1.0f};  // white
        if (!shader_system_uniform_set_by_index(internal_data->locations.properties, &white_color))
        {
    ...
    {
        BERROR("UI renderpass failed to end");
        return false;
    }
