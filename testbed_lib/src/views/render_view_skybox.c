...
b8 render_view_skybox_on_registered(struct render_view* self)
{
    if (self)
    {
        self->internal_data = ballocate(sizeof(render_view_skybox_internal_data), MEMORY_TAG_RENDERER);
    ...
    BERROR("render_view_skybox_on_create - Requires a valid pointer to a view");
    return false;
}
...
void render_view_skybox_on_destroy(struct render_view* self)
{
    if (self && self->internal_data)
    {
        ...
        // NOTE: shader is automatically destroyed by shader system on shutdown

        bfree(self->internal_data, sizeof(render_view_skybox_internal_data), MEMORY_TAG_RENDERER);
...
b8 render_view_skybox_on_packet_build(const struct render_view* self, struct linear_allocator* frame_allocator, void* data, struct render_view_packet* out_packet)
{
    ...
    // Just set extended data to the skybox data
    out_packet->extended_data = linear_allocator_allocate(frame_allocator, sizeof(skybox_packet_data));
    bcopy_memory(out_packet->extended_data, skybox_data, sizeof(skybox_packet_data));
...
b8 render_view_skybox_on_render(const struct render_view* self, const struct render_view_packet* packet, u64 frame_number, u64 render_target_index, const struct frame_data* p_frame_data)
{
    render_view_skybox_internal_data* data = self->internal_data;
    ...
    for (u32 p = 0; p < self->renderpass_count; ++p)
    {
        ...
        {
            BERROR("render_view_skybox_on_render pass index %u failed to start", p);
            return false;
        }
        ...
        if (skybox_data && skybox_data->sb)
        {
            ...
            {
                BERROR("Failed to use skybox shader. Render frame failed");
                return false;
            }

            // Get view matrix, but zero out position so skybox stays put on screen
        ...
        {
            BERROR("render_view_skybox_on_render pass index %u failed to end", p);
            return false;
        }
    }
