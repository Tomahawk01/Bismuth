...
b8 render_view_ui_on_registered(struct render_view* self)
{
    if (self)
    {
        self->internal_data = ballocate(sizeof(render_view_ui_internal_data), MEMORY_TAG_RENDERER);
        ...
        {
            BERROR("Failed to load builtin UI shader");
            return false;
        }
    ...
    BERROR("render_view_ui_on_create - Requires a valid pointer to a view");
    return false;
}
...
void render_view_ui_on_destroy(struct render_view* self)
{
    if (self && self->internal_data)
    {
        ...
        bfree(self->internal_data, sizeof(render_view_ui_internal_data), MEMORY_TAG_RENDERER);
