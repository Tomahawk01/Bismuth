...
void main()
{
    ...
    vec4 normal_color = texture(sampler2D(material_textures[MAT_WATER_IDX_NORMAL], material_samplers[MAT_WATER_IDX_NORMAL]), distorted_texcoords);
    ...
    if(render_mode == 0)
    {
        ...
        out_color = mix(out_color, tint, tint_strength);
    }
    else
    {
    ...
    // Apply lighting
    vec4 lighting = do_lighting(material_frame_ubo.views[material_draw_ubo.view_index], in_dto.frag_position.xyz, out_color.rgb, normal, render_mode);
    out_color = lighting;
...
vec3 calculate_point_light_radiance(point_light light, vec3 view_direction, vec3 frag_position_xyz)
{
    ...
    return (light.color.rgb * energy_multiplier) * attenuation;
}
...
vec3 calculate_directional_light_radiance(directional_light light, vec3 view_direction)
{
    ...
    // PBR lights are energy-based, so convert to a scale of 0-100
    float energy_multiplier = 100.0;
    return light.color.rgb * energy_multiplier;
}
...
float calculate_shadow(vec4 light_space_frag_pos, vec3 normal, directional_light light, int cascade_index)
{
    ...
    // NOTE: Transform to NDC not needed for Vulkan, but would be for OpenGL projected.xy = projected.xy * 0.5 + 0.5;
