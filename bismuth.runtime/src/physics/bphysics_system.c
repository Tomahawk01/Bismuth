#include "bphysics_system.h"

#include "containers/darray.h"
#include "containers/bpool.h"
#include "defines.h"
...
typedef struct bphysics_collision_data
{
    vec3 normal;
    f32 depth;
    bphysics_body* body_0;
    bphysics_body* body_1;
} bphysics_collision_data;

typedef struct bphysics_system_state
{
    bphysics_system_config config;

    bphysics_world* active_world;

    // A pool of all physics bodies created
    bpool all_bodies;
} bphysics_system_state;

static bphysics_body* create_body(bphysics_system_state* state, bhandle* out_handle);
static void destroy_body(bphysics_system_state* state, bhandle* handle);
static bphysics_body* get_body(bphysics_system_state* state, bhandle handle);

b8 bphysics_system_initialize(u64* memory_requirement, bphysics_system_state* state, bphysics_system_config* config)
{
    if (!memory_requirement)
        return false;

    *memory_requirement = sizeof(bphysics_system_state);
    ...
    if (!bpool_create(sizeof(bphysics_body), 512, &state->all_bodies))
    {
        BERROR("Failed to create physics body pool");
        return false;
    }

    BINFO("Physics system initialized");

    return true;
}

void bphysics_system_shutdown(bphysics_system_state* state)
{
    if (state)
    {
        bpool_destroy(&state->all_bodies);
        bzero_memory(state, sizeof(bphysics_system_state));
    }
}

static void resolve_collision(bphysics_body* body_0, bphysics_body* body_1, const bphysics_collision_data* collision)
{
    if (body_0->body_type == BPHYSICS_BODY_TYPE_STATIC)
    {
    ...
    // Resolve
    if (body_1->body_type == BPHYSICS_BODY_TYPE_DYNAMIC)
    {
...
b8 check_sphere_sphere_collision(vec3 position_0, f32 radius_0, vec3 position_1, f32 radius_1, bphysics_collision_data* out_collision)
{
...
b8 check_sphere_triangle_collision(vec3 sphere_center, f32 sphere_radius, const triangle_3d* tri, bphysics_collision_data* out_collision)
{
...
b8 check_obb_sphere_collision(const oriented_bounding_box* obb, vec3 sphere_center, f32 sphere_radius, bphysics_collision_data* out_collision)
{
    ...
    vec3 closest_point = {
        BMAX(-obb->half_extents.x, BMIN(local_sphere_center.x, obb->half_extents.x)),
        BMAX(-obb->half_extents.y, BMIN(local_sphere_center.y, obb->half_extents.y)),
        BMAX(-obb->half_extents.z, BMIN(local_sphere_center.z, obb->half_extents.z))};
...
b8 check_obb_obb_collision(const oriented_bounding_box* obb_0, const oriented_bounding_box* obb_1, bphysics_collision_data* out_collision)
{
    vec3 axes[15];
    ...
    f32 min_penetration = B_FLOAT_MAX;
    ...
    for (u32 i = 0; i < axis_count; ++i)
    {
        ...
        f32 center_proj = babs(vec3_dot(axis, center_delta));
...
static b8 check_obb_triangle_collision(const oriented_bounding_box* obb, const triangle_3d* tri, bphysics_collision_data* out_collision)
{
    vec3 obb_axes[3] = {
        vec3_rotate((vec3){1, 0, 0}, obb->rotation),
        vec3_rotate((vec3){0, 1, 0}, obb->rotation),
    ...
    for (u8 i = 0; i < 3; ++i)
    {
        ...
        f32 penetration = BMIN(max_0 - min_1, max_1 - min_0);
        if (penetration < min_penetration)
        {
            min_penetration = penetration;
...
/* static b8 check_obb_triangle_collision(const oriented_bounding_box* obb, const triangle_3d* tri, vec3* out_collision_normal, f32* out_penetration_depth)
{
    ...
    for (u8 i = 0; i < 3; ++i)
    {
        ...
        min_1 = -(
            obb->half_extents.x * babs(vec3_dot(obb_axes[i], obb_axes[0])) +
            obb->half_extents.y * babs(vec3_dot(obb_axes[i], obb_axes[1])) +
            obb->half_extents.z * babs(vec3_dot(obb_axes[i], obb_axes[2])));

        // min_1 = -obb->half_extents.x * babs(vec3_dot(obb_axes[i], vec3_right())) +
        //         -obb->half_extents.y * babs(vec3_dot(obb_axes[i], vec3_up())) +
        //         -obb->half_extents.z * babs(vec3_dot(obb_axes[i], vec3_backward()));
        max_1 = -min_1;
        if (max_0 < min_1 || max_1 < min_0)
    ...
    *out_penetration_depth = babs(max_0 - min_1);

    return true;
} */

// NOTE: The first body should always be dynamic. The second can be either dynamic or static
static b8 collide_bodies(bphysics_body* body_0, bphysics_body* body_1)
{
    if (body_0->body_type != BPHYSICS_BODY_TYPE_DYNAMIC)
    {
        BERROR("%s - body_0 should always be dynamic", __FUNCTION__);
        return false;
    }

    bphysics_collision_data collision = {0};
    ...
    case BPHYSICS_SHAPE_TYPE_SPHERE:
        ...
        case BPHYSICS_SHAPE_TYPE_MESH:
        {
            ...
            if (collision_count)
            {
                ...
                bphysics_collision_data accumulated_collision_data = {
                    .depth = max_pen_depth,
    ...
    case BPHYSICS_SHAPE_TYPE_RECTANGLE:
        ...
        case BPHYSICS_SHAPE_TYPE_MESH:
        {
            ...
            for (u32 i = 0; i < body_1->triangle_count; ++i)
            {
                ...
                {
                    BTRACE("Rectangle collided with mesh!");

                    // TODO: what if multiple triangles collide? Should an average be taken? Iterate them all?
        ...
        break;
    case BPHYSICS_SHAPE_TYPE_MESH:
        BERROR("Dynamic mesh shapes are not supported");
        return false;
    }
...
static void apply_gravity(bphysics_system_state* state, f64 fixed_update_time)
{
    ...
    {
        if (bhandle_is_valid(state->active_world->bodies[i]))
        {
            bphysics_body* body_0 = bpool_get_by_index(&state->all_bodies, state->active_world->bodies[i].handle_index);
            if (body_0)
            {
...
static b8 physics_step(bphysics_system_state* state, f64 delta_time)
{
    u32 body_count = darray_length(state->active_world->bodies);
    for (u32 i = 0; i < body_count; ++i)
    {
            ...
            if (body_0)
            {
                ...
                if (body_0->body_type == BPHYSICS_BODY_TYPE_DYNAMIC)
                {
                    ...
                    for (u32 j = 0; j < body_count; ++j)
                    {
                        if (bhandle_is_valid(state->active_world->bodies[i]))
                        {
                            bphysics_body* body_1 = bpool_get_by_index(&state->all_bodies, state->active_world->bodies[j].handle_index);
                            if (body_0 == body_1)
                            {
                                // Skip self
                                continue;
                            ...
                            if (body_1)
                            {
                                ...
                                {
                                    BERROR("Failed to handle body collision. See logs for details. Physics step failed");
                                    return false;
                                }
                            }
                        }
                    }
                    // TODO: May want to apply some sort of "air friction" if no other collision happened
                }
            }
        }
    }
...
b8 bphysics_system_fixed_update(bphysics_system_state* state, f64 fixed_update_time)
{
    ...
    if (state->active_world)
    {
        ...
        for (u32 i = 0; i < state->config.steps_per_frame; ++i)
        {
            ...
            {
                BERROR("Failed to apply physics step. See logs for details");
                return false;
            }
        }
    }
...
b8 bphysics_world_create(struct bphysics_system_state* state, bname name, vec3 gravity, bphysics_world* out_world)
{
    if (!out_world)
    {
        BERROR("bphysics_world_create requires a valid pointer to out_world");
        return false;
    }
    ...
    out_world->bodies = darray_create(bhandle);

    return true;
}

void bphysics_world_destroy(struct bphysics_system_state* state, bphysics_world* world)
{
    if (world)
    {
        if (world->bodies)
        ...
        bzero_memory(world, sizeof(bphysics_world));
    }
}
...
b8 bphysics_world_add_body(struct bphysics_system_state* state, bphysics_world* world, bhandle body)
{
    if (!world || bhandle_is_invalid(body))
    {
        BERROR("bphysics_world_add_body requires a valid pointer to world and a valid handle to body!");
        return false;
    }
    ...
    for (u32 i = 0; i < body_count; ++i)
    {
        if (bhandle_is_invalid(world->bodies[i]))
        {
            world->bodies[i] = body;
            return true;
        }
...
b8 bphysics_world_remove_body(struct bphysics_system_state* state, bphysics_world* world, bhandle body)
{
    if (!world || bhandle_is_invalid(body))
    {
        BERROR("bphysics_world_remove_body requires a valid pointer to world and a valid handle to a body!");
        return false;
    }
    ...
    for (u32 i = 0; i < body_count; ++i)
    {
        ...
        {
            bhandle_invalidate(&world->bodies[i]);
            return true;
        }
    }

    BWARN("bphysics_world_remove_body - body not found in world, nothing to be done");
    return false;
}

b8 bphysics_body_create_sphere(struct bphysics_system_state* state, bname name, vec3 position, f32 radius, bphysics_body_type body_type, bhandle* out_handle)
{
    if (!state || !out_handle)
    {
        BERROR("%s - A pointer to handle out_body is required", __FUNCTION__);
        return false;
    }

    bphysics_body* body = create_body(state, out_handle);

    body->name = name;
    body->body_type = body_type;
    body->shape_type = BPHYSICS_SHAPE_TYPE_SPHERE;
...
b8 bphysics_body_create_rectangle(struct bphysics_system_state* state, bname name, vec3 position, vec3 half_extents, bphysics_body_type body_type, bhandle* out_handle)
{
    if (!state || !out_handle)
    {
        BERROR("%s - A pointer to out_body is required", __FUNCTION__);
        return false;
    }
    ...
    body->name = name;
    body->body_type = body_type;
    body->shape_type = BPHYSICS_SHAPE_TYPE_RECTANGLE;
    body->position = position;
...
b8 bphysics_body_create_mesh(struct bphysics_system_state* state, bname name, vec3 position, u32 triangle_count, triangle_3d* tris, bphysics_body_type body_type, bhandle* out_handle)
{
    if (!state || !out_handle)
    {
        BERROR("%s - A pointer to out_body is required", __FUNCTION__);
    ...
    body->name = name;
    body->body_type = body_type;
    body->shape_type = BPHYSICS_SHAPE_TYPE_MESH;
    body->position = position;
    body->triangle_count = triangle_count;

    body->tris = BALLOC_TYPE_CARRAY(triangle_3d, triangle_count);
    BCOPY_TYPE_CARRAY(body->tris, tris, triangle_3d, triangle_count);

    return true;
}

void bphysics_body_destroy(struct bphysics_system_state* state, bhandle* body)
{
    if (!state || !body)
    {
        BWARN("%s - A pointer to body handle is required", __FUNCTION__);
        return;
    }
}

b8 bphysics_body_position_set(struct bphysics_system_state* state, bhandle body, vec3 position)
{
    if (!state)
        return false;

    bphysics_body* b = get_body(state, body);
    if (!b)
        return false;
...
b8 bphysics_body_rotation_set(struct bphysics_system_state* state, bhandle body, quat rotation)
{
    if (!state)
        return false;
...
b8 bphysics_body_rotate(struct bphysics_system_state* state, bhandle body, quat rotation)
{
    if (!state)
        return false;
...
b8 bphysics_body_apply_velocity(struct bphysics_system_state* state, bhandle body, vec3 velocity)
{
    if (!state)
        return false;
...
b8 bphysics_body_orientation_get(struct bphysics_system_state* state, bhandle body, mat4* out_orientation)
{
    if (!state)
        return false;
...
b8 bphysics_body_velocity_get(struct bphysics_system_state* state, bhandle body, vec3* out_velocity)
{
    if (!state)
        return false;
...
b8 bphysics_body_apply_impulse(struct bphysics_system_state* state, bhandle body, vec3 point, vec3 force)
{
    if (!state)
        return false;
...
static bphysics_body* create_body(bphysics_system_state* state, bhandle* out_handle)
{
    ...
    u32 handle_index;
    bphysics_body* new_body = bpool_allocate(&state->all_bodies, &handle_index);
    bzero_memory(new_body, sizeof(bphysics_body));
    // Always setup a default rotation
    new_body->rotation = quat_identity();
    if (!new_body)
    {
        BERROR("Failed to allocate from body pool. Pool is full. Increase pool size");
        return 0;
    }

    *out_handle = bhandle_create(handle_index);
...
static void destroy_body(bphysics_system_state* state, bhandle* handle)
{
    ...
    if (bhandle_is_valid(*handle))
    {
        bphysics_body* body = bpool_get_by_index(&state->all_bodies, handle->handle_index);
        if (bhandle_is_pristine(*handle, body->uniqueid))
        {
            if (body->shape_type == BPHYSICS_SHAPE_TYPE_MESH)
            {
                if (body->triangle_count && body->tris)
                    BFREE_TYPE_CARRAY(body->tris, triangle_3d, body->triangle_count);
            }

            bzero_memory(body, sizeof(bphysics_body));
            body->uniqueid = INVALID_ID_U64;

            bpool_free_by_index(&state->all_bodies, handle->handle_index);

            bhandle_invalidate(handle);
        }
    }
}
