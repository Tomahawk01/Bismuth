    ...
    while (filesystem_read_line(&f, 511, &p, &line_length))
    {
        ...
        else if (strings_equali(trimmed_var_name, "max_instances"))
        {
            if (!string_to_u32(trimmed_value, &resource_data->max_instances))
                BERROR("Invalid value for max_instances. Cannot be parsed to u32. Defaulting to &u", resource_data->max_instances);
        }
        ...
        else if (strings_equali(trimmed_var_name, "uniform"))
        {
            ...
            else
            {
                ...
                {
                    // Sampler uniforms are handled entirely different from other uniforms, but share a lot of logic among each other
                    ...
                    else
                    {
                        ...
                        BERROR("Error in shader file: Unsupported sampler type '%s' found. %s:%u", fields[0], full_file_path, line_number);
                        return false;
                    }
                }
