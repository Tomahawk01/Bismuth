...
static b8 shader_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource)
{
    ...
    {
        BERROR("shader_loader_load - unable to open shader file for reading: '%s'", full_file_path);
    ...
    shader_config* resource_data = ballocate(sizeof(shader_config), MEMORY_TAG_RESOURCE);
    ...
    // NOTE: By default, all shaders write to the color buffer unless otherwise specified
    resource_data->flags = SHADER_FLAG_COLOR_WRITE;
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length))
    {
        ...
        if (equal_index == -1)
        {
            BWARN("Potential formatting issue found in file '%s': '=' token not found. Skipping line %ui", out_resource->full_path, line_number);
            line_number++;
            continue;
        }
        ...
        else if (strings_equali(trimmed_var_name, "max_instances"))
        {
            if (!string_to_u32(trimmed_value, &resource_data->max_groups))
                BERROR("Invalid value for max_instances. Cannot be parsed to u32. Defaulting to &u", resource_data->max_groups);
        }
        else if (strings_equali(trimmed_var_name, "stages"))
        {
            ...
            if (resource_data->stage_count == 0)
            {
                ...
                resource_data->stage_configs = ballocate(sizeof(shader_stage_config) * count, MEMORY_TAG_ARRAY);
            }
            else if (resource_data->stage_count != count)
            {
                BERROR("shader_loader_load: Invalid file layout. Count mismatch between stage names and stage filenames");
                return false;
            }
            // Parse stage names
            for (u32 sn_idx = 0; sn_idx < count; ++sn_idx)
            {
                ...
                {
                    BERROR("shader_loader_load: Invalid file layout. Unrecognized stage '%s'", stage_names[sn_idx]);
                }
            }
        ...
        else if (strings_equali(trimmed_var_name, "color_write"))
        {
            b8 color_write;
            string_to_bool(trimmed_value, &color_write);
            // Unset the bit here if color_write is explicitly set to false
            if (!color_write)
                resource_data->flags &= ~SHADER_FLAG_COLOR_WRITE;
        }
        ...
        else if (strings_equali(trimmed_var_name, "attribute"))
        {
            ...
            if (field_count != 2)
            {
                BERROR("shader_loader_load: Invalid file layout. Attribute fields must be 'type,name'. Skipping...");
            }
            else
            {
                ...
                {
                    BERROR("shader_loader_load: Invalid file layout. Attribute type must be f32, vec2, vec3, vec4, i8, i16, i32, u8, u16, or u32");
                    BWARN("Defaulting to f32");
                    attribute.type = SHADER_ATTRIB_TYPE_FLOAT32;
                    attribute.size = 4;
                }
        ...
        else if (strings_equali(trimmed_var_name, "uniform"))
        {
            ...
            if (field_count != 3)
            {
                BERROR("shader_loader_load: Invalid file layout. Uniform fields must be 'type,scope,name'. Skipping...");
            }
            else
            {
                ...
                if (array_length < 1)
                {
                    BWARN("Cannot have an array with a length < 1. Defaulting to 1");
                    array_length = 1;
                }
                char base_type[100];
                if (is_array)
                ...
                else if (string_starts_with(fields[0], "samp"))
                {
                    ...
                    else
                    {
                        ...
                        BERROR("Error in shader file: Unsupported sampler type '%s' found. %s:%u", fields[0], out_resource->full_path, line_number);
                        return false;
                    }
                }
                else if (string_starts_with(fields[0], "tex"))
                {
                    // Texture uniforms are handled entirely different from other uniforms, but share a lot of logic among each other
                    ...
                    {
                        // List out the entire unparsed field to make the error more useful
                        BERROR("Error in shader file: Unsupported texture type '%s' found. %s:%u", fields[0], out_resource->full_path, line_number);
                        return false;
                    }
                }
                ...
                {
                    BERROR("shader_loader_load: Invalid file layout: Uniform frequency must be 0 for per_frame, 1 for per_group or 2 for per_draw");
                    BWARN("Defaulting to global");
                    uniform.frequency = SHADER_UPDATE_FREQUENCY_PER_FRAME;
                }

                // Take copy of the attribute name
...
static void shader_loader_unload(struct resource_loader* self, resource* resource)
{
    ...
    {
        bfree(data->stage_configs, sizeof(shader_stage_config) * data->stage_count, MEMORY_TAG_ARRAY);
        data->stage_count = 0;
    }
    ...
    for (u32 i = 0; i < count; ++i)
    {
        ...
        bfree(data->attributes[i].name, sizeof(char) * (len + 1), MEMORY_TAG_STRING);
    }
    ...
    for (u32 i = 0; i < count; ++i)
    {
        ...
        bfree(data->uniforms[i].name, sizeof(char) * (len + 1), MEMORY_TAG_STRING);
    }
    ...
    if (!resource_unload(self, resource, MEMORY_TAG_RESOURCE))
        BWARN("shader_loader_unload called with nullptr for self or resource");
}
