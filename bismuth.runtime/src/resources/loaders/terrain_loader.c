...
static b8 terrain_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource)
{
    ...
    // FIXME: This won't work until the asset/resource loaders are completed
    return false;
    ...
    /*
        ...
        string_format_unsafe(full_file_path, format_str, resource_system_base_path(), self->type_path, name, ".bterrain");

        file_handle f;
        if (!filesystem_open(full_file_path, FILE_MODE_READ, false, &f))
        {
            BERROR("terrain_loader_load - unable to open terrain file for reading: '%s'", full_file_path);
            return false;
        }
        ...
        while (filesystem_read_line(&f, 511, &p, &line_length))
        {
            ...
            if (strings_equali(trimmed_var_name, "version"))
            {
                if (!string_to_u32(trimmed_value, &version))
                    BWARN("Format error - invalid file version");
            }
            ...
            else if (strings_equali(trimmed_var_name, "chunk_size"))
            {
                ...
                {
                    BWARN("Format error - invalid chunk size. Defaulting to 16");
        ...
        if (heightmap_file)
        {
            ...
            if (!resource_system_load(heightmap_file, RESOURCE_TYPE_IMAGE, &params, &heightmap_image_resource))
            {
                BERROR("Unable to load heightmap file for terrain. Setting some reasonable defaults");
            ...
            {
                BERROR("Heightmap dimensions must be a multiple of chunk size. (map w='%u', h='%u', chunk_size='%u')", resource_data->tile_count_x, resource_data->tile_count_z, resource_data->chunk_size);
                return false;
            }
