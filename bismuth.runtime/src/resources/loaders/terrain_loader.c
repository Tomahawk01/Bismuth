...
static b8 terrain_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource)
{
    ...
    // Load heightmap if one is configured
    if (heightmap_file)
    {
        ...
        {
            BERROR("Unable to load heightmap file for terrain. Setting defaults");
        ...
        {
            BERROR("Heightmap dimensions must be a multiple of chunk size. (map w='%u', h='%u', chunk_size='%u')",
                resource_data->tile_count_x,
                resource_data->tile_count_z,
                resource_data->chunk_size);
            return false;
        }
    ...
    else
    {
        ...
        BWARN("No heightmap was included, using defaults for terrain generation");
        resource_data->tile_count_x = resource_data->tile_count_z = 128;
        resource_data->chunk_size = 16;
        resource_data->vertex_data_length = resource_data->tile_count_x * resource_data->tile_count_z;
        resource_data->vertex_datas = darray_reserve(terrain_vertex_data, resource_data->vertex_data_length);
