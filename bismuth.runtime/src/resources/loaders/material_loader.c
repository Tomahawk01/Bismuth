...
#define MATERIAL_PARSE_VERIFY_MODE(expected_mode, actual_mode, var_name, expected_mode_str)                                  \
    ...
    {                                                                                                                        \
        BERROR("Format error: unexpected variable '%s', should only exist inside a '%s' node", var_name, expected_mode_str); \
        return false;                                                                                                        \
    }
...
static b8 material_parse_filter(const char *trimmed_value, const char *trimmed_var_name, material_parse_mode parse_mode, texture_filter *filter)
{
    ...
    {
        BERROR("Format error, unknown filter mode %s, defaulting to LINEAR", trimmed_value);
        *filter = TEXTURE_FILTER_MODE_LINEAR;
    }
    return true;
}
...
static b8 material_parse_repeat(const char *trimmed_value, const char *trimmed_var_name, material_parse_mode parse_mode, texture_repeat *repeat)
{
    ...
    {
        BERROR("Format error, unknown repeat mode %s, defaulting to REPEAT", trimmed_value);
        *repeat = TEXTURE_REPEAT_REPEAT;
    }
    return true;
}

static material_map material_map_create_default(const char *name, const char *texture_name)
{
    material_map new_map = {};
    // Set reasonable defaults for old material types
    new_map.name = string_duplicate(name);
    new_map.repeat_u = new_map.repeat_v = new_map.repeat_w = TEXTURE_REPEAT_REPEAT;
...
static shader_uniform_type material_parse_prop_type(const char *strval)
{
    ...
    {
        BERROR("Unexpected type: '%s'. Defaulting to i32", strval);
        return SHADER_UNIFORM_TYPE_INT32;
    }
}
...
static b8 material_loader_load(struct resource_loader* self, const char* name, void* params, resource* out_resource)
{
    ...
    string_format_unsafe(full_file_path, format_str, resource_system_base_path(), self->type_path, name, ".bmt");

    file_handle f;
    if (!filesystem_open(full_file_path, FILE_MODE_READ, false, &f))
    {
        BERROR("material_loader_load - unable to open material file for reading: '%s'", full_file_path);
        return false;
    }

    out_resource->full_path = string_duplicate(full_file_path);

    material_config *resource_data = ballocate(sizeof(material_config), MEMORY_TAG_RESOURCE);
    // Set some defaults
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length))
    {
        ...
        if (trimmed[0] == '[')
        {
            ...
            if (trimmed[1] == '/')
            {
                ...
                switch (parse_mode)
                {
                    ...
                    case MATERIAL_PARSE_MODE_GLOBAL:
                        BERROR("Unexpected token '/' at line %d, Format error: closing tag found while in global scope", line_number);
                        return false;
                    case MATERIAL_PARSE_MODE_MAP:
                        darray_push(resource_data->maps, current_map);
                        bzero_memory(&current_map, sizeof(material_map));
                        parse_mode = MATERIAL_PARSE_MODE_GLOBAL;
                        continue;
                    case MATERIAL_PARSE_MODE_PROPERTY:
                        darray_push(resource_data->properties, current_prop);
                        bzero_memory(&current_prop, sizeof(material_config_prop));
                        parse_mode = MATERIAL_PARSE_MODE_GLOBAL;
                        continue;
                }
            }
            else
            {
                ...
                {
                    BERROR("Format error: Unexpected opening tag %s at line %d", trimmed, line_number);
                    return false;
                }
            }
        }
        ...
        if (strings_equali(trimmed_var_name, "version"))
        {
            ...
            {
                BERROR("Format error: failed to parse version. Aborting");
            ...
            switch (parse_mode)
            {
                ...
                case MATERIAL_PARSE_MODE_GLOBAL:
                    if (resource_data->name)
                    {
                        ...
                        bfree(resource_data->name, len + 1, MEMORY_TAG_STRING);
        ...
        else if (strings_equali(trimmed_var_name, "diffuse_map_name"))
        {
            if (resource_data->version == 1)
            {
                ...
                BERROR("Format error: unexpected variable 'diffuse_map_name', this should only exist for version 1 materials. Ignored");
        }
           ...
           if (resource_data->version == 1)
            {
                ...
                BERROR("Format error: unexpected variable 'specular_map_name', this should only exist for version 1 materials. Ignored");
        }
        else if (strings_equali(trimmed_var_name, "normal_map_name"))
        {
            if (resource_data->version == 1)
            {
                ...
                BERROR("Format error: unexpected variable 'normal_map_name', this should only exist for version 1 materials. Ignored");
        }
        else if (strings_equali(trimmed_var_name, "diffuse_color"))
        {
            if (resource_data->version == 1)
            {
                material_config_prop new_prop = material_config_prop_create("diffuse_color", SHADER_UNIFORM_TYPE_FLOAT32_4, trimmed_value);
                darray_push(resource_data->properties, new_prop);
            }
            else
                BERROR("Format error: unexpected variable 'diffuse_color', this should only exist for version 1 materials. Ignored");
        }
        ...
        else if (strings_equali(trimmed_var_name, "shininess"))
        {
            if (resource_data->version == 1)
            {
                material_config_prop new_prop = material_config_prop_create("shininess", SHADER_UNIFORM_TYPE_FLOAT32, trimmed_value);
                darray_push(resource_data->properties, new_prop);
            }
            else
                BERROR("Format error: unexpected variable 'shininess', this should only exist for version 1 materials. Ignored");
        }
        else if (strings_equali(trimmed_var_name, "type"))
        {
            if (resource_data->version >= 2)
            {
                if (parse_mode == MATERIAL_PARSE_MODE_GLOBAL)
                {
                    ...
                    {
                        BERROR("Phong materials are no longer supported. Attempting to convert to PBR");
                        ...
                        BERROR("Format error: Unexpected material type '%s' (Material='%s')", trimmed_value, resource_data->name);
                }
                else if (parse_mode == MATERIAL_PARSE_MODE_PROPERTY)
                    current_prop.type = material_parse_prop_type(trimmed_value);
                else
                  BERROR("Format error: Unexpected variable 'type' in mode");
            }
            else
                BERROR("Format error: Unexpected variable 'type', this should only exist for version 2+ materials");
        }
        else if (strings_equali(trimmed_var_name, "filter_min"))
        {
            ...
            {
                // NOTE: Handled successfully with a default
            }
        }
