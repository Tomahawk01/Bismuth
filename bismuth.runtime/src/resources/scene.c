...
b8 scene_create(bresource_scene* config, scene_flags flags, scene* out_scene)
{
    if (!out_scene)
    {
...
void scene_node_initialize(scene* s, bhandle parent_handle, scene_node_config* node_config)
{
    if (node_config)
    {
        ...
        bhandle hierarchy_node_handle = hierarchy_graph_child_add_with_xform(&s->hierarchy, parent_handle, xform_handle);
        ...
        if (node_config->skybox_configs)
        {
            ...
            for (u32 i = 0; i < count; ++i)
            {
                ...
                else
                {
                    ...
                    attachment->resource_handle = bhandle_create(index);
                    ...
                    {
                        attachment->tags = BALLOC_TYPE_CARRAY(bname, attachment->tag_count);
                        BCOPY_TYPE_CARRAY(attachment->tags, typed_attachment_config->base.tags, bname, attachment->tag_count);
                    }
        ...
        if (node_config->dir_light_configs)
        {
            ...
            for (u32 i = 0; i < count; ++i)
            {
                ...
                new_light.data.color = typed_attachment_config->color;
                ...
                new_light.debug_data = BALLOC_TYPE(scene_debug_data, MEMORY_TAG_RESOURCE);
        ...
        if (node_config->audio_emitter_configs)
        {
            ...
            for (u32 i = 0; i < count; ++i)
            {
                ...
                /* vec3 pos = vec3_zero();
                if (bhandle_is_valid(xform_handle))
        ...
        if (node_config->static_mesh_configs)
        {
            ...
            for (u32 i = 0; i < count; ++i)
            {
                ...
                {
                    BWARN("Invalid static mesh config, asset_name is required");
                    return;
                }
        ...
        if (node_config->heightmap_terrain_configs)
        {
            ...
            for (u32 i = 0; i < count; ++i)
            {
                ...
                if (!typed_attachment_config->asset_name)
                {
                    BWARN("Invalid heightmap terrain config, asset_name is required");
                    return;
                }
                ...
                request.type = BRESOURCE_TYPE_HEIGHTMAP_TERRAIN;
                request.assets = array_bresource_asset_info_create(1);
                request.assets.data[0].type = BASSET_TYPE_HEIGHTMAP_TERRAIN;
                ...
                bresource_heightmap_terrain* terrain_resource = (bresource_heightmap_terrain*)bresource_system_request(
                ...
                {
                    BWARN("Failed to load heightmap terrain");
                    return;
                }

                if (!terrain_initialize(&new_terrain))
                {
                    BERROR("Failed to initialize heightmap terrain");
                    return;
                }
                else
                {
        ...
        if (node_config->volume_configs)
        {
            ...
            for (u32 i = 0; i < count; ++i)
            {
                ...
                {
                    new_volume.hit_sphere_tags = BALLOC_TYPE_CARRAY(bname, new_volume.hit_sphere_tag_count);
                    BCOPY_TYPE_CARRAY(new_volume.hit_sphere_tags, typed_attachment_config->hit_sphere_tags, bname, new_volume.hit_sphere_tag_count);
                }
                else
                {
                ...
                case SCENE_VOLUME_SHAPE_TYPE_RECTANGLE:
                    ...
                    if (init_result)
                    {
                        debug_box3d_color_set(&debug->box, (vec4){1.0f, 0.0f, 1.0f, 1.0f});
                ...
                {
                    BERROR("Failed to create debug shape for volume");
                }
            }
        }
        ...
        if (node_config->hit_sphere_configs)
        {
            ...
            for (u32 i = 0; i < count; ++i)
            {
                ...
                // Add debug data and initialize it
                new_hit_sphere.debug_data = BALLOC_TYPE(scene_debug_data, MEMORY_TAG_RESOURCE);
                scene_debug_data* debug = new_hit_sphere.debug_data;
                // Create debug sphere TODO: color?
                if (!debug_sphere3d_create(typed_attachment_config->radius, (vec4){0.0f, 0.5f, 1.0f, 1.0f}, bhandle_invalid(), &debug->sphere))
                {
                    BERROR("Failed to create debug sphere for hit sphere");
                }
                else
                {
                    // xform_position_set(debug->sphere.xform, new_emitter.data.position);
                ...
                if (debug_sphere3d_initialize(&debug->sphere))
                {
                    ...
                    attachment->resource_handle = bhandle_create(hit_sphere_count);
                    attachment->hierarchy_node_handle = hierarchy_node_handle;
                ...
                {
                    BERROR("Failed to create debug shape for hit sphere");
                }
            }
        }
...
b8 scene_initialize(scene* scene)
{
    ...
    if (scene->config)
    {
        bresource_scene* config = scene->config;
...
b8 scene_load(scene* scene)
{
    ...
    {
        struct baudio_system_state* audio_state = engine_systems_get()->audio_system;
        ...
        for (u32 i = 0; i < emitter_count; ++i)
        {
            ...
            bhandle xform_handle = hierarchy_graph_xform_handle_get(&scene->hierarchy, emitter_attachment->hierarchy_node_handle);
    ...
    if (scene->volumes)
    {
        ...
        for (u32 i = 0; i < volume_count; ++i)
        {
            ...
            if (!load_result)
            {
                BERROR("debug shape failed to load");

                bfree(scene->volumes[i].debug_data, sizeof(scene_debug_data), MEMORY_TAG_RESOURCE);
    ...
    if (scene->hit_spheres)
    {
        ...
        for (u32 i = 0; i < hit_sphere_count; ++i)
        {
            ...
            if (!debug_sphere3d_load(&debug->sphere))
            {
                ...
                bfree(scene->hit_spheres[i].debug_data, sizeof(scene_debug_data), MEMORY_TAG_RESOURCE);
...
static b8 any_tags_match(const bname* list_0, u32 count_0, const bname* list_1, u32 count_1)
{
    ...
    if (scene->state == SCENE_STATE_LOADED)
    {
        ...
        if (scene->volumes)
        {
            ...
            for (u32 i = 0; i < volume_count; ++i)
            {
                ...
                /* bhandle vol_xform_handle = hierarchy_graph_xform_handle_get(&scene->hierarchy, vol_attachment->hierarchy_node_handle); */
                ...
                mat4 world;
                if (!bhandle_is_invalid(vol_xform_handle))
                {
                ...
                for (u32 j = 0; j < hit_sphere_count; ++j)
                {
                    ...
                    if (matches_tag)
                    {
                        ...
                        /* scene_attachment* hs_attachment = &scene->hit_sphere_attachments[j];
                        bhandle hs_hierarchy_node_handle = hs_attachment->hierarchy_node_handle;
                        bhandle hs_xform_handle = hierarchy_graph_xform_handle_get(&scene->hierarchy, hs_hierarchy_node_handle);
                        ...
                        scene_attachment* hs_attachment = &scene->hit_sphere_attachments[j];
                        bhandle hs_xform_handle = hierarchy_graph_xform_handle_get(&scene->hierarchy, hs_attachment->hierarchy_node_handle);

                        mat4 world;
                        if (!bhandle_is_invalid(hs_xform_handle))
                        {
        ...
        if (scene->audio_emitters)
        {
            ...
            {
                scene_audio_emitter* emitter = &scene->audio_emitters[i];
                // Update the audio emitter's data position (world position) to take into account the owning node's transform
                scene_attachment* emitter_attachment = &scene->audio_emitter_attachments[i];
                bhandle xform_handle = hierarchy_graph_xform_handle_get(&scene->hierarchy, emitter_attachment->hierarchy_node_handle);
        ...
        if (scene->volumes) {
            ...
            for (u32 i = 0; i < volume_count; ++i)
            {
                ...
                bhandle xform_handle = hierarchy_graph_xform_handle_get(&scene->hierarchy, volume_attachment->hierarchy_node_handle);
...
b8 scene_node_xform_get_by_name(const scene* scene, bname name, bhandle* out_xform_handle)
{
    ...
    for (u32 i = 0; i < scene->node_metadata_count; ++i)
    {
        ...
        {
            bhandle hierarchy_node_handle = bhandle_create_with_u64_identifier(node_meta->index, node_meta->uniqueid);
...
b8 scene_node_xform_get(const scene* scene, bhandle node_handle, bhandle* out_xform_handle)
{
    if (!scene || bhandle_is_invalid(node_handle) || !out_xform_handle)
        return false;

    if (!bhandle_is_pristine(node_handle, scene->node_metadata->uniqueid))
    {
        BERROR("%s was called with a stale node_handle", __FUNCTION__);
        return false;
    }
...
b8 scene_node_local_matrix_get(const scene* scene, bhandle node_handle, mat4* out_matrix)
{
    if (!scene || bhandle_is_invalid(node_handle) || !out_matrix)
        return false;
...
b8 scene_node_local_matrix_get_by_name(const scene* scene, bname name, mat4* out_matrix)
{
...
b8 scene_node_exists(const scene* s, bname name)
{
    if (!s || !name)
        return false;
...
b8 scene_node_child_count_get(const scene* s, bname name, u32* out_child_count)
{
...
b8 scene_node_child_name_get_by_index(const scene* s, bname name, u32 index, bname* out_child_name)
{
    ...
    for (u32 i = 0; i < s->node_metadata_count; ++i)
    {
        ...
        if (node_meta->name == name)
        {
            bhandle parent_node_handle = bhandle_create_with_u64_identifier(node_meta->index, node_meta->uniqueid);

            bhandle child_handle = {0};
...

    {
        ...
        for (u32 i = 0; i < volume_count; ++i)
        {
            ...
            if (s->volumes[i].debug_data)
            {
                ...
                bfree(s->volumes[i].debug_data, sizeof(scene_debug_data), MEMORY_TAG_RESOURCE);
...
static b8 scene_serialize_node(const scene* s, const hierarchy_graph_view* view, const hierarchy_graph_view_node* view_node, bson_property* node)
{
    ...
    // Node name
    bson_object_value_add_bname_as_string(&node->value.o, "name", node_meta->name);
    ...
    for (u32 m = 0; m < mesh_count; ++m)
    {
        if (s->mesh_attachments[m].hierarchy_node_handle.handle_index == view_node->node_handle.handle_index)
        {
            ...
            bson_object_value_add_bname_as_string(&attachment.value.o, "asset_name", s->mesh_metadata[m].resource_name);
            bson_object_value_add_bname_as_string(&attachment.value.o, "package_name", s->mesh_metadata[m].package_name);
    ...
    for (u32 m = 0; m < skybox_count; ++m)
    {
        if (s->skybox_attachments[m].hierarchy_node_handle.handle_index == view_node->node_handle.handle_index)
        {
            ...
            bson_object_value_add_bname_as_string(&attachment.value.o, "cubemap_image_asset_name", s->skybox_metadata[m].cubemap_name);
            bson_object_value_add_bname_as_string(&attachment.value.o, "cubemap_image_asset_package_name", s->mesh_metadata[m].package_name);
    ...
    for (u32 m = 0; m < terrain_count; ++m)
    {
        if (s->terrain_attachments[m].hierarchy_node_handle.handle_index == view_node->node_handle.handle_index)
        {
            ...
            bson_object_value_add_bname_as_string(&attachment.value.o, "name", s->terrain_metadata[m].name);
            bson_object_value_add_bname_as_string(&attachment.value.o, "asset_name", s->terrain_metadata[m].resource_name);
            bson_object_value_add_bname_as_string(&attachment.value.o, "package_name", s->terrain_metadata[m].package_name);
...
b8 scene_save(scene* s)
{
    ...
    bson_object_value_add_bname_as_string(&properties.value.o, "name", s->name);
    ...
    if (s && handle_index != INVALID_ID_U64)
    {
        ...
        if (s->node_metadata_count < new_count)
        {
            ...
            for (u32 i = s->node_metadata_count; i < new_count; ++i)
            {
                ...
                s->node_metadata[i].name = INVALID_BNAME;
            }
