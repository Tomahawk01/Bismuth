...
b8 debug_box3d_create(vec3 size, bhandle parent_xform, debug_box3d* out_box)
{
    if (!out_box)
        return false;
...
void debug_box3d_parent_set(debug_box3d* box, bhandle parent_xform)
{
    if (box)
...
void debug_box3d_color_set(debug_box3d* box, vec4 color)
{
    if (box)
    {
        if (color.a == 0)
            color.a = 1.0f;
        box->color = color;
        vec4_clamp(&box->color, 0.0f, 1.0f);
...
static void update_vert_color(debug_box3d* box)
{
    if (box)
    {
        ...
        {
            color_vertex_3d* verts = (color_vertex_3d*)box->geometry.vertices;
            for (u32 i = 0; i < box->geometry.vertex_count; ++i)
                verts[i].color = box->color;
            box->is_dirty = true;
        }
    }
}
