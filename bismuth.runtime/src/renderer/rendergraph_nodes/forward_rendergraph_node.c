...
#define UNIFORM_APPLY_OR_FAIL(expr)                   \
    if (!expr)                                        \
    {                                                 \
        BERROR("Failed to apply uniform: %s", #expr); \
        return false;                                 \
    }
...
typedef struct forward_rendergraph_node_internal_data
{
    ...
    bresource_texture** ibl_cube_textures;

    // Skybox shader
    bhandle skybox_shader;
...
b8 forward_rendergraph_node_create(struct rendergraph* graph, struct rendergraph_node* self, const struct rendergraph_node_config* config)
{
    if (!self)
    {
        BERROR("forward_rendergraph_node_create requires a valid pointer to a pass");
        return false;
    }
    if (!config)
    {
        BERROR("forward_rendergraph_node_create requires a valid configuration");
        return false;
    }
    ...
    {
        BERROR("Forward rendergraph node requires configuration for sink called 'colorbuffer'");
        return false;
    }
    else
    {
    ...
    {
        BERROR("Forward rendergraph node requires configuration for sink called 'depthbuffer'");
        return false;
    }
    else
    {
    ...
    if (!depthbuffer_sink_config)
    {
        BERROR("Forward rendergraph node requires configuration for sink called 'shadow'");
        return false;
    }
    else
    {
    ...
    // Has two sources, one for the colorbuffer and one for the depthbuffer
    self->source_count = 2;
...
b8 forward_rendergraph_node_initialize(struct rendergraph_node* self)
{
    ...
    internal_data->skybox_shader_locations.frame_ubo = shader_system_uniform_location(internal_data->skybox_shader, bname_create("skybox_frame_ubo"));
    if (internal_data->skybox_shader_locations.frame_ubo == INVALID_ID_U16)
    {
        BERROR("Failed to shader get uniform location for skybox_frame_ubo");
        return false;
    }

    internal_data->skybox_shader_locations.cube_texture = shader_system_uniform_location(internal_data->skybox_shader, bname_create("cube_texture"));
    if (internal_data->skybox_shader_locations.cube_texture == INVALID_ID_U16)
    {
        BERROR("Failed to shader get uniform location for cube_texture");
        return false;
    }

    internal_data->skybox_shader_locations.cube_sampler = shader_system_uniform_location(internal_data->skybox_shader, bname_create("cube_sampler"));
    if (internal_data->skybox_shader_locations.cube_sampler == INVALID_ID_U16)
    {
        BERROR("Failed to shader get uniform location for cube_sampler");
        return false;
    }

    internal_data->skybox_shader_locations.draw_ubo = shader_system_uniform_location(internal_data->skybox_shader, bname_create("skybox_draw_ubo"));
    if (internal_data->skybox_shader_locations.draw_ubo == INVALID_ID_U16)
    {
        BERROR("Failed to shader get uniform location for skybox_draw_ubo");
        return false;
    }
...
b8 forward_rendergraph_node_load_resources(struct rendergraph_node* self)
{
    ...
    {
        BERROR("Required '%s' source not hooked up to forward pass. Creation fails", "shadowmap");
        return false;
    }
...
b8 render_water_planes(forward_rendergraph_node_internal_data* internal_data, u32 plane_count, water_plane** planes, bresource_texture* color, bresource_texture* depth, vec4 clipping_plane, camera* cam, struct frame_data* p_frame_data)
{
    ...
    if (plane_count)
    {
        ...
        for (u32 i = 0; i < plane_count; ++i)
        {
            ...
            {
                BERROR("Error applying material. See logs for details");
                return false;
            }
            ...
            {
                BERROR("Failed to apply per-instance material data. See logs for details");
                return false;
            }
            ...
            {
                BERROR("Failed to bind vertex buffer data for water plane");
                return false;
            }
            if (!renderer_renderbuffer_draw(internal_data->index_buffer, plane->index_buffer_offset, 6, false))
            {
                BERROR("Failed to draw water plane using index data");
                return false;
            }
        }
    }
...
b8 render_scene(forward_rendergraph_node_internal_data* internal_data, bresource_texture* color, bresource_texture* depth, u32 plane_count, water_plane** planes, b8 include_water_plane, vec4 clipping_plane, camera* cam, camera* inverted_cam, b8 use_inverted, struct frame_data* p_frame_data)
{
    ...
    else
    {
        ...
        rect_2d viewport_rect = (vec4){0, 0 + (f32)color->height, (f32)color->width, -(f32)color->height};
    ...
    if (internal_data->sb && internal_data->sb->state == SKYBOX_STATE_LOADED)
    {
        if (internal_data->sb->geometry.generation != INVALID_ID_U16)
        {
            ...
            renderer_begin_rendering(internal_data->renderer, p_frame_data, internal_data->vp.rect, 1, &color->renderer_texture_handle, bhandle_invalid(), 0);
            ...
            // Apply per-frame
            {
                ...
                {
                    BERROR("Failed to apply skybox frame ubo");
                    return false;
                }
    ...
    if (terrain_geometry_count > 0)
    {
        ...
        {
            BERROR("Failed to use terrain pbr shader");
            return false;
        }
        ...
        for (u32 i = 0; i < terrain_geometry_count; ++i)
        {
            ...
            // Apply instance
            {
                ...
                m->maps[TERRAIN_SAMP_IDX_SHADOW_MAP].texture = shadow_map_texture ? shadow_map_texture : texture_system_get_default_bresource_terrain_texture(internal_data->texture_system);
    ...
    // Static geometries
    {
        ...
        if (geometry_count > 0)
        {
            ...
            // Keep track of when transparent rendering begins. The water plane, if drawn, must happen before this.
            // NOTE: This may cause problems with transparent objects behind the water plane
            ...
            for (u32 i = 0; i < count; ++i)
            {
                ...
                if (inst->material.handle_index != current_material.handle_index)
                {
                    ...
                    b8 has_transparency = material_flag_get(internal_data->material_system, inst->material, BMATERIAL_FLAG_HAS_TRANSPARENCY_BIT);
                    if (include_water_plane && !transparency_started && has_transparency)
                    {
                        if (!render_water_planes(internal_data, plane_count, planes, color, depth, vec4_zero(), cam, p_frame_data))
                        {
                            BERROR("Failed to draw water plane! See logs for details");
                            return false;
                        }
                        else
                        {
    ...
    {
        if (!render_water_planes(internal_data, internal_data->water_plane_count, internal_data->water_planes, color, depth, vec4_zero(), cam, p_frame_data))
        {
            BERROR("Failed to draw water plane! See logs for details");
            return false;
        }
...
b8 forward_rendergraph_node_execute(struct rendergraph_node* self, struct frame_data* p_frame_data)
{
    ...
    // renderer_winding_set(RENDERER_WINDING_COUNTER_CLOCKWISE);
    // renderer_active_viewport_set(&internal_data->vp);
    // rect_2d scissor_rect = (vec4){0, 0, (f32)internal_data->colorbuffer_texture->width, (f32)internal_data->colorbuffer_texture->height};
    // renderer_scissor_set(scissor_rect);
...
b8 forward_rendergraph_node_cascade_data_set(struct rendergraph_node* self, f32 split, mat4 dir_light_view, mat4 dir_light_projection, u8 cascade_index)
{
    if (self && self->internal_data)
    {
        ...
        {
            BERROR("Shadow cascade index out of bounds: %d is not in range [0-%s]", cascade_index, MATERIAL_MAX_SHADOW_CASCADES - 1);
            return false;
        }
        forward_rendergraph_node_internal_data* internal_data = self->internal_data;
...
b8 forward_rendergraph_node_water_planes_set(struct rendergraph_node* self, struct frame_data* p_frame_data, u32 count, struct water_plane** planes)
{
    if (self && self->internal_data)
    {
        ...
        if (count > 0)
        {
            ...
            bcopy_memory(internal_data->water_planes, planes, sizeof(struct water_plane*) * count);
        }
        else
        {
