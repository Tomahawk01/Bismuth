...
b8 forward_rendergraph_node_initialize(struct rendergraph_node* self)
{
    ...
    // Save off a pointer to the color3d shader as well as its uniform locations
...
b8 forward_rendergraph_node_execute(struct rendergraph_node* self, struct frame_data* p_frame_data)
{
    ...
    // Begin rendering
    renderer_begin_rendering(internal_data->renderer, p_frame_data, 1, &internal_data->colorbuffer_texture->renderer_texture_handle, internal_data->depthbuffer_texture->renderer_texture_handle);
    ...
    {
        shader_system_use_by_id(internal_data->color_shader_id);

        // Globals
        UNIFORM_APPLY_OR_FAIL(shader_system_uniform_set_by_location(internal_data->color_shader_id, internal_data->debug_locations.projection, &internal_data->projection_matrix));
        UNIFORM_APPLY_OR_FAIL(shader_system_uniform_set_by_location(internal_data->color_shader_id, internal_data->debug_locations.view, &internal_data->view_matrix));

        if (!shader_system_apply_global(internal_data->color_shader_id))
        {
            BERROR("Failed to apply globals for color shader");
            return false;
        }
        ...
        for (u32 i = 0; i < debug_geometry_count; ++i)
        {
            ...
            // Local
            UNIFORM_APPLY_OR_FAIL(shader_system_uniform_set_by_location(internal_data->color_shader_id, internal_data->debug_locations.model, &internal_data->debug_geometries[i].model));
            if (!shader_system_apply_local(internal_data->color_shader_id))
                BERROR("Failed to apply locals for color shader");
...
b8 forward_rendergraph_node_debug_geometries_set(struct rendergraph_node* self, struct frame_data* p_frame_data, u32 geometry_count, const struct geometry_render_data* geometries)
{
    if (self && self->internal_data)
    {
        ...
        bcopy_memory(internal_data->debug_geometries, geometries, sizeof(geometry_render_data) * geometry_count);
        return true;
    }
    return false;
}
