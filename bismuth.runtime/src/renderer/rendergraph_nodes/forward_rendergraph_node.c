...
b8 render_water_planes(forward_rendergraph_node_internal_data* internal_data, u32 plane_count, water_plane** planes, bresource_texture* color, bresource_texture* depth, vec4 clipping_plane, camera* cam, struct frame_data* p_frame_data)
{
    ...
    if (plane_count)
    {
        ...
        b_handle game_timeline = timeline_system_get_game();
        ...
        for (u32 i = 0; i < plane_count; ++i)
        {
            ...
            plane->maps[WATER_PLANE_MAP_SHADOW].texture = shadow_map_texture ? shadow_map_texture : texture_system_get_default_bresource_terrain_texture(internal_data->texture_system);
            ...
            for (u32 i = 0; i < count; ++i)
            {
                ...
                if (m->internal_id != current_material_id)
                {
                    ...
                    if (include_water_plane && !transparency_started && m->maps[0].texture->flags & TEXTURE_FLAG_HAS_TRANSPARENCY)
                    {
                        ...
                        if (!shader_system_use_by_id(internal_data->pbr_shader_id))
                        {
                            BERROR("Failed to switch back to PBR shader. Render frame failed");
                            return false;
                        }
...
b8 forward_rendergraph_node_cascade_data_set(struct rendergraph_node* self, f32 split, mat4 dir_light_view, mat4 dir_light_projection, u8 cascade_index)
{
    if (self && self->internal_data)
    {
        ...
        {
            BERROR("Shadow cascade index out of bounds: %d is not in range [0-%s]", cascade_index, MAX_SHADOW_CASCADE_COUNT - 1);
            return false;
        }
        forward_rendergraph_node_internal_data* internal_data = self->internal_data;
