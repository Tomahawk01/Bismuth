...
b8 clear_depth_rendergraph_node_create(struct rendergraph* graph, struct rendergraph_node* self, const struct rendergraph_node_config* config)
{
    ...
    {
        BERROR("Failed to deserialize configuration for clear_depth_rendergraph_node. Node creation failed");
        return false;
    }

    self->internal_data = ballocate(sizeof(clear_depth_rendergraph_node_internal_data), MEMORY_TAG_RENDERER);
...
static b8 deserialize_config(const char* source_str, clear_depth_rendergraph_node_config* out_config)
{
    ...
    {
        BERROR("Failed to parse config for clear_depth_rendergraph_node");
        return false;
    }
    ...
    result = bson_object_property_value_get_float(&tree.root, "depth_clear_value", &out_config->depth_clear_value);
    if (!result)
        out_config->depth_clear_value = 1.0f;

    i64 stencil_clear_value;
    result = bson_object_property_value_get_int(&tree.root, "depth_clear_value", &stencil_clear_value);
