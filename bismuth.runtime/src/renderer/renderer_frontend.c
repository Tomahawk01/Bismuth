...
typedef struct renderer_system_state
{
    ...
    // Default textures. Registered from the texture system
    bhandle default_textures[RENDERER_DEFAULT_TEXTURE_COUNT];
...
b8 renderer_system_deserialize_config(const char* config_str, renderer_system_config* out_config)
{
    ...
    {
        BERROR("renderer_system_deserialize_config requires a valid pointer to out_config and config_str");
        return false;
    }
    ...
    {
        BERROR("Failed to parse renderer system config");
        return false;
    }
    ...
    {
        BERROR("renderer_system_deserialize_config: config does not contain backend_plugin_name, which is required");
        return false;
    }

    if (!bson_object_property_value_get_bool(&tree.root, "vsync", &out_config->vsync))
    {
        // Default to true if not defined
        out_config->vsync = true;
    }

    if (!bson_object_property_value_get_bool(&tree.root, "enable_validation", &out_config->enable_validation))
    {
        // Default to true if not defined
        out_config->enable_validation = true;
    }

    if (!bson_object_property_value_get_bool(&tree.root, "power_saving", &out_config->power_saving))
    {
        // Default to true if not defined
...
static b8 renderer_on_event(u16 code, void* sender, void* listener_inst, event_context context)
{
    if (code == EVENT_CODE_BVAR_CHANGED)
    {
        ...
        {
            i32 use_pcf_val;
            bvar_i32_get("use_pcf", &use_pcf_val);
...
b8 renderer_system_initialize(u64* memory_requirement, renderer_system_state* state, const renderer_system_config* config)
{
    *memory_requirement = sizeof(renderer_system_state);
    ...
    // Create vsync bvar
    bvar_i32_set("vsync", 0, (renderer_config.flags & RENDERER_CONFIG_FLAG_VSYNC_ENABLED_BIT) ? 1 : 0);

    // Renderer options
    // Add a bvar to track PCF filtering enabled/disabled
    bvar_i32_set("use_pcf", 0, 1); // On by default
    i32 use_pcf_val;
    bvar_i32_get("use_pcf", &use_pcf_val);
    state->use_pcf = use_pcf_val == 0 ? false : true;
    event_register(EVENT_CODE_BVAR_CHANGED, state, renderer_on_event);

    // Initialize backend
    if (!state->backend->initialize(state->backend, &renderer_config))
    {
        BERROR("Renderer backend failed to initialize. Shutting down...");
        return false;
    }
    ...
    {
        state->default_textures[i] = bhandle_invalid();
    }
...
b8 renderer_on_window_created(struct renderer_system_state* state, struct bwindow* window)
{
    if (!window)
    {
        BERROR("renderer_on_window_created requires a valid pointer to a window");
        return false;
    }
...
void renderer_on_window_destroyed(struct renderer_system_state* state, struct bwindow* window)
{
    if (window)
    {
...
b8 renderer_frame_prepare_window_surface(struct renderer_system_state* state, struct bwindow* window, struct frame_data* p_frame_data)
{
    BASSERT(state && window && p_frame_data);
...
b8 renderer_frame_present(struct renderer_system_state* state, struct bwindow* window, struct frame_data* p_frame_data)
{
...
// Verify handles in debug builds, but not release
#ifdef BISMUTH_DEBUG
    // If color targets are used, none should be invalid
    ...
    state->backend->begin_rendering(state->backend, p_frame_data, render_area, color_target_count, color_targets, depth_stencil_target, depth_stencil_layer);
...
b8 renderer_bresource_texture_resources_acquire(struct renderer_system_state* state, bname name, bresource_texture_type type, u32 width, u32 height, u8 channel_count, u8 mip_levels, u16 array_size, bresource_texture_flag_bits flags, bhandle* out_renderer_texture_handle)
{
    ...
    *out_renderer_texture_handle = bhandle_invalid();
...
void renderer_default_texture_register(struct renderer_system_state* state, renderer_default_texture default_texture, bhandle renderer_texture_handle)
{
    if (state && !bhandle_is_invalid(renderer_texture_handle))
    {
...
bhandle renderer_default_texture_get(struct renderer_system_state* state, renderer_default_texture default_texture)
{
    if (state)
    {
...
renderbuffer* renderer_renderbuffer_get(renderbuffer_type type)
{
    ...
    switch (type)
    {
        ...
        default:
            BERROR("Unsupported buffer type %u", type);
            return 0;
    }
}
...
b8 renderer_geometry_upload(bgeometry* g)
{
    if (!g)
    {
        BERROR("renderer_geometry_upload requires a valid pointer to geometry");
        return false;
    }
    renderer_system_state* state_ptr = engine_systems_get()->renderer_system;
...
void renderer_geometry_vertex_update(bgeometry* g, u32 offset, u32 vertex_count, void* vertices, b8 include_in_frame_workload)
{
    renderer_system_state* state_ptr = engine_systems_get()->renderer_system;
    // Load the data
...
void renderer_geometry_destroy(bgeometry* g)
{
    renderer_system_state* state_ptr = engine_systems_get()->renderer_system;
...
void renderer_clear_color_set(struct renderer_system_state* state, vec4 color)
{
    if (state)
        state->backend->clear_color_set(state->backend, color);
}
...
b8 renderer_clear_color(struct renderer_system_state* state, bhandle texture_handle)
{
    ...
    {
        state->backend->clear_color(state->backend, texture_handle);
        return true;
    }

    BERROR("renderer_clear_color requires a valid handle to a texture. Nothing was done");
    return false;
}
...
void renderer_color_texture_prepare_for_present(struct renderer_system_state* state, bhandle texture_handle)
{
    if (state && !bhandle_is_invalid(texture_handle))
    {
        state->backend->color_texture_prepare_for_present(state->backend, texture_handle);
        return;
    }
...
b8 renderer_shader_create(struct renderer_system_state* state, bhandle shader, const bresource_shader* shader_resource)
{
    return state->backend->shader_create(state->backend, shader, shader_resource);
}

void renderer_shader_destroy(struct renderer_system_state* state, bhandle shader)
{
    state->backend->shader_destroy(state->backend, shader);
}

b8 renderer_shader_reload(struct renderer_system_state* state, bhandle shader, u32 shader_stage_count, shader_stage_config* shader_stages)
{
    return state->backend->shader_reload(state->backend, shader, shader_stage_count, shader_stages);
}

b8 renderer_shader_use(struct renderer_system_state* state, bhandle shader)
{
    return state->backend->shader_use(state->backend, shader);
}

b8 renderer_shader_supports_wireframe(struct renderer_system_state* state, bhandle shader)
{
    return state->backend->shader_supports_wireframe(state->backend, shader);
}

b8 renderer_shader_flag_get(struct renderer_system_state* state, bhandle shader, shader_flags flag)
{
    return state->backend->shader_flag_get(state->backend, shader, flag);
}

void renderer_shader_flag_set(struct renderer_system_state* state, bhandle shader, shader_flags flag, b8 enabled)
{
    state->backend->shader_flag_set(state->backend, shader, flag, enabled);
}

b8 renderer_shader_bind_per_frame(struct renderer_system_state* state, bhandle shader)
{
    return state->backend->shader_bind_per_frame(state->backend, shader);
}

b8 renderer_shader_bind_per_group(struct renderer_system_state* state, bhandle shader, u32 group_id)
{
    return state->backend->shader_bind_per_group(state->backend, shader, group_id);
}

b8 renderer_shader_bind_per_draw(struct renderer_system_state* state, bhandle shader, u32 draw_id)
{
...
b8 renderer_shader_per_group_resources_release(struct renderer_system_state* state, bhandle shader, u32 group_id)
{
    return state->backend->shader_per_group_resources_release(state->backend, shader, group_id);
}

b8 renderer_shader_per_draw_resources_acquire(struct renderer_system_state* state, bhandle shader, u32* out_draw_id)
{
    return state->backend->shader_per_draw_resources_acquire(state->backend, shader, out_draw_id);
}

b8 renderer_shader_per_draw_resources_release(struct renderer_system_state* state, bhandle shader, u32 draw_id)
{
    return state->backend->shader_per_draw_resources_release(state->backend, shader, draw_id);
}

b8 renderer_shader_uniform_set(struct renderer_system_state* state, bhandle shader, shader_uniform* uniform, u32 array_index, const void* value)
{
    renderer_system_state* state_ptr = engine_systems_get()->renderer_system;
...
b8 renderer_renderbuffer_create(const char* name, renderbuffer_type type, u64 total_size, renderbuffer_track_type track_type, renderbuffer* out_buffer)
{
    ...
    bzero_memory(out_buffer, sizeof(renderbuffer));
...
void renderer_renderbuffer_destroy(renderbuffer* buffer)
{
    ...
    if (buffer)
    {
        ...
        if (buffer->name)
        {
            ...
            bfree(buffer->name, length + 1, MEMORY_TAG_STRING);
            buffer->name = 0;
        }
...
b8 renderer_renderbuffer_bind(renderbuffer* buffer, u64 offset)
{
    ...
    if (!buffer)
    {
        BERROR("renderer_renderbuffer_bind requires a valid pointer to a buffer");
        return false;
    }
...
b8 renderer_renderbuffer_resize(renderbuffer* buffer, u64 new_total_size)
{
    ...
    {
        BERROR("renderer_renderbuffer_resize requires that new size be larger than the old. Not doing this could lead to data loss");
        return false;
    }
    ...
    if (buffer->track_type == RENDERBUFFER_TRACK_TYPE_FREELIST)
    {
        ...
        void* new_block = ballocate(new_memory_requirement, MEMORY_TAG_RENDERER);
        void* old_block = 0;
        if (!freelist_resize(&buffer->buffer_freelist, &new_memory_requirement, new_block, new_total_size, &old_block))
        {
            BERROR("renderer_renderbuffer_resize failed to resize internal free list");
            bfree(new_block, new_memory_requirement, MEMORY_TAG_RENDERER);
            return false;
        }

        // Clean up old memory, then assign new properties over
        bfree(old_block, buffer->freelist_memory_requirement, MEMORY_TAG_RENDERER);
    ...
    if (result)
        ...
        BERROR("Failed to resize internal renderbuffer resources");
    return result;
}
...
b8 renderer_renderbuffer_allocate(renderbuffer* buffer, u64 size, u64* out_offset)
{
    ...
    {
        BERROR("renderer_renderbuffer_allocate requires valid buffer, a nonzero size and valid pointer to hold offset");
        return false;
    }
...
b8 renderer_renderbuffer_free(renderbuffer* buffer, u64 size, u64 offset)
{
    if (!buffer || !size)
    {
        BERROR("renderer_renderbuffer_free requires valid buffer and a nonzero size");
        return false;
    }
...
b8 renderer_renderbuffer_clear(renderbuffer* buffer, b8 zero_memory)
{
    if (!buffer)
    {
        BERROR("renderer_renderbuffer_clear requires valid buffer and a nonzero size");
        return false;
    }
