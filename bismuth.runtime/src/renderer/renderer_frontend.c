...
void renderer_begin_rendering(struct renderer_system_state* state, struct frame_data* p_frame_data, rect_2d render_area, u32 color_target_count, bhandle* color_targets, bhandle depth_stencil_target, u32 depth_stencil_layer)
{
    ...
    if (color_target_count)
    {
        if (color_target_count == 1)
        {
            ...
            {
                color_datas = &state->textures[color_targets[0].handle_index].data;
            }
        }
        else
        {
            ...
            for (u32 i = 0; i < color_target_count; ++i)
            {
                ...
                {
                    color_datas[i] = state->textures[color_targets[i].handle_index].data;
                }
            }
        }
    }
    ...
    state->backend->begin_rendering(state->backend, p_frame_data, render_area, color_target_count, color_datas, depth_data, depth_stencil_layer);
...
b8 renderer_shader_set_wireframe(struct renderer_system_state* state, shader* s, b8 wireframe_enabled)
{
    ...
    {
        // Not supported, don't enable
        BWARN("Shader does not support wireframe mode: '%s'", s->name);
        return false;
    }
...
shader_uniform* renderer_shader_uniform_get(shader* s, const char* name)
{
    ...
    {
        BERROR("Shader '%s' does not contain a uniform named '%s'", s->name, name);
        return false;
    }
