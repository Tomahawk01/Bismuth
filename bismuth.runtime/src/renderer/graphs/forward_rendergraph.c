...
b8 forward_rendergraph_create(const forward_rendergraph_config* config, forward_rendergraph* out_graph)
{
    ...
    // Add global sources
    if (!rendergraph_global_source_add(&out_graph->internal_graph, "colorbuffer", RENDERGRAPH_SOURCE_TYPE_RENDER_TARGET_COLOR, RENDERGRAPH_SOURCE_ORIGIN_GLOBAL))
    {
    ...
    RG_CHECK(rendergraph_pass_sink_add(&out_graph->internal_graph, "skybox", "colorbuffer"));
    RG_CHECK(rendergraph_pass_source_add(&out_graph->internal_graph, "skybox", "colorbuffer", RENDERGRAPH_SOURCE_TYPE_RENDER_TARGET_COLOR, RENDERGRAPH_SOURCE_ORIGIN_OTHER));
    RG_CHECK(rendergraph_pass_set_sink_linkage(&out_graph->internal_graph, "skybox", "colorbuffer", 0, "colorbuffer"));
    ...
    RG_CHECK(rendergraph_pass_sink_add(&out_graph->internal_graph, "scene", "colorbuffer"));
    ...
    RG_CHECK(rendergraph_pass_source_add(&out_graph->internal_graph, "scene", "colorbuffer", RENDERGRAPH_SOURCE_TYPE_RENDER_TARGET_COLOR, RENDERGRAPH_SOURCE_ORIGIN_OTHER));
    ...
    RG_CHECK(rendergraph_pass_set_sink_linkage(&out_graph->internal_graph, "scene", "colorbuffer", "skybox", "colorbuffer"));
...
b8 forward_rendergraph_initialize(forward_rendergraph* graph)
{
    ...
    {
        BERROR("Failed to load rendergraph resources");
        return false;
    }
...
b8 forward_rendergraph_frame_prepare(forward_rendergraph* graph, struct frame_data* p_frame_data, struct camera* current_camera, struct viewport* current_viewport, struct scene* scene, u32 render_mode)
{
    ...
    if (scene->state == SCENE_STATE_LOADED)
    {
        ...
        if (dir_light)
        {
            ...
            // Setup extended data for the pass
            ...
            for (u32 c = 0; c < MAX_CASCADE_COUNT; c++)
            {
                ...
                // "Pull" the min inward and "push" the max outward on the z axis to make sure shadow casters outside the view are captured as well (trees above the player)
                // TODO: This should be adjustable per scene
            ...
            // Track number of meshes drawn in the shadow pass
