...
b8 rendergraph_create(const char* config_str, struct bresource_texture* global_colorbuffer, struct bresource_texture* global_depthbuffer, rendergraph* out_graph)
{
    if (!out_graph)
        return false;
    ...
    if (!global_colorbuffer || !global_depthbuffer)
    {
    ...
    {
        BERROR("Failed to deserialize rendergraph config. See logs for details");
        return false;
    }
    ...
    out_graph->nodes = ballocate(sizeof(rendergraph_node) * out_graph->node_count, MEMORY_TAG_ARRAY);
    out_graph->execution_list = ballocate(sizeof(u32) * out_graph->node_count, MEMORY_TAG_ARRAY);
    for (u32 i = 0; i < out_graph->node_count; ++i)
    {
        ...
        if (!built)
        {
            BERROR("No registered factory found for rendergraph node type '%s'", node_config->type);
            return false;
        }
        ...
        if (strings_equali(node_config->type, "frame_begin"))
        {
            ...
            {
                BERROR("A node of type 'frame_begin' has already been assigned to this graph. Only one may exist");
                return false;
            }
        ...
        else if (strings_equali(node_config->type, "frame_end"))
        {
            ...
            {
                BERROR("A node of type 'frame_end' has already been assigned to this graph. Only one may exist");
                return false;
            }
    ...
    if (!out_graph->begin_node)
    {
        BERROR("Rendergraph is missing required node of type 'frame_begin'");
        return false;
    }
    if (!out_graph->end_node)
    {
        BERROR("Rendergraph is missing required node of type 'frame_end'");
        return false;
    }
...
void rendergraph_destroy(rendergraph* graph)
{
    if (graph)
    {
        ...
        bfree(graph->nodes, sizeof(graph->nodes[0]) * graph->node_count, MEMORY_TAG_ARRAY);
        bfree(graph->execution_list, sizeof(graph->execution_list[0]) * graph->node_count, MEMORY_TAG_ARRAY);

        bzero_memory(graph, sizeof(rendergraph));
    }
}
...
b8 rendergraph_finalize(rendergraph* graph)
{
    ...
    // the last to the first of the frame, making sure the colorbuffer makes it through the entire graph.
    if (!graph->end_node)
    {
        BERROR("End node is missing. Cannot continue");
        return false;
    }
    if (!graph->begin_node)
    {
        BERROR("Begin node is missing. Cannot continue");
        return false;
    }
...
b8 rendergraph_load_resources(rendergraph* graph)
{
    ...
    for (u32 i = 0; i < graph->node_count; ++i)
    {
        ...
        if (node->load_resources)
        {
            ...
            {
                BERROR("Failed to load resources for node '%s'", node->name);
                return false;
            }
        }
    }
...
b8 rendergraph_execute_frame(rendergraph* graph, frame_data* p_frame_data)
{
    ...
    for (u32 i = 0; i < graph->node_count; ++i)
    {
        ...
        {
            BERROR("Error executing rendergraph node. Check logs for additional details");
            return false;
        }
    }
...
rendergraph_resource_type string_to_resource_type(const char* str)
{
    ...
    {
        BERROR("string_to_resource_type requires a valid string. Returning undefined");
    ...
    BERROR("string_to_resource_type - unrecognized type '%s', returning undefined", str);
    return RENDERGRAPH_RESOURCE_TYPE_UNDEFINED;
}
...
b8 rendergraph_system_initialize(u64* memory_requirement, struct rendergraph_system_state* state)
{
    ...
    {
        BERROR("rendergraph_system_initialize requires a valid pointer to memory_requirement");
        return false;
    }
    ...
    *memory_requirement = sizeof(rendergraph_system_state);
    ...
    {
        BERROR("Failed to register known rendergraph factory type 'frame_begin'");
        return false;
    }
    if (!frame_end_rendergraph_node_register_factory())
    {
        BERROR("Failed to register known rendergraph factory type 'frame_end'");
        return false;
    }

    if (!clear_color_rendergraph_node_register_factory())
    {
        BERROR("Failed to register known rendergraph factory type 'clear_color'");
        return false;
    }
...
BAPI b8 rendergraph_system_node_factory_register(struct rendergraph_system_state* state, const rendergraph_node_factory* new_factory)
{
    ...
    {
        BERROR("rendergraph_system_node_factory_register requires valid pointers to state and a new_factory");
        return false;
    }
    ...
    for (u32 i = 0; i < factory_count; ++i)
    {
        ...
        // NOTE: Yes, a hashtable would likely be faster, but since this only happens on startup, it isn't really that big of a deal here
        if (strings_equali(factory->type, new_factory->type))
        {
            BDEBUG("A factory named '%s' already exists, so the one with this name will be overwritten. Note that names are case-insensitive", new_factory->type);
...
static b8 rendergraph_config_deserialize(const char* source_string, rendergraph_config* out_config)
{
    ...
    {
        BERROR("rendergraph_config_deserialize requires valid source_string and valid pointer to out_config");
        return false;
    }

    bson_tree tree;
    b8 result = bson_tree_from_string(source_string, &tree);
    if (!result)
    {
        BERROR("Failed to parse bson config for rendergraph. See logs for details");
        return false;
    }

    // graph name
    if (!bson_object_property_value_get_string(&tree.root, "name", &out_config->name))
    {
        BWARN("Missing name property. Assigning defualt name");
    ...
    if (bson_object_property_value_get_array(&tree.root, "nodes", &nodes))
    {
        ...
        {
            BERROR("Failed to get node count from nodes array");
            RG_DESERIALIZE_FAIL();
        }

        if (out_config->node_count > 0)
        {
            out_config->node_configs = ballocate(sizeof(rendergraph_node_config) * out_config->node_count, MEMORY_TAG_ARRAY);
            ...
            for (u32 i = 0; i < out_config->node_count; ++i)
            {
                ...
                bson_object node;
                if (!bson_array_element_value_get_object(&nodes, i, &node))
                {
                    BERROR("Failed to parse node at index %d", i);
                    RG_DESERIALIZE_FAIL();
                }

                // Name
                if (!bson_object_property_value_get_string(&node, "name", &node_config->name))
                {
                    BERROR("Missing node name property. This property is required");
                    RG_DESERIALIZE_FAIL();
                }

                // type
                if (!bson_object_property_value_get_string(&node, "type", &node_config->type))
                {
                    BERROR("Missing node type property. This property is required");
                    RG_DESERIALIZE_FAIL();
                }

                // config string
                bson_object config_block;
                if (bson_object_property_value_get_object(&node, "config", &config_block))
                {
                    ...
                    node_config->config_str = bson_tree_to_string(&config_tree);
                ...
                if (bson_object_property_value_get_array(&node, "sinks", &sinks_array))
                {
                    ...
                    if (!bson_array_element_count_get(&sinks_array, &node_config->sink_count))
                    {
                        BERROR("Failed to get node count from nodes array");
                        RG_DESERIALIZE_FAIL();
                    }
                    ...
                    {
                        node_config->sinks = ballocate(sizeof(rendergraph_node_sink_config) * node_config->sink_count, MEMORY_TAG_ARRAY);
                        ...
                        for (u32 s = 0; s < node_config->sink_count; ++s)
                        {
                            ...
                            bson_object sink;
                            if (!bson_array_element_value_get_object(&sinks_array, s, &sink))
                            {
                                BERROR("Failed to parse sink at index %d for node '%s'", s, node_config->name);
                                RG_DESERIALIZE_FAIL();
                            }

                            // name
                            if (!bson_object_property_value_get_string(&sink, "name", &sink_config->name))
                            {
                                BERROR("Missing required name field for sink at index %d for node '%s'", s, node_config->name);
                                RG_DESERIALIZE_FAIL();
                            }

                            // type
                            if (bson_object_property_value_get_string(&sink, "type", &sink_config->type))
                            {
                                // NOTE: this property is optional, as default sinks for nodes already have type defined
                            }

                            // source name
                            if (!bson_object_property_value_get_string(&sink, "source_name", &sink_config->source_name))
                            {
                                BERROR("Missing required source_name field for sink at index %d for node '%s'", s, node_config->name);
                                RG_DESERIALIZE_FAIL();
                            }
                        }
                    }
        ...
        if (config->node_count && config->node_configs)
        {
            for (u32 i = 0; i < config->node_count; ++i)
            {
                ...
                if (node->sink_count && node->sinks)
                {
                    ...
                    bfree(node->sinks, sizeof(node->sinks[0]), MEMORY_TAG_ARRAY);
                    node->sinks = 0;
                    node->sink_count = 0;
                }
            }
            bfree(config->node_configs, sizeof(config->node_configs[0]), MEMORY_TAG_ARRAY);
...
static rg_dep_graph* dep_graph_create(rendergraph* rg)
{
    rg_dep_graph* graph = ballocate(sizeof(rg_dep_graph), MEMORY_TAG_RENDERER);
    graph->node_count = rg->node_count;
    graph->nodes = ballocate(sizeof(rg_dep_node*) * rg->node_count, MEMORY_TAG_ARRAY);
    for (u32 i = 0; i < graph->node_count; ++i)
...
static void dep_graph_destroy(rg_dep_graph* graph)
{
    for (u32 i = 0; i < graph->node_count; ++i)
    {
        ...
        while (conn)
        {
            ...
            bfree(conn, sizeof(rg_node_connection), MEMORY_TAG_RENDERER);
            conn = next;
        }
        bfree(graph->nodes[i], sizeof(rg_dep_node), MEMORY_TAG_RENDERER);
    }
    bfree(graph->nodes, sizeof(rg_dep_node*) * graph->node_count, MEMORY_TAG_ARRAY);
    bfree(graph, sizeof(rg_dep_graph), MEMORY_TAG_RENDERER);
}

static rg_dep_node* dep_node_create(u32 index)
{
    rg_dep_node* node = ballocate(sizeof(rg_dep_node), MEMORY_TAG_RENDERER);
    node->index = index;
    node->outputs = 0;
    node->visited = 0;
    return node;
...
static void dep_node_connection_add(rg_dep_graph* dgraph, u32 from_index, u32 to_index)
{
    rg_node_connection* conn = ballocate(sizeof(rg_node_connection), MEMORY_TAG_RENDERER);
...
static b8 rg_dep_graph_topological_sort_recurse(rendergraph* graph, rg_dep_node* node, rg_dep_node** stack, i32* stack_index)
{
    if (node->visited == 1)
    {
        ...
        BERROR("Circular dependency detected");
        return false;
    }
    if (node->visited == 0)
    {
        ...
        while (conn != 0)
        {
            ...
            {
                BERROR("Connection causes circular dependency: '%s->%s'", graph->nodes[node->index].name, graph->nodes[conn->dest->index].name);
                return false;
            }
            conn = conn->next;
        }
...
static b8 rg_dep_graph_topological_sort(rendergraph* graph)
{
    rg_dep_node** stack = (rg_dep_node**)ballocate(sizeof(rg_dep_node*) * graph->node_count, MEMORY_TAG_ARRAY);
    i32 stack_index = 0;
    ...
    BDEBUG("Rengergraph will be executed in the following order:");
    for (u32 i = 0; i < graph->node_count; ++i)
    {
        rendergraph_node* node = &graph->nodes[i];
