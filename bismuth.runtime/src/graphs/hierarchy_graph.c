...
static b_handle node_acquire(hierarchy_graph* graph, u32 parent_index, b_handle xform_handle);
static void node_release(hierarchy_graph* graph, b_handle* node_handle, b8 release_transform);
...
static u32 hierarchy_graph_parent_index_get(const hierarchy_graph* graph, b_handle node_handle);
...
void hierarchy_graph_destroy(hierarchy_graph* graph)
{
    if (graph)
    {
        ...
        {
            bfree(graph->node_handles, sizeof(b_handle) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
        ...
        {
            bfree(graph->xform_handles, sizeof(b_handle) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
...
b_handle hierarchy_graph_xform_handle_get(const hierarchy_graph* graph, b_handle node_handle)
{
    return graph->xform_handles[node_handle.handle_index];
}

b_handle hierarchy_graph_parent_handle_get(const hierarchy_graph* graph, b_handle node_handle)
{
...
b_handle hierarchy_graph_parent_xform_handle_get(const hierarchy_graph* graph, b_handle node_handle)
{
    u32 parent_index = hierarchy_graph_parent_index_get(graph, node_handle);
    if (parent_index == INVALID_ID)
        return b_handle_invalid();
...
b_handle hierarchy_graph_root_add(hierarchy_graph* graph)
{
    return hierarchy_graph_child_add_with_xform(graph, b_handle_invalid(), b_handle_invalid());
}

b_handle hierarchy_graph_root_add_with_xform(hierarchy_graph* graph, b_handle xform_handle)
{
    return hierarchy_graph_child_add_with_xform(graph, b_handle_invalid(), xform_handle);
}

b_handle hierarchy_graph_child_add(hierarchy_graph* graph, b_handle parent_node_handle)
{
    return hierarchy_graph_child_add_with_xform(graph, parent_node_handle, b_handle_invalid());
}

b_handle hierarchy_graph_child_add_with_xform(hierarchy_graph* graph, b_handle parent_node_handle, b_handle xform_handle)
{
    return node_acquire(graph, parent_node_handle.handle_index, xform_handle);
}

void hierarchy_graph_node_remove(hierarchy_graph* graph, b_handle* node_handle, b8 release_transform)
{
    node_release(graph, node_handle, release_transform);
}

quat hierarchy_graph_world_rotation_get(const hierarchy_graph* graph, b_handle node_handle)
{
    BASSERT(graph);

    if (b_handle_is_invalid(node_handle))
    {
    ...
    while (parent_index != INVALID_ID)
    {
        ...
        b_handle xform_handle = graph->xform_handles[parent_index];
...
vec3 hierarchy_graph_world_position_get(const hierarchy_graph* graph, b_handle node_handle)
{
    BASSERT(graph);
...
vec3 hierarchy_graph_world_scale_get(const hierarchy_graph* graph, b_handle node_handle)
{
    BASSERT(graph);
...
static b_handle node_acquire(hierarchy_graph* graph, u32 parent_index, b_handle xform_handle)
{
    ...
    {
        if (b_handle_is_invalid(graph->node_handles[i]))
        {
            // Found a free slot. Setup the handle and id
            graph->node_handles[i] = b_handle_create(i);
    ...
    graph->node_handles[new_index] = b_handle_create(new_index);
    // If parent is INVALID_ID, then it is a root node. Otherwise, nest it below the parent in the hierarchy
...
static void node_release(hierarchy_graph* graph, b_handle* node_handle, b8 release_transform)
{
    ...
    else
    {
        ...
        else
        {
            ...
            b_handle_invalidate(&graph->xform_handles[node_handle->handle_index]);

            // Invalidate the node handle itself
            b_handle_invalidate(&graph->node_handles[node_handle->handle_index]);
            // Also invalidate the one passed in
            b_handle_invalidate(node_handle);
        }
    }
}
...
static void ensure_allocated(hierarchy_graph* graph, u32 new_node_count)
{
    ...
    {
        // Realloc all the arrays
        b_handle* new_node_handles = ballocate(sizeof(b_handle) * new_node_count, MEMORY_TAG_ARRAY);
        if (graph->node_handles)
        {
            bcopy_memory(new_node_handles, graph->node_handles, sizeof(b_handle) * graph->nodes_allocated);
            bfree(graph->node_handles, sizeof(b_handle) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
        }
        ...
        for (u32 node_handle_index = graph->nodes_allocated; node_handle_index < new_node_count; ++node_handle_index)
            graph->node_handles[node_handle_index] = b_handle_invalid();
        ...
        b_handle* new_xform_handles = ballocate(sizeof(b_handle) * new_node_count, MEMORY_TAG_ARRAY);
        if (graph->xform_handles)
        {
            bcopy_memory(new_xform_handles, graph->xform_handles, sizeof(b_handle) * graph->nodes_allocated);
            bfree(graph->xform_handles, sizeof(b_handle) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
        }
        ...
        for (u32 xform_handle_index = graph->nodes_allocated; xform_handle_index < new_node_count; ++xform_handle_index)
            graph->xform_handles[xform_handle_index] = b_handle_invalid();

        graph->nodes_allocated = new_node_count;
    }
}

static u32 hierarchy_node_create(hierarchy_graph_view* view, b_handle node_handle, b_handle xform_handle, u32 parent_index)
{
...
static void build_view_tree(hierarchy_graph* graph, hierarchy_graph_view* out_view)
{
    ...
    for (u32 i = 0; i < graph->nodes_allocated; ++i)
    {
        ...
        if (!b_handle_is_invalid(graph->node_handles[i]) && graph->parent_indices[i] == INVALID_ID)
        {
...
static void hierarchy_graph_update_tree_view_node(hierarchy_graph* graph, u32 node_index)
{
    ...
    if (b_handle_is_invalid(node->xform_handle))
        return;
    ...
    if (node->parent_index != INVALID_ID)
    {
        ...
        b_handle parent_xform_handle = parent->xform_handle;
        while (b_handle_is_invalid(parent_xform_handle))
        {
            ...
            {
                parent_xform_handle = b_handle_invalid();
                break;
            }
        }
        if (b_handle_is_invalid(parent_xform_handle))
        {
