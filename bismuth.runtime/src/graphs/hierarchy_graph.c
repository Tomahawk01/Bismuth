...
static bhandle node_acquire(hierarchy_graph* graph, u32 parent_index, bhandle xform_handle);
static void node_release(hierarchy_graph* graph, bhandle* node_handle, b8 release_transform);
...
static u32 hierarchy_graph_parent_index_get(const hierarchy_graph* graph, bhandle node_handle);

b8 hierarchy_graph_create(hierarchy_graph* out_graph)
{
    if (!out_graph)
    {
        BERROR("hierarchy_graph_create requires a valid pointer to hold the created graph");
        return false;
    }
...
void hierarchy_graph_destroy(hierarchy_graph* graph)
{
    if (graph)
    {
        ...
        {
            bfree(graph->node_handles, sizeof(bhandle) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
            graph->node_handles = 0;
        }

        if (graph->parent_indices)
        {
            bfree(graph->parent_indices, sizeof(u32) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
            graph->parent_indices = 0;
        }

        if (graph->levels)
        {
            bfree(graph->levels, sizeof(u8) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
            graph->levels = 0;
        }

        if (graph->dirty_flags)
        {
            bfree(graph->dirty_flags, sizeof(b8) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
            graph->dirty_flags = 0;
        }

        if (graph->xform_handles)
        {
            bfree(graph->xform_handles, sizeof(bhandle) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
...
bhandle hierarchy_graph_xform_handle_get(const hierarchy_graph* graph, bhandle node_handle)
{
    return graph->xform_handles[node_handle.handle_index];
}

b8 hierarchy_graph_xform_local_matrix_get(const hierarchy_graph* graph, bhandle node_handle, mat4* out_matrix)
{
    if (!graph || bhandle_is_invalid(node_handle) || !out_matrix)
        return false;

    bhandle xform_handle = graph->xform_handles[node_handle.handle_index];
...
bhandle hierarchy_graph_parent_handle_get(const hierarchy_graph* graph, bhandle node_handle)
{
...
bhandle hierarchy_graph_parent_xform_handle_get(const hierarchy_graph* graph, bhandle node_handle)
{
    u32 parent_index = hierarchy_graph_parent_index_get(graph, node_handle);
    if (parent_index == INVALID_ID)
        return bhandle_invalid();
...
bhandle hierarchy_graph_root_add(hierarchy_graph* graph)
{
    return hierarchy_graph_child_add_with_xform(graph, bhandle_invalid(), bhandle_invalid());
}

bhandle hierarchy_graph_root_add_with_xform(hierarchy_graph* graph, bhandle xform_handle)
{
    return hierarchy_graph_child_add_with_xform(graph, bhandle_invalid(), xform_handle);
}

bhandle hierarchy_graph_child_add(hierarchy_graph* graph, bhandle parent_node_handle)
{
    return hierarchy_graph_child_add_with_xform(graph, parent_node_handle, bhandle_invalid());
}

bhandle hierarchy_graph_child_add_with_xform(hierarchy_graph* graph, bhandle parent_node_handle, bhandle xform_handle)
{
    return node_acquire(graph, parent_node_handle.handle_index, xform_handle);
}

u32 hierarchy_graph_child_count_get(const hierarchy_graph* graph, bhandle parent_node_handle)
{
    if (!graph || bhandle_is_invalid(parent_node_handle))
        return 0;
...
b8 hierarchy_graph_child_get_by_index(const hierarchy_graph* graph, bhandle parent_node_handle, u32 index, bhandle* out_handle)
{
    if (!graph || bhandle_is_invalid(parent_node_handle))
        return 0;
...
void hierarchy_graph_node_remove(hierarchy_graph* graph, bhandle* node_handle, b8 release_transform)
{
    node_release(graph, node_handle, release_transform);
}

quat hierarchy_graph_world_rotation_get(const hierarchy_graph* graph, bhandle node_handle)
{
    BASSERT(graph);

    if (bhandle_is_invalid(node_handle))
    {
        BERROR("Invalid handle passed to get world rotation. Returning identity rotation");
    ...
    while (parent_index != INVALID_ID)
    {
        ...
        bhandle xform_handle = graph->xform_handles[parent_index];
    ...
    while (rot_stack.element_count)
    {
        ...
        {
            BERROR("Failed to pop from rotation stack. Result might be invalid");
        }
        else
        {
...
vec3 hierarchy_graph_world_position_get(const hierarchy_graph* graph, bhandle node_handle)
{
    BASSERT(graph);

    if (bhandle_is_invalid(node_handle))
    {
        BERROR("Invalid handle passed to get world position. Returning zero position");
        return vec3_zero();
    }
...
vec3 hierarchy_graph_world_scale_get(const hierarchy_graph* graph, bhandle node_handle)
{
    BASSERT(graph);

    if (bhandle_is_invalid(node_handle))
    {
        BERROR("Invalid handle passed to get world rotation. Returning one vector");
        return vec3_one();
    }
    ...
    while (scale_stack.element_count)
    {
        ...
        {
            BERROR("Failed to pop from scale stack. Result might be invalid");
        }
        else
        {
...
static bhandle node_acquire(hierarchy_graph* graph, u32 parent_index, bhandle xform_handle)
{
    ...
    for (u32 i = 0; i < graph->nodes_allocated; ++i)
    {
        if (bhandle_is_invalid(graph->node_handles[i]))
        {
            // Found a free slot. Setup the handle and id
            graph->node_handles[i] = bhandle_create(i);
            // If parent is INVALID_ID, then it is a root node. Otherwise, nest it below the parent in the hierarchy
    ...
    // Reaching this point means there is no more space in the table. Realloc everything and move to a larger list
    ...
    graph->node_handles[new_index] = bhandle_create(new_index);
    // If parent is INVALID_ID, then it is a root node. Otherwise, nest it below the parent in the hierarchy
...
static void node_release(hierarchy_graph* graph, bhandle* node_handle, b8 release_transform)
{
    ...
    if (bhandle_is_invalid(*node_handle))
    {
        BERROR("Tried to release a node using an invalid handle. Nothing was done");
    }
    else
    {
        if (node_handle->unique_id.uniqueid != graph->node_handles[node_handle->handle_index].unique_id.uniqueid)
        {
            BERROR("Tried to release a node using a stale handle. Nothing was done");
        }
        else
        {
            // The handle is valid and matching. Take any node that is a child of this node and move it up in the hierarchy.
            ...
            bhandle_invalidate(&graph->xform_handles[node_handle->handle_index]);

            // Invalidate the node handle itself
            bhandle_invalidate(&graph->node_handles[node_handle->handle_index]);
            // Also invalidate the one passed in
            bhandle_invalidate(node_handle);
        }
    }
}
...
static void ensure_allocated(hierarchy_graph* graph, u32 new_node_count)
{
    ...
    {
        // Realloc all the arrays
        bhandle* new_node_handles = ballocate(sizeof(bhandle) * new_node_count, MEMORY_TAG_ARRAY);
        if (graph->node_handles)
        {
            bcopy_memory(new_node_handles, graph->node_handles, sizeof(bhandle) * graph->nodes_allocated);
            bfree(graph->node_handles, sizeof(bhandle) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
        }
        ...
        for (u32 node_handle_index = graph->nodes_allocated; node_handle_index < new_node_count; ++node_handle_index)
            graph->node_handles[node_handle_index] = bhandle_invalid();

        u32* new_parent_indices = ballocate(sizeof(u32) * new_node_count, MEMORY_TAG_ARRAY);
        if (graph->parent_indices)
        {
            bcopy_memory(new_parent_indices, graph->parent_indices, sizeof(u32) * graph->nodes_allocated);
            bfree(graph->parent_indices, sizeof(u32) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
        }
        ...
        u8* new_levels = ballocate(sizeof(u8) * new_node_count, MEMORY_TAG_ARRAY);
        if (graph->levels)
        {
            bcopy_memory(new_levels, graph->levels, sizeof(u8) * graph->nodes_allocated);
            bfree(graph->levels, sizeof(u8) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
        }
        graph->levels = new_levels;

        b8* new_dirty_flags = ballocate(sizeof(b8) * new_node_count, MEMORY_TAG_ARRAY);
        if (graph->dirty_flags)
        {
            bcopy_memory(new_dirty_flags, graph->dirty_flags, sizeof(b8) * graph->nodes_allocated);
            bfree(graph->dirty_flags, sizeof(b8) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
        }
        graph->dirty_flags = new_dirty_flags;

        bhandle* new_xform_handles = ballocate(sizeof(bhandle) * new_node_count, MEMORY_TAG_ARRAY);
        if (graph->xform_handles)
        {
            bcopy_memory(new_xform_handles, graph->xform_handles, sizeof(bhandle) * graph->nodes_allocated);
            bfree(graph->xform_handles, sizeof(bhandle) * graph->nodes_allocated, MEMORY_TAG_ARRAY);
        }
        ...
        for (u32 xform_handle_index = graph->nodes_allocated; xform_handle_index < new_node_count; ++xform_handle_index)
            graph->xform_handles[xform_handle_index] = bhandle_invalid();

        graph->nodes_allocated = new_node_count;
    }
}

static u32 hierarchy_node_create(hierarchy_graph_view* view, bhandle node_handle, bhandle xform_handle, u32 parent_index)
{
...
static void build_view_tree(hierarchy_graph* graph, hierarchy_graph_view* out_view)
{
    ...
    for (u32 i = 0; i < graph->nodes_allocated; ++i)
    {
        ...
        if (!bhandle_is_invalid(graph->node_handles[i]) && graph->parent_indices[i] == INVALID_ID)
        {
...
static void hierarchy_graph_update_tree_view_node(hierarchy_graph* graph, u32 node_index)
{
    ...
    if (bhandle_is_invalid(node->xform_handle))
        return;
    ...
    if (node->parent_index != INVALID_ID)
    {
        ...
        bhandle parent_xform_handle = parent->xform_handle;
        while (bhandle_is_invalid(parent_xform_handle))
        {
            ...
            {
                parent_xform_handle = bhandle_invalid();
                break;
            }
        }
        if (bhandle_is_invalid(parent_xform_handle))
        {
