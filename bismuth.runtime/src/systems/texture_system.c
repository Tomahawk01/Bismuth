...
b8 texture_system_initialize(u64* memory_requirement, void* state, void* config)
{
    ...
    *memory_requirement = sizeof(texture_system_state);
    ...
    state_ptr->basset_system = engine_systems_get()->asset_state;
...
btexture texture_acquire_with_options(btexture_load_options options, void* listener, PFN_texture_loaded_callback callback)
{
    ...
    {
        BERROR("%s - Either name or image_asset_name is required", __FUNCTION__);
        return 0;
    }
    ...
    if (image_asset_names)
    {
        ...
        texture_asset_load_listener_context* context = BALLOC_TYPE(texture_asset_load_listener_context, MEMORY_TAG_TEXTURE);
        context->texture = t;
        ...
        for (u16 i = 0; i < array_size; ++i)
        {
            if (package_names[i])
            {
                context->assets[i] = asset_system_request_image_from_package(state_ptr->basset_system, image_asset_names[i], package_names[i], false, context, texture_basset_image_loaded);
            }
            else
            {
                context->assets[i] = asset_system_request_image(state_ptr->basset_system, image_asset_names[i], false, context, texture_basset_image_loaded);
            }
            if (!context->assets[i])
            {
                // NOTE: Continue to load other images instead of booting here
                BERROR("%s - Asset named '%s' does not exist, thus a texture cannot be loaded from it", __FUNCTION__, options.image_asset_name);
            }
        }

        // NOTE: Return pointer to texture here because all of the resource acquisition and pixel data upload must be done after all assets are loaded
        return t;
    }
...
btexture texture_acquire_with_options_sync(btexture_load_options options)
{
    ...
    if (image_asset_names)
    {
        ...
        for (u16 i = 0; i < state_ptr->array_sizes[t]; ++i)
        {
            if (package_names[i])
            {
                assets[i] = asset_system_request_image_from_package_sync(state_ptr->basset_system, image_asset_names[i], package_names[i], false);
            }
            else
            {
                assets[i] = asset_system_request_image_sync(state_ptr->basset_system, image_asset_names[i], false);
            }
            if (!assets[i])
            {
                // NOTE: Continue to load other images instead of booting here
    ...
    if (!texture_resources_acquire(t, name))
    {
        BERROR("%s - Failed to acquire renderer texture resources for texture '%s'", __FUNCTION__, options.image_asset_name);
        goto texture_acquire_with_options_sync_cleanup;
    }
...
texture_acquire_with_options_sync_cleanup:
    ...
    if (all_pixels && free_pixels)
        bfree(all_pixels, all_pixel_size, MEMORY_TAG_ARRAY);
...
static btexture texture_get_if_exists(bname name)
{
    ...
    if (node)
    {
        ...
        BTRACE("%s - Texture '%s' already exists - returning", __FUNCTION__, bname_string_get(name));
...
static b8 get_image_asset_names_from_options(const btexture_load_options* options, u16* out_count, const char*** image_asset_names, const char*** package_names)
{
    ...
    {
        // Multiple assets
        if (options->type == BTEXTURE_TYPE_2D_ARRAY)
        {
            count = options->layer_count;

            *image_asset_names = BALLOC_TYPE_CARRAY(const char*, count);
            *package_names = BALLOC_TYPE_CARRAY(const char*, count);
        ...
        else if (options->type == BTEXTURE_TYPE_CUBE_ARRAY)
        {
            // FIXME: Support for loading an array of cubemaps
            BERROR("%s - BTEXTURE_TYPE_CUBE_ARRAY not yet supported", __FUNCTION__);
            return false;
        }
        else
        {
            BFATAL("%s - 'layer_image_asset_names' should only be used with 'BTEXTURE_TYPE_2D_ARRAY'", __FUNCTION__);
            return false;
        }
    }
    ...
    {
        // Single asset
        if (options->type == BTEXTURE_TYPE_CUBE)
        {
        ...
        else if (options->type == BTEXTURE_TYPE_2D)
        {
        ...
        else
        {
            ...
            if (options->type == BTEXTURE_TYPE_2D_ARRAY || options->type == BTEXTURE_TYPE_CUBE_ARRAY)
            {
                BERROR("%s - 'image_asset_name' is only to be used with BTEXTURE_TYPE_2D and BTEXTURE_TYPE_CUBE. Use layer_image_asset_names instead");
                return false;
            }
        }
    }
...
static void combine_asset_pixel_data(basset_image** assets, u32 count, u32 expected_width, u32 expected_height, b8 release_assets, u32* out_size, void** out_pixels)
{
    ...
    *out_pixels = ballocate(*out_size, MEMORY_TAG_TEXTURE);
    ...
    {
        b8 mismatch = false;
        basset_image* asset = assets[i];
        if (!asset)
        {
            BERROR("No asset at index %u. This layer may not appear correctly", i);
        ...
        {
            BERROR("Layer pixel data size mismatch at index %u. Expected: %u, Actual: %u", i, layer_size, asset->pixel_array_size);
            goto acquire_with_options_sync_asset_continue;
        }
        ...
        bcopy_memory((*out_pixels) + offset, asset->pixels, asset->pixel_array_size);

    acquire_with_options_sync_asset_continue:
        if (release_assets)
        {
            // Release the asset here since it is no longer needed at this point
            asset_system_release_image(state_ptr->basset_system, asset);
            assets[i] = 0;
        }
...
static b8 texture_apply_asset_data(btexture t, bname name, const btexture_load_options* options, basset_image** assets)
{
    ...
    if (!write_result)
    {
        BERROR("%s - Failed to write texture data resource '%s'", __FUNCTION__, bname_string_get(name));
        goto texture_apply_asset_data_cleanup;
    }
