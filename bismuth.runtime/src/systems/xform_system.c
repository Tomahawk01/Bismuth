...
b8 xform_from_string(const char* str, bhandle* out_xform)
{
    if (!out_xform)
    {
        BERROR("string_to_scene_xform_config requires a valid pointer to out_xform");
        return false;
    }
    ...
    if (!str)
    {
        BWARN("Format error: invalid string provided. Identity transform will be used");
        result = false;
    }
    else
    {
    ...
    {
        BERROR("Attempted to create a xform before the system was initialized");
        *out_xform = bhandle_invalid();
        return false;
    }
