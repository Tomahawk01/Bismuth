...
b8 geometry_system_initialize(u64* memory_requirement, void* state, void* config)
{
    ...
    {
        BFATAL("geometry_system_initialize - typed_config->max_geometry_count must be > 0");
        return false;
    }
    ...
    *memory_requirement = struct_requirement + array_requirement;
    ...
    {
        BFATAL("Failed to create default geometries. Application cannot continue");
        return false;
    }
...
geometry* geometry_system_acquire_by_id(u32 id)
{
    ...
    BERROR("geometry_system_acquire_by_id cannot load invalid geometry id. Returning nullptr");
    return 0;
}
...
geometry* geometry_system_acquire_from_config(geometry_config config, b8 auto_release)
{
    ...
    if (!g)
    {
        BERROR("Unable to obtain free slot for geometry. Adjust configuration to allow more space. Returning nullptr");
        return 0;
    }

    if (!create_geometry(state_ptr, config, g))
    {
        BERROR("Failed to create geometry. Returning nullptr");
        return 0;
    }
...
void geometry_system_release(geometry* geometry)
{
    if (geometry && geometry->id != INVALID_ID)
    {
        ...
        {
            BFATAL("Geometry id mismatch. Check registration logic, as this should never occur");
        }
        return;
    }

    BWARN("geometry_system_release cannot release invalid geometry id. Nothing was done");
}
...
geometry* geometry_system_get_default(void)
{
    ...
    BFATAL("geometry_system_get_default called before system was initialized. Returning nullptr");
    return 0;
}
...
static b8 create_geometry(geometry_system_state* state, geometry_config config, geometry* g)
{
    if (!g)
    {
        BERROR("geometry_system->create_geometry requires a valid pointer to geometry");
        return false;
    }
    // Create geometry
    ...
    {
        BERROR("Geometry creation failed during renderer_geometry_create");
    ...
    {
        BERROR("Geometry creation failed during renderer_geometry_upload");
        // Invalidate entry
        state->registered_geometries[g->id].reference_count = 0;
        state->registered_geometries[g->id].auto_release = false;
        g->id = INVALID_ID;
...
{
    vertex_3d verts[4];
    bzero_memory(verts, sizeof(vertex_3d) * 4);
    ...
    {
        BFATAL("Failed to create default geometry. Application cannot continue");
        return false;
    }
    if (!renderer_geometry_upload(&state->default_geometry))
    {
        BFATAL("Failed to upload default geometry. Application cannot continue");
        return false;
    }
...
geometry_config geometry_system_generate_plane_config(f32 width, f32 height, u32 x_segment_count, u32 y_segment_count, f32 tile_x, f32 tile_y, bname name, bname material_name)
{
    if (width == 0)
    {
    ...
    config.vertices = ballocate(sizeof(vertex_3d) * config.vertex_count, MEMORY_TAG_ARRAY);
    config.index_size = sizeof(u32);
    config.index_count = x_segment_count * y_segment_count * 6;  // 6 indices per segment
    config.indices = ballocate(sizeof(u32) * config.index_count, MEMORY_TAG_ARRAY);

    // TODO: This generates extra vertices, deduplicate them later
    ...
    {
        config.name = bname_create(DEFAULT_GEOMETRY_NAME);
    }
    else
    {
        config.name = name;
    ...
    if (material_name == INVALID_BNAME)
    {
        config.material_name = bname_create(DEFAULT_PBR_MATERIAL_NAME);
    }
    else
    {
...
geometry_config geometry_system_generate_cube_config(f32 width, f32 height, f32 depth, f32 tile_x, f32 tile_y, bname name, bname material_name)
{
    if (width == 0)
    {
        BWARN("Width must be nonzero. Defaulting to one");
    ...
    bcopy_memory(config.vertices, verts, config.vertex_size * config.vertex_count);
