#ifndef _BISMUTH_APPLICATION_CONFIG_H_
#define _BISMUTH_APPLICATION_CONFIG_H_

#include <defines.h>
#include <math/math_types.h>

struct bwindow_config;
...
// Represents the top-level configuration for a core engine system
...
typedef struct application_config
{
    ...
    /** @brief darray of window configurations for the application */
    struct bwindow_config* windows;

    /** @brief darray of configurations for core engine systems */
    application_system_config* systems;

    /** @brief darray of rendergraph configurations */
...
/**
 * @brief Attempt to parse the application config file's content into the actual application config structure
 *
 * @param The file content of the application config (BSON format).
 * @param out_config A pointer to hold the application config structure.
 * @returns True on success; otherwise false.
 */
BAPI b8 application_config_parse_file_content(const char* file_content, application_config* out_config);
 ...
 * @returns True on success; otherwise false.
 */
BAPI b8 application_config_system_config_get(const application_config* config, const char* system_name, application_system_config* out_sys_config);
