...
b8 application_config_parse_file_content(const char* file_content, application_config* out_config)
{
    ...
    {
        BERROR("application_config_parse_file_content requires valid pointers to file_content and out_config");
        return false;
    }

    // Attempt to parse the file into a tree
    bson_tree app_config_tree = {0};
    if (!bson_tree_from_string(file_content, &app_config_tree))
    {
        BERROR("Failed to parse application config file content. See logs for details");
        return false;
    }

    // Extract required properties
    if (!bson_object_property_value_get_string(&app_config_tree.root, "app_name", &out_config->name))
    {
        BERROR("Failed to find property 'app_name', which is required");
        return false;
    }

    // FIXME: Move this to audio system config
    // if (!bson_object_property_value_get_string(&app_config_tree.root, "audio_plugin_name", &out_config->audio_plugin_name))
    // {
    //     BERROR("Failed to find property 'audio_plugin_name', which is required");
    //     return false;
    // }

    // frame_allocator_size is optional, so use a defualt if it isn't defined
    i64 frame_alloc_size = 0; // bson doesn't do unsigned ints, so convert it after
    if (!bson_object_property_value_get_int(&app_config_tree.root, "frame_allocator_size", &frame_alloc_size))
    ...
    if (!bson_object_property_value_get_int(&app_config_tree.root, "app_frame_data_size", &iapp_frame_data_size))
        out_config->app_frame_data_size = 0;
    else
        out_config->app_frame_data_size = (u64)iapp_frame_data_size;
    
    // Asset manifest file path
    if (!bson_object_property_value_get_string(&app_config_tree.root, "manifest_file_path", &out_config->manifest_file_path))
    {
        BERROR("'manifest_file_path' is a required field in application config. Cannot continue");
        return false;
    }
    ...
    if (bson_object_property_value_get_array(&app_config_tree.root, "windows", &window_configs_array))
    {
        ...
        if (!bson_array_element_count_get(&window_configs_array, &window_config_count))
            BERROR("Failed to get element count of 'windows' array. Using default config");

        for (u32 i = 0; i < window_config_count; ++i)
        {
            bson_object window_config = {0};
            if (!bson_array_element_value_get_object(&window_configs_array, i, &window_config))
            {
                BERROR("Failed to get window object at index %u. Continuing on and trying the next...", i);
                continue;
            }

            // Parse window properties. Nothing is technically required here, just use sane defaults for undefined options
            // Name
            bwindow_config new_window = {0};
            if (!bson_object_property_value_get_string(&window_config, "name", &new_window.name))
            {
                char name_buf[256] = {0};
            ...
            // Title
            if (!bson_object_property_value_get_string(&window_config, "title", &new_window.title))
            {
                char title_buf[256] = {0};
                string_format_unsafe(title_buf, "Bismuth Application Window %u", i);
                new_window.title = string_duplicate(title_buf);
            }

            // Resolution
            const char* res_str = 0;
            if (bson_object_property_value_get_string(&window_config, "resolution", &res_str))
            {
                vec2 resolution;
            ...
            // Starting position
            const char* sp_str = 0;
            if (bson_object_property_value_get_string(&window_config, "position", &sp_str))
            {
                vec2 start_position;
    ...
    if (window_count == 0)
    {
        BWARN("A window configuration was not provided or was not valid, so a default one will be used");
        bwindow_config win = {0};
        win.name = "main_window";
        win.title = "Bismuth Application Main Window";
        win.position_x = 100;
        win.position_y = 100;
        win.width = 1280;
        win.height = 720;
    ...
    if (!bson_array_element_count_get(&system_configs_array, &system_config_count))
    {
        BERROR("Failed to get element count of 'systems' array. This configuration is required");
        return false;
    }

    for (u32 i = 0; i < system_config_count; ++i)
    {
        bson_object system_config = {0};
        if (!bson_array_element_value_get_object(&system_configs_array, i, &system_config))
        {
            BERROR("Failed to get system config object at index %u. Continuing on and trying the next...", i);
            continue;
        }

        // Name
        application_system_config new_system = {0};
        if (!bson_object_property_value_get_string(&system_config, "name", &new_system.name))
        {
            BERROR("Required property 'name' is missing from system config. Cannot process system");
            continue;
        }

        // Obtain the 'config' property and set it up as a tree to re-serialize into a string
        bson_tree temp = {0};
        if (!bson_object_property_value_get_object(&system_config, "config", &temp.root))
        {
            BERROR("Required property 'config' is missing from system config. Cannot process system");
            continue;
        }

        new_system.configuration_str = bson_tree_to_string(&temp);
    ...
    if (!bson_array_element_count_get(&rendergraph_configs_array, &rendergraph_config_count))
    {
        BERROR("Failed to get element count of 'rendergraphs' array. This configuration is required");
        return false;
    }

    for (u32 i = 0; i < rendergraph_config_count; ++i)
    {
        bson_object rendergraph_config = {0};
        if (!bson_array_element_value_get_object(&rendergraph_configs_array, i, &rendergraph_config))
        {
            BERROR("Failed to get rendergraph config object at index %u. Continuing on and trying the next...", i);
            continue;
        }

        // Name
        application_rendergraph_config new_rendergraph = {0};
        if (!bson_object_property_value_get_string(&rendergraph_config, "name", &new_rendergraph.name))
        {
            BERROR("Required property 'name' is missing from rendergraph config. Cannot process system");
            continue;
        }
        ...
        new_rendergraph.configuration_str = bson_tree_to_string(&temp);

        // NOTE: No need to clean up the temp tree since it reuses objects already present in the main tree. This can/will be cleaned up at the end of processing
