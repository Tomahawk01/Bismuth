...
typedef struct engine_state_t
{
    ...
    bregistry external_systems_registry;

    bruntime_plugin* renderer_plugin;
    bruntime_plugin* audio_plugin;
...
static void engine_on_window_closed(const struct bwindow* window);
static void engine_on_window_resized(const struct bwindow* window);
...
b8 engine_create(application* game_inst)
{
    ...
    // Platform initialization first. NOTE: NOT window creation - that should happen much later
    {
        ...
        {
            BERROR("Failed to initialize platform layer");
            return false;
        }
    }
    ...
    // Console system
    {
        ...
        systems->console_system = ballocate(systems->console_memory_requirement, MEMORY_TAG_ENGINE);
        if (!console_initialize(&systems->console_memory_requirement, systems->console_system, 0))
        {
            BERROR("Failed to initialize console");
            return false;
        }
        ...
        {
            BFATAL("Unable to open '%s' for writing", log_filename);
            return false;
        }
    ...
    // Plugin system
    {
        ...
        {
            BERROR("Failed to deserialize plugin system config, which is required");
            return false;
        }

        plugin_system_intialize(&systems->plugin_system_memory_requirement, 0, &plugin_sys_config);
        systems->plugin_system = ballocate(systems->plugin_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!plugin_system_intialize(&systems->plugin_system_memory_requirement, systems->plugin_system, &plugin_sys_config))
        {
            BERROR("Failed to initialize plugin system");
            return false;
        }
    }
    ...
    // BVar system
    {
        bvar_system_initialize(&systems->bvar_system_memory_requirement, 0, 0);
        systems->bvar_system = ballocate(systems->bvar_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!bvar_system_initialize(&systems->bvar_system_memory_requirement, systems->bvar_system, 0))
        {
            BERROR("Failed to initialize BVar system");
            return false;
        }
    }
    ...
    // Event system
    {
        ...
        systems->event_system = ballocate(systems->event_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!event_system_initialize(&systems->event_system_memory_requirement, systems->event_system, 0))
        {
            BERROR("Failed to initialize event system");
            return false;
        }
    ...
    // Input system
    {
        ...
        systems->input_system = ballocate(systems->input_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!input_system_initialize(&systems->input_system_memory_requirement, systems->input_system, 0))
        {
            BERROR("Failed to initialize input system");
            return false;
        }
    ...
    // Resource system
    {
        ...
        resource_sys_config.asset_base_path = "../testbed.assets"; // TODO: The application should configure this
        ...
        systems->resource_system = ballocate(systems->resource_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!resource_system_initialize(&systems->resource_system_memory_requirement, systems->resource_system, &resource_sys_config))
        {
            BERROR("Failed to iniitialize resource system");
            return false;
        }
    }
    ...
    // Renderer system
    {
        ...
        {
            BERROR("Failed to deserialize renderer system config, which is required");
            return false;
        }

        renderer_system_initialize(&systems->renderer_system_memory_requirement, 0, &renderer_sys_config);
        systems->renderer_system = ballocate(systems->renderer_system_memory_requirement, MEMORY_TAG_ENGINE);
    ...
    // Job system
    {
        ...
        if (thread_count < 1)
        {
            BFATAL("Error: Platform reported processor count (minus one for main thread) as %i. Need at least one additional thread for the job system", thread_count);
            return false;
        }
        else
        {
        ...
        systems->job_system = ballocate(systems->job_system_memory_requirement, MEMORY_TAG_ENGINE);

        if (!job_system_initialize(&systems->job_system_memory_requirement, systems->job_system, &job_sys_config))
        {
            BERROR("Failed to initialize job system");
            return false;
        }
    }
    ...
    // xform
    {
        ...
        systems->xform_system = ballocate(systems->xform_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!xform_system_initialize(&systems->xform_system_memory_requirement, systems->xform_system, &xform_sys_config))
        {
            BERROR("Failed to intialize xform system");
            return false;
        }
    }
    ...
    // Timeline
    {
        ...
        systems->timeline_system = ballocate(systems->timeline_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!timeline_system_initialize(&systems->timeline_system_memory_requirement, systems->timeline_system, &timeline_config))
        {
            BERROR("Failed to initialize timeline system");
            return false;
        }
    }
    ...
    // Texture system
    {
        ...
        systems->texture_system = ballocate(systems->texture_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!texture_system_initialize(&systems->texture_system_memory_requirement, systems->texture_system, &texture_sys_config))
        {
            BERROR("Failed to initialize texture system");
            return false;
        }
    }
    ...
    // Static mesh system
    {
        ...
        systems->static_mesh_system = ballocate(systems->static_mesh_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!static_mesh_system_initialize(&systems->static_mesh_system_memory_requirement, systems->static_mesh_system))
        {
            BERROR("Failed to initialize geometry system");
            return false;
        }
    }
    ...
    // Light system
    {
        ...
        systems->light_system = ballocate(systems->light_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!light_system_initialize(&systems->light_system_memory_requirement, systems->light_system, 0))
        {
            BERROR("Failed to initialize light system");
            return false;
        }
    }
    ...
    // Camera system
    {
        ...
        systems->camera_system = ballocate(systems->camera_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!camera_system_initialize(&systems->camera_system_memory_requirement, systems->camera_system, &camera_sys_config))
        {
            BERROR("Failed to initialize camera system");
            return false;
        }
    }
    ...
    // Rendergraph system
    {
        ...
        systems->rendergraph_system = ballocate(systems->rendergraph_system_memory_requirement, MEMORY_TAG_ENGINE);
        if (!rendergraph_system_initialize(&systems->rendergraph_system_memory_requirement, systems->rendergraph_system))
        {
            BERROR("Failed to initialize rendergraph system");
            return false;
        }
    }
    ...
    {
        BERROR("Plugin(s) failed initialization. See logs for details");
        return false;
    }
...
b8 engine_run(application* game_inst)
{
    ...
    bwindow* w = &engine_state->windows[0];
...
struct bwindow* engine_active_window_get(void)
{
...
static void engine_on_window_resized(const struct bwindow* window)
{
    ...
    else
    {
        ...
        event_fire(EVENT_CODE_WINDOW_RESIZED, (bwindow*)window, context);
    }
}
