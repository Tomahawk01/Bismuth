...
b8 sui_panel_control_create(standard_ui_state* state, const char* name, vec2 size, vec4 color, struct sui_control* out_control)
{
...
b8 sui_panel_control_load(standard_ui_state* state, struct sui_control* self)
{
    ...
    // Create simple plane
    typed_data->g = geometry_generate_quad(typed_data->rect.width, typed_data->rect.height, xmin, xmax, ymin, ymax, bname_create(self->name));
    if (!renderer_geometry_upload(&typed_data->g))
    {
        BERROR("sui_panel_control_load - Failed to upload geometry quad");
        return false;
    }
