...
b8 debug_console_create(standard_ui_state* sui_state, debug_console_state* out_console_state)
{
    ...
    {
        if (!sui_label_control_create(sui_state, "debug_console_log_text", FONT_TYPE_SYSTEM, bname_create("Noto Sans CJK JP"), font_size, "", &out_console_state->text_control))
        {
        ...
        {
            BERROR("Unable to register console text label control");
            return false;
        }
        if (!standard_ui_system_control_add_child(sui_state, &out_console_state->bg_panel, &out_console_state->text_control))
        {
            BERROR("Failed to add background console text label as a child of the panel");
            return false;
        }
    ...
    {
        if (!sui_textbox_control_create(sui_state, "debug_console_entry_textbox", FONT_TYPE_SYSTEM, bname_create("Noto Sans CJK JP"), font_size, "", &out_console_state->entry_textbox))
        {
...
void debug_console_update(debug_console_state* state)
{
    if (state && state->loaded && state->dirty)
    {
        ...
        // This has a limit of DEBUG_CONSOLE_BUFFER_LENGTH, which should be more than enough anyway, but is clamped to avoid a buffer overflow
        ...
        bzero_memory(buffer, sizeof(char) * DEBUG_CONSOLE_BUFFER_LENGTH);
