...
b8 vulkan_renderer_shader_create(renderer_backend_interface* backend, shader* s, const shader_config* config)
{
    ...
    if (has_local)
    {
        ...
        for (u32 i = 0; i < s->local_uniform_sampler_count; ++i)
        {
            ...
            set_config->bindings[instance_binding_index].binding = instance_binding_index;
            set_config->bindings[instance_binding_index].descriptorCount = BMAX(u->array_length, 1); // Either treat as an array or a single texture, depending on what is passed in
            set_config->bindings[instance_binding_index].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
            set_config->bindings[instance_binding_index].stageFlags = VK_SHADER_STAGE_ALL;
            instance_binding_index++;
        }
    ...
    internal_shader->local_states = ballocate(sizeof(vulkan_shader_instance_state) * internal_shader->max_local_count, MEMORY_TAG_ARRAY);
...
void vulkan_renderer_shader_destroy(renderer_backend_interface* backend, shader* s)
{
    if (s && s->internal_data)
    {
        ...
        for (u32 i = 0; i < internal_shader->max_instances; ++i)
        {
            ...
            {
                bfree(instance->sampler_uniforms, sizeof(vulkan_uniform_sampler_state) * s->instance_uniform_sampler_count, MEMORY_TAG_ARRAY);
        ...
        for (u32 i = 0; i < internal_shader->max_local_count; ++i)
        {
            ...
            {
                bfree(local->sampler_uniforms, sizeof(vulkan_uniform_sampler_state) * s->instance_uniform_sampler_count, MEMORY_TAG_ARRAY);
                local->sampler_uniforms = 0;
            }
        }
        bfree(internal_shader->local_states, sizeof(vulkan_shader_instance_state) * internal_shader->max_local_count, MEMORY_TAG_ARRAY);
        ...
        if (sampler_count > 0)
        {
            ...
            for (u32 sb = 0; sb < sampler_count; ++sb)
            {
                ...
                for (u32 d = 0; d < binding_descriptor_count; ++d)
                {
                    ...
                    binding_image_infos[sb][d].sampler = context->samplers[map_internal_id];
...
b8 vulkan_renderer_shader_apply_local(renderer_backend_interface* backend, shader* s, u64 renderer_frame_number)
{
    ...
    if (s->local_uniform_sampler_count > 0)
    {
        ...
        // Determine the descriptor set index which will be first. If there are no globals and no instance uniforms, for example this will be 0
        // If there are globals but not instance, this will be 1, if there are both this will be 2
        b8 has_global = s->global_uniform_count > 0 || s->global_uniform_sampler_count > 0;
        b8 has_instance = s->instance_uniform_count > 0 || s->instance_uniform_sampler_count > 0;
        u32 descriptor_set_index = 0;
        ...
        {
            BERROR("Failed to update/bind local sampler descriptor");
            return false;
        }
    }
...
static b8 bresource_create_sampler(vulkan_context* context, bresource_texture_map* map, VkSampler* sampler)
{
...
b8 vulkan_renderer_bresource_texture_map_resources_acquire(renderer_backend_interface* backend, bresource_texture_map* map)
{
    ...
    if (!bresource_create_sampler(context, map, &context->samplers[selected_id]))
        return false;

#if _DEBUG
    char* formatted_name = string_format("%s_texmap_sampler", map->texture ? bname_string_get(map->texture->base.name) : "__noname__");
...
b8 vulkan_renderer_bresource_texture_map_resources_refresh(renderer_backend_interface* backend, bresource_texture_map* map)
{
    ...
    if (map && map->internal_id != INVALID_ID)
    {
        ...
        if (!bresource_create_sampler(context, map, &new_sampler))
            return false;
    ...
    *out_instance_id = INVALID_ID;
    ...
    if (s->instance_texture_count > 0)
    {
        ...
        for (u32 ii = 0; ii < s->instance_uniform_sampler_count; ++ii)
        {
            ...
            sampler_state->uniform_bresource_texture_maps = ballocate(sizeof(bresource_texture_map*) * array_length, MEMORY_TAG_ARRAY);
    ...
    // Per swapchain image
    instance_state->descriptor_sets = ballocate(sizeof(VkDescriptorSet) * image_count, MEMORY_TAG_ARRAY);
    VkDescriptorSetLayout* layouts = ballocate(sizeof(VkDescriptorSetLayout) * image_count, MEMORY_TAG_ARRAY);
    for (u32 i = 0; i < image_count; ++i)
...
b8 vulkan_renderer_shader_local_resources_acquire(renderer_backend_interface* backend, struct shader* s, const shader_instance_resource_config* config, u32* out_local_id)
{
    ...
    *out_local_id = INVALID_ID;
    ...
    {
        BERROR("vulkan_shader_acquire_local_resources failed to acquire new id for shader '%s', max local count=%u", s->name, internal->max_local_count);
        return false;
    }
    const bresource_texture* default_bresource_texture = texture_system_get_default_bresource_texture(engine_systems_get()->texture_system);
    // Map texture maps in the config to the correct uniforms
    ...
    {
        local_state->sampler_uniforms = ballocate(sizeof(vulkan_uniform_sampler_state) * s->local_uniform_sampler_count, MEMORY_TAG_ARRAY);
        // Assign uniforms to each of the sampler states
        for (u32 ii = 0; ii < s->local_uniform_sampler_count; ++ii)
        {
            ...
            for (u32 d = 0; d < array_length; ++d)
            {
                sampler_state->uniform_bresource_texture_maps[d] = tc->bresource_texture_maps[d];
                // Make sure it has a texture map assigned. Use default if not
                // FIXME: This check should be done by the texture system
                if (!sampler_state->uniform_bresource_texture_maps[d]->texture)
                    sampler_state->uniform_bresource_texture_maps[d]->texture = default_bresource_texture;
                sampler_state->descriptor_states[d].generations = ballocate(sizeof(u8) * image_count, MEMORY_TAG_ARRAY);
                sampler_state->descriptor_states[d].ids = ballocate(sizeof(u32) * image_count, MEMORY_TAG_ARRAY);
    ...
    // Per swapchain image
    local_state->descriptor_sets = ballocate(sizeof(VkDescriptorSet) * image_count, MEMORY_TAG_ARRAY);
    VkDescriptorSetLayout* layouts = ballocate(sizeof(VkDescriptorSetLayout) * image_count, MEMORY_TAG_ARRAY);
    for (u32 i = 0; i < image_count; ++i)
    ...
    if (result != VK_SUCCESS)
    {
        BERROR("Error allocating local descriptor sets in shader: '%s'", vulkan_result_string(result, true));
        return false;
    }
#ifdef _DEBUG
    for (u32 i = 0; i < image_count; ++i)
...
b8 vulkan_renderer_shader_instance_resources_release(renderer_backend_interface* backend, shader* s, u32 instance_id)
{
    ...
    if (result != VK_SUCCESS)
        BERROR("Error freeing instance shader descriptor sets");
    ...
    for (u32 a = 0; a < s->instance_uniform_sampler_count; ++a)
    {
        ...
        {
            bfree(sampler_state->uniform_bresource_texture_maps, sizeof(bresource_texture_map*) * array_length, MEMORY_TAG_ARRAY);
...
b8 vulkan_renderer_shader_local_resources_release(renderer_backend_interface* backend, shader* s, u32 local_id)
{
    ...
    if (result != VK_SUCCESS)
        BERROR("Error freeing local shader descriptor sets!");
...
b8 vulkan_renderer_uniform_set(renderer_backend_interface* backend, shader* s, shader_uniform* uniform, u32 array_index, const void* value)
{
    ...
    if (uniform_type_is_sampler(uniform->type))
    {
        ...
        case SHADER_SCOPE_INSTANCE:
            ...
            {
                BERROR("Trying to set an instance-level uniform without having bound an instance first");
                return false;
            }
            vulkan_shader_instance_state* instance_state = &internal->instance_states[s->bound_instance_id];
        ...
        case SHADER_SCOPE_LOCAL:
            ...
            {
                BERROR("Trying to set a local-level uniform without having bound an local id first");
                return false;
            }
            vulkan_shader_instance_state* local_state = &internal->local_states[s->bound_local_id];
    ...
    else
    {
        ...
        case SHADER_SCOPE_LOCAL:
            if (s->bound_local_id == INVALID_ID)
            {
                BERROR("An local id must be bound before setting a local uniform");
                return false;
            }
