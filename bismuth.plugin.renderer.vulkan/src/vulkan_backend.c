...
b8 vulkan_renderer_backend_initialize(renderer_backend_interface* backend, const renderer_backend_config* config)
{
    ...
    context->rhi = (brhi_vulkan){0};
    if (!vulkan_loader_initialize(&context->rhi))
    {
        BERROR("Failed to initialize Vulkan RHI. See logs for details");
        return false;
    }
    ...
    {
        BERROR("Failed to load core Vulkan functions. See logs for details");
        return false;
    }
    ...
    u32 api_version = 0;
    rhi->bvkEnumerateInstanceVersion(&api_version);
    ...
    rhi->bvkEnumerateInstanceExtensionProperties(0, &available_extension_count, 0);
    VkExtensionProperties* available_extensions = darray_reserve(VkExtensionProperties, available_extension_count);
    rhi->bvkEnumerateInstanceExtensionProperties(0, &available_extension_count, available_extensions);
    ...
    if (context->validation_enabled)
    {
        ...
        VK_CHECK(rhi->bvkEnumerateInstanceLayerProperties(&available_layer_count, 0));
        VkLayerProperties* available_layers = darray_reserve(VkLayerProperties, available_layer_count);
        VK_CHECK(rhi->bvkEnumerateInstanceLayerProperties(&available_layer_count, available_layers));
    ...
    VkResult instance_result = rhi->bvkCreateInstance(&create_info, context->allocator, &context->instance);
    ...
    {
        BERROR("Failed to load Vulkan instance functions. Renderer init failed");
        return false;
    }
    ...
    if (context->validation_enabled)
    {
        ...
        PFN_vkCreateDebugUtilsMessengerEXT func = (PFN_vkCreateDebugUtilsMessengerEXT)rhi->bvkGetInstanceProcAddr(context->instance, "vkCreateDebugUtilsMessengerEXT");
        ...
        context->pfnSetDebugUtilsObjectNameEXT = (PFN_vkSetDebugUtilsObjectNameEXT)rhi->bvkGetInstanceProcAddr(context->instance, "vkSetDebugUtilsObjectNameEXT");
        ...
        context->pfnSetDebugUtilsObjectTagEXT = (PFN_vkSetDebugUtilsObjectTagEXT)rhi->bvkGetInstanceProcAddr(context->instance, "vkSetDebugUtilsObjectTagEXT");
        ...
        context->pfnCmdBeginDebugUtilsLabelEXT = (PFN_vkCmdBeginDebugUtilsLabelEXT)rhi->bvkGetInstanceProcAddr(context->instance, "vkCmdBeginDebugUtilsLabelEXT");
        ...
        context->pfnCmdEndDebugUtilsLabelEXT = (PFN_vkCmdEndDebugUtilsLabelEXT)rhi->bvkGetInstanceProcAddr(context->instance, "vkCmdEndDebugUtilsLabelEXT");
...
void vulkan_renderer_backend_shutdown(renderer_backend_interface* backend)
{
    ...
    if (context->validation_enabled)
    {
        ...
        {
            PFN_vkDestroyDebugUtilsMessengerEXT func = (PFN_vkDestroyDebugUtilsMessengerEXT)rhi->bvkGetInstanceProcAddr(context->instance, "vkDestroyDebugUtilsMessengerEXT");
            func(context->instance, context->debug_messenger, context->allocator);
        }
    }

    BDEBUG("Destroying Vulkan instance...");
    rhi->bvkDestroyInstance(context->instance, context->allocator);
...
b8 vulkan_renderer_on_window_created(renderer_backend_interface* backend, bwindow* window)
{
    ...
    // Create per-frame-in-flight resources
    {
        ...
        for (u8 i = 0; i < window_backend->max_frames_in_flight; ++i)
        {
            ...
            rhi->bvkCreateSemaphore(context->device.logical_device, &semaphore_create_info, context->allocator, &window_backend->image_available_semaphores[i]);
            rhi->bvkCreateSemaphore(context->device.logical_device, &semaphore_create_info, context->allocator, &window_backend->queue_complete_semaphores[i]);
            ...
            VK_CHECK(rhi->bvkCreateFence(context->device.logical_device, &fence_create_info, context->allocator, &window_backend->in_flight_fences[i]));
...
void vulkan_renderer_on_window_destroyed(renderer_backend_interface* backend, bwindow* window)
{
    ...
    // Destroy per-frame-in-flight resources
    {
        for (u32 i = 0; i < window_backend->max_frames_in_flight; ++i)
        {
            ...
            {
                rhi->bvkDestroySemaphore(context->device.logical_device, window_backend->image_available_semaphores[i], context->allocator);
                window_backend->image_available_semaphores[i] = 0;
            }
            if (window_backend->queue_complete_semaphores[i])
            {
                rhi->bvkDestroySemaphore(context->device.logical_device, window_backend->queue_complete_semaphores[i], context->allocator);
                window_backend->queue_complete_semaphores[i] = 0;
            }

            rhi->bvkDestroyFence(context->device.logical_device, window_backend->in_flight_fences[i], context->allocator);
    ...
    {
        rhi->bvkDestroySurfaceKHR(context->instance, window_backend->surface, context->allocator);
...
b8 vulkan_renderer_frame_prepare_window_surface(renderer_backend_interface* backend, struct bwindow* window, struct frame_data* p_frame_data)
{
    ...
    {
        VkResult result = rhi->bvkDeviceWaitIdle(device->logical_device);
    ...
    VkResult result = rhi->bvkWaitForFences(context->device.logical_device, 1, &window_backend->in_flight_fences[window_backend->current_frame], true, U64_MAX);
    if (!vulkan_result_is_success(result))
    {
    ...
    result = rhi->bvkAcquireNextImageKHR(
    ...
    VK_CHECK(rhi->bvkResetFences(context->device.logical_device, 1, &window_backend->in_flight_fences[window_backend->current_frame]));
...
b8 vulkan_renderer_frame_command_list_end(renderer_backend_interface* backend, struct frame_data* p_frame_data)
{
    ...
    // Source is the window's color buffer texture
    vulkan_texture_handle_data* source_image_handle = &context->textures[context->current_window->renderer_state->colorbuffer->renderer_texture_handle.handle_index];
    vulkan_image* source_image = &source_image_handle->images[window_backend->image_index];
    // Target is the current swapchain image
    vulkan_texture_handle_data* target_image_handle = &context->textures[window_backend->swapchain.swapchain_color_texture->renderer_texture_handle.handle_index];
    vulkan_image* target_image = &target_image_handle->images[window_backend->swapchain.image_index];

    // Before ending the command buffer, blit the current color buffer's contents to the current swapchain image. Start by transitioning to transfer source layout
    {
        VK_BEGIN_DEBUG_LABEL(context, command_buffer->handle, "window color->transfer_src", vec4_one());
    ...
    // Transition the swapchain image to transfer destination layout
    {
        ...
        rhi->bvkCmdPipelineBarrier(
            command_buffer->handle,
            VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, // VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, // VK_PIPELINE_STAGE_TRANSFER_BIT,
    ...
    {
        VK_BEGIN_DEBUG_LABEL(context, command_buffer->handle, "window color->att. opt.", vec4_one());
        VkImageMemoryBarrier barrier = {};
        barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    ...
    // Transition target for presentation
    {
        ...
        rhi->bvkCmdPipelineBarrier(
            command_buffer->handle,
            VK_PIPELINE_STAGE_TRANSFER_BIT,
            VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, // After the image has been blitted
            0, 0, 0, 0, 0, 1, &barrier);
        VK_END_DEBUG_LABEL(context, command_buffer->handle);
    }
    ...
    // Increment (and wrap) the color buffer image index
...
b8 vulkan_renderer_frame_present(renderer_backend_interface* backend, struct bwindow* window, struct frame_data* p_frame_data)
{
    ...
    VkResult result = rhi->bvkQueuePresentKHR(context->device.present_queue, &present_info);
...
void vulkan_renderer_viewport_set(renderer_backend_interface* backend, vec4 rect)
{
    ...
    rhi->bvkCmdSetViewport(command_buffer->handle, 0, 1, &viewport);
}
...
void vulkan_renderer_scissor_set(renderer_backend_interface* backend, vec4 rect)
{
    ...
    rhi->bvkCmdSetScissor(command_buffer->handle, 0, 1, &scissor);
}
...
void vulkan_renderer_winding_set(struct renderer_backend_interface* backend, renderer_winding winding)
{
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT)
        rhi->bvkCmdSetFrontFace(command_buffer->handle, vk_winding);
...
void vulkan_renderer_cull_mode_set(struct renderer_backend_interface* backend, renderer_cull_mode cull_mode)
{
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT) {
        rhi->bvkCmdSetCullMode(command_buffer->handle, vulkan_cull_mode);
    } else if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_DYNAMIC_STATE_BIT) {
        context->vkCmdSetCullModeEXT(command_buffer->handle, vulkan_cull_mode);
    } else {
        BFATAL("renderer_cull_mode_set cannot be used on a device without dynamic state support");
    }
}
...
void vulkan_renderer_set_stencil_test_enabled(struct renderer_backend_interface* backend, b8 enabled)
{
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT)
        rhi->bvkCmdSetStencilTestEnable(command_buffer->handle, (VkBool32)enabled);
    else if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_DYNAMIC_STATE_BIT)
...
void vulkan_renderer_set_depth_test_enabled(struct renderer_backend_interface* backend, b8 enabled)
{
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT)
        rhi->bvkCmdSetDepthTestEnable(command_buffer->handle, (VkBool32)enabled);
    else if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_DYNAMIC_STATE_BIT)
...
void vulkan_renderer_set_depth_write_enabled(struct renderer_backend_interface* backend, b8 enabled)
{
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT)
        rhi->bvkCmdSetDepthWriteEnable(command_buffer->handle, (VkBool32)enabled);
    else if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_DYNAMIC_STATE_BIT)
...
void vulkan_renderer_set_stencil_reference(struct renderer_backend_interface* backend, u32 reference)
{
    ...
    rhi->bvkCmdSetStencilReference(command_buffer->handle, VK_STENCIL_FACE_FRONT_AND_BACK, reference);
}
...
void vulkan_renderer_begin_rendering(struct renderer_backend_interface* backend, frame_data* p_frame_data, rect_2d render_area, u32 color_target_count, bhandle* color_targets, bhandle depth_stencil_target, u32 depth_stencil_layer)
{
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT)
        rhi->bvkCmdBeginRendering(secondary->handle, &render_info);
    else
...
void vulkan_renderer_end_rendering(struct renderer_backend_interface* backend, frame_data* p_frame_data)
{
    ...
    // End rendering
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT)
        rhi->bvkCmdEndRendering(secondary->handle);
    else
...
void vulkan_renderer_set_stencil_compare_mask(struct renderer_backend_interface* backend, u32 compare_mask)
{
    ...
    rhi->bvkCmdSetStencilCompareMask(command_buffer->handle, VK_STENCIL_FACE_FRONT_AND_BACK, compare_mask);
}
...
void vulkan_renderer_set_stencil_write_mask(struct renderer_backend_interface* backend, u32 write_mask)
{
    ...
    // Supported as of vulkan 1.0, so no need to check for dynamic state support
    rhi->bvkCmdSetStencilWriteMask(command_buffer->handle, VK_STENCIL_FACE_FRONT_AND_BACK, write_mask);
}
...
void vulkan_renderer_clear_depth_stencil(renderer_backend_interface* backend, bhandle renderer_texture_handle)
{
    ...
    // Clear the image
    rhi->bvkCmdClearDepthStencilImage(
        command_buffer->handle,
        image->handle,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
...
void vulkan_renderer_color_texture_prepare_for_present(renderer_backend_interface* backend, bhandle renderer_texture_handle)
{
    ...
    VkImageMemoryBarrier barrier = {0};
    barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    barrier.oldLayout = VK_IMAGE_LAYOUT_UNDEFINED; // Throw away data
    barrier.newLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR;
    barrier.srcQueueFamilyIndex = context->device.graphics_queue_index;
    barrier.dstQueueFamilyIndex = context->device.graphics_queue_index;
    barrier.image = image->handle;
...
static i32 find_memory_index(vulkan_context* context, u32 type_filter, u32 property_flags)
{
    ...
    rhi->bvkGetPhysicalDeviceMemoryProperties(context->device.physical_device, &memory_properties);
...
b8 vulkan_renderer_texture_resources_acquire(renderer_backend_interface* backend, const char* name, texture_type type, u32 width, u32 height, u8 channel_count, u8 mip_levels, u16 array_size, texture_flag_bits flags, bhandle* out_renderer_texture_handle)
{
    vulkan_context* context = (vulkan_context*)backend->internal_context;
    ...
    if (bhandle_is_invalid(*out_renderer_texture_handle))
    {
    ...
    texture_data->images = BALLOC_TYPE_CARRAY(vulkan_image, texture_data->image_count);
    for (u32 i = 0; i < texture_data->image_count; ++i)
    {
...
void vulkan_renderer_texture_resources_release(renderer_backend_interface* backend, bhandle* renderer_texture_handle)
{
    ...
    *renderer_texture_handle = bhandle_invalid();
    ...
    for (u32 i = 0; i < texture_data->image_count; ++i)
        vulkan_image_destroy(context, &texture_data->images[i]);
    BFREE_TYPE_CARRAY(texture_data->images, vulkan_image, texture_data->image_count);
...
static void calculate_sorted_indices(vulkan_shader_frequency_info* frequency_info)
{
    ...
    frequency_info->sorted_indices = BALLOC_TYPE_CARRAY(u32, sampler_and_image_count);
    ...
    BASSERT_DEBUG(count == sampler_and_image_count);

    // Sort them
    bquick_sort(sizeof(u32), frequency_info->sorted_indices, 0, count - 1, bquicksort_compare_u32);
}
...
b8 vulkan_renderer_shader_create(renderer_backend_interface* backend, bhandle shader, const bresource_shader* shader_resource)
{
    ...
    bzero_memory(&internal_shader->per_group_info, sizeof(vulkan_shader_frequency_info));
    ...
    bzero_memory(&internal_shader->per_draw_info, sizeof(vulkan_shader_frequency_info));
    ...
    // NOTE: The Vulkan spec only guarantees 128 bytes of data. Therefore we align the "UBO" aka push constant stride to that, and only ever use one
    ...
    bzero_memory(internal_shader->descriptor_set_configs, sizeof(vulkan_descriptor_set_config) * VULKAN_SHADER_DESCRIPTOR_SET_LAYOUT_COUNT);
    ...
    // For now, shaders will only ever have these 2 types of descriptor pools. One is for unifrom buffers, and the other is for images and/or samplers
    ...
    VkResult result = rhi->bvkCreateDescriptorPool(logical_device, &pool_info, vk_allocator, &internal_shader->descriptor_pool);
    if (!vulkan_result_is_success(result))
    {
    ...
    char* desc_pool_name = string_format("desc_pool_shader_%s", bname_string_get(shader_resource->base.name));
    ...
    for (u32 i = 0; i < internal_shader->descriptor_set_count; ++i)
    {
        ...
        result = rhi->bvkCreateDescriptorSetLayout(logical_device, &layout_info, vk_allocator, &internal_shader->descriptor_set_layouts[i]);
        if (!vulkan_result_is_success(result))
        {
        ...
        if (internal_shader->pipelines[i])
        {
            ...
            for (u32 j = 1; j < PRIMITIVE_TOPOLOGY_TYPE_MAX_BIT; j = j << 1)
            {
                if (internal_shader->pipelines[i]->supported_topology_types & j)
                {
                    ...
                    // Break out here and just assume the first one for now. This can be overidden by whatever is using the shader if need be
                    break;
                }
            }
            pipeline_found = true;
    ...
    bzero_memory(internal_shader->mapped_uniform_buffer_blocks, sizeof(void*) * VULKAN_RESOURCE_IMAGE_COUNT);
    bzero_memory(internal_shader->uniform_buffers, sizeof(renderbuffer) * VULKAN_RESOURCE_IMAGE_COUNT);
...
void vulkan_renderer_shader_destroy(renderer_backend_interface* backend, bhandle shader)
{
    if (!bhandle_is_invalid(shader))
    {
        ...
        for (u32 i = 0; i < internal_shader->descriptor_set_count; ++i)
        {
            ...
            {
                BFREE_TYPE_CARRAY(set_config->bindings, VkDescriptorSetLayoutBinding, set_config->binding_count);
                set_config->bindings = 0;
            }
            if (internal_shader->descriptor_set_layouts[i])
            {
                rhi->bvkDestroyDescriptorSetLayout(logical_device, internal_shader->descriptor_set_layouts[i], vk_allocator);
                internal_shader->descriptor_set_layouts[i] = 0;
            }
        }

        // Global descriptor sets
        bzero_memory(internal_shader->per_frame_state.descriptor_sets, sizeof(VkDescriptorSet) * VULKAN_RESOURCE_IMAGE_COUNT);

        // Descriptor pool
        if (internal_shader->descriptor_pool)
        {
            rhi->bvkDestroyDescriptorPool(logical_device, internal_shader->descriptor_pool, vk_allocator);
        ...
        // Destroy frame state
        {
            ...
            bzero_memory(frequency_state, sizeof(vulkan_shader_frequency_state));
        }
        ...
        // Destroy the group states
        {
            ...
            if (internal_shader->group_states && internal_shader->max_groups)
                BFREE_TYPE_CARRAY(internal_shader->group_states, vulkan_shader_frequency_state, internal_shader->max_groups);
        ...
        // Destroy the per-draw states
        {
            ...
            if (internal_shader->per_draw_states && internal_shader->max_per_draw_count)
                BFREE_TYPE_CARRAY(internal_shader->per_draw_states, vulkan_shader_frequency_state, internal_shader->max_per_draw_count);
        ...
        for (u32 i = 0; i < internal_shader->stage_count; ++i)
            rhi->bvkDestroyShaderModule(context->device.logical_device, internal_shader->stages[i].handle, context->allocator);
...
b8 vulkan_renderer_shader_use(renderer_backend_interface* backend, bhandle shader)
{
    ...
    if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_NATIVE_DYNAMIC_STATE_BIT)
    {
        rhi->bvkCmdSetPrimitiveTopology(command_buffer->handle, internal_shader->current_topology);
    }
    else if (context->device.support_flags & VULKAN_DEVICE_SUPPORT_FLAG_DYNAMIC_STATE_BIT)
    {
...
b8 vulkan_renderer_shader_supports_wireframe(const renderer_backend_interface* backend, bhandle shader)
{
    vulkan_context* context = (vulkan_context*)backend->internal_context;
...
b8 vulkan_renderer_shader_apply_per_group(renderer_backend_interface* backend, bhandle shader, u16 renderer_frame_number)
{
    ...
    {
        BERROR("Cannot apply per-group uniforms without having first bound a group");
        return false;
    }
    ...
    // Determine the descriptor set index which will be first. If there are no per-frame uniforms, for example, this will be 0. If there are per-frame uniforms, this will be 1
    b8 has_per_frame = frequency_has_uniforms(&internal_shader->per_frame_info);
...
b8 vulkan_renderer_shader_apply_per_draw(renderer_backend_interface* backend, bhandle shader, u16 renderer_frame_number)
{
    ...
    if (frequency_info->bound_id == INVALID_ID)
    {
        BERROR("Cannot apply per-draw uniforms without having first bound a group");
        return false;
    }
    ...
    if (internal_shader->per_draw_info.uniform_sampler_count > 0)
    {
        ...
        if (!vulkan_descriptorset_update_and_bind(
                ...
                descriptor_set_index))
        {
            BERROR("Failed to update/bind per-draw sampler descriptor set");
            return false;
        }
    }
...
static b8 sampler_create_internal(vulkan_context* context, texture_filter filter, texture_repeat repeat, f32 anisotropy, vulkan_sampler_handle_data* out_sampler_handle_data)
{
    ...
    f32 actual_anisotropy = BMIN(anisotropy, context->device.properties.limits.maxSamplerAnisotropy);
    if (use_anisotropy)
    {
    ...
    VkResult result = rhi->bvkCreateSampler(context->device.logical_device, &sampler_info, context->allocator, &out_sampler_handle_data->sampler);
    ...
    VK_SET_DEBUG_OBJECT_NAME(context, VK_OBJECT_TYPE_SAMPLER, out_sampler_handle_data->sampler, bname_string_get(out_sampler_handle_data->name));
...
void vulkan_renderer_sampler_release(renderer_backend_interface* backend, bhandle* sampler)
{
    ...
    if (!bhandle_is_invalid(*sampler))
    {
        ...
        if (s->sampler && s->handle_uniqueid == sampler->unique_id.uniqueid)
        {
            ...
            rhi->bvkDeviceWaitIdle(context->device.logical_device);
            rhi->bvkDestroySampler(context->device.logical_device, s->sampler, context->allocator);
...
b8 vulkan_renderer_sampler_refresh(renderer_backend_interface* backend, bhandle* sampler, texture_filter filter, texture_repeat repeat, f32 anisotropy, u32 mip_levels)
{
    ...
    if (s->sampler && s->handle_uniqueid == sampler->unique_id.uniqueid)
    {
        ...
        // Destroy the old
        rhi->bvkDestroySampler(context->device.logical_device, old, context->allocator);
...
bname vulkan_renderer_sampler_name_get(renderer_backend_interface* backend, bhandle sampler)
{
    ...
    if (bhandle_is_invalid(sampler))
    {
        BERROR("Attempted to obtain a sampler name via an invalid handle");
        return INVALID_BNAME;
    }

    vulkan_sampler_handle_data* data = &context->samplers[sampler.handle_index];
    if (bhandle_is_stale(sampler, data->handle_uniqueid))
        BERROR("Attempted to obtain a sampler name via an stale handle");
...
b8 vulkan_renderer_shader_per_draw_resources_acquire(renderer_backend_interface* backend, bhandle shader, u32* out_per_draw_id)
{
    vulkan_context* context = (vulkan_context*)backend->internal_context;
...
b8 vulkan_renderer_shader_per_group_resources_release(renderer_backend_interface* backend, bhandle shader, u32 per_group_id)
{
    vulkan_context* context = (vulkan_context*)backend->internal_context;
...
b8 vulkan_renderer_shader_per_draw_resources_release(renderer_backend_interface* backend, bhandle shader, u32 per_draw_id)
{
    vulkan_context* context = (vulkan_context*)backend->internal_context;
...
b8 vulkan_renderer_shader_uniform_set(renderer_backend_interface* backend, bhandle shader, shader_uniform* uniform, u32 array_index, const void* value)
{
    vulkan_context* context = (vulkan_context*)backend->internal_context;
    vulkan_shader* internal = &context->shaders[shader.handle_index];
    ...
    case SHADER_UPDATE_FREQUENCY_PER_GROUP:
    {
        ...
        if (frequency_info->bound_id == INVALID_ID)
        {
            BERROR("Trying to set an per-group-level uniform without having bound a group first");
            return false;
        }
    ...
    case SHADER_UPDATE_FREQUENCY_PER_DRAW:
    {
        ...
        if (frequency_info->bound_id == INVALID_ID)
        {
            BERROR("Trying to set a per_draw-level uniform without having bound a draw id first");
            return false;
        }
    ...
    if (uniform_type_is_texture(uniform->type))
    {
        ...
        for (u32 i = 0; i < frequency_info->uniform_texture_count; ++i)
        {
            ...
            if (texture_state->uniform.tex_samp_index == uniform->tex_samp_index)
            {
                ...
                {
                    BERROR("vulkan_renderer_shader_uniform_set error: index (%u) is out of range (0-%u)", index, texture_state->uniform.array_length);
                    return false;
                }
        ...
        BERROR("texture_state_try_set: Unable to find uniform tex/samp_index %u. Sampler uniform not set", uniform->tex_samp_index);
        return false;
    }
    else if (uniform_type_is_sampler(uniform->type))
    {
        // TODO: Should be able to set a custom sampler by bhandle
...
static b8 create_shader_module(vulkan_context* context, vulkan_shader* internal_shader, shader_stage stage, const char* source, const char* filename, vulkan_shader_stage* out_stage)
{
    ...
    VK_CHECK(rhi->bvkCreateShaderModule(context->device.logical_device, &out_stage->create_info, context->allocator, &out_stage->handle));
...
b8 vulkan_buffer_create_internal(renderer_backend_interface* backend, renderbuffer* buffer)
{
    ...
    VK_CHECK(rhi->bvkCreateBuffer(context->device.logical_device, &buffer_info, context->allocator, &internal_buffer.handle));

    // Gather memory requirements
    rhi->bvkGetBufferMemoryRequirements(context->device.logical_device, internal_buffer.handle, &internal_buffer.memory_requirements);
    ...
    // Allocate the memory
    VkResult result = rhi->bvkAllocateMemory(context->device.logical_device, &allocate_info, context->allocator, &internal_buffer.memory);
    if (!vulkan_result_is_success(result))
    {
    ...
    VK_CHECK(rhi->bvkBindBufferMemory(
        context->device.logical_device,
...
void vulkan_buffer_destroy_internal(renderer_backend_interface* backend, renderbuffer* buffer)
{
    ...
    if (buffer)
    {
        ...
        if (internal_buffer)
        {
            ...
            {
                rhi->bvkFreeMemory(context->device.logical_device, internal_buffer->memory, context->allocator);
                internal_buffer->memory = 0;
            }
            if (internal_buffer->handle)
            {
                rhi->bvkDestroyBuffer(context->device.logical_device, internal_buffer->handle, context->allocator);
...
b8 vulkan_buffer_resize(renderer_backend_interface* backend, renderbuffer* buffer, u64 new_size)
{
    ...
    VkBuffer new_buffer;
    VK_CHECK(rhi->bvkCreateBuffer(context->device.logical_device, &buffer_info, context->allocator, &new_buffer));

    // Gather memory requirements
    VkMemoryRequirements requirements;
    rhi->bvkGetBufferMemoryRequirements(context->device.logical_device, new_buffer, &requirements);
    ...
    VkResult result = rhi->bvkAllocateMemory(context->device.logical_device, &allocate_info, context->allocator, &new_memory);
    if (result != VK_SUCCESS)
    {
    ...
    VK_CHECK(rhi->bvkBindBufferMemory(context->device.logical_device, new_buffer, new_memory, 0));
...
b8 vulkan_buffer_bind(renderer_backend_interface* backend, renderbuffer* buffer, u64 offset)
{
    ...
    if (buffer->type == RENDERBUFFER_TYPE_VERTEX)
    {
        ...
        rhi->bvkCmdBindVertexBuffers(command_buffer->handle, 0, 1, &internal_buffer->handle, offsets);
        return true;
    }
    else if (buffer->type == RENDERBUFFER_TYPE_INDEX)
    {
        // Bind index buffer at offset
        rhi->bvkCmdBindIndexBuffer(command_buffer->handle, internal_buffer->handle, offset, VK_INDEX_TYPE_UINT32);
        return true;
    }
    else
    {
        BWARN("Cannot bind buffer of type: %i", buffer->type);
        return false;
    }
...
void* vulkan_buffer_map_memory(renderer_backend_interface* backend, renderbuffer* buffer, u64 offset, u64 size)
{
    ...
    vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
    void* data;
    VK_CHECK(rhi->bvkMapMemory(context->device.logical_device, internal_buffer->memory, offset, size, 0, &data));
    return data;
}
...
void vulkan_buffer_unmap_memory(renderer_backend_interface* backend, renderbuffer* buffer, u64 offset, u64 size)
{
    ...
    vulkan_buffer* internal_buffer = (vulkan_buffer*)buffer->internal_data;
    rhi->bvkUnmapMemory(context->device.logical_device, internal_buffer->memory);
}
...
b8 vulkan_buffer_flush(renderer_backend_interface* backend, renderbuffer* buffer, u64 offset, u64 size)
{
    ...
    if (!vulkan_buffer_is_host_coherent(backend, internal_buffer))
    {
        ...
        range.offset = offset;
        range.size = size;
        VK_CHECK(rhi->bvkFlushMappedMemoryRanges(context->device.logical_device, 1, &range));
    }
...
b8 vulkan_buffer_read(renderer_backend_interface* backend, renderbuffer* buffer, u64 offset, u64 size, void** out_memory)
{
        ...
        !vulkan_buffer_is_host_visible(backend, internal_buffer))
    {
        ...
        // host visible but is device-local, create the read buffer, copy data to it, then read from that buffer
        ...
        // Map/copy/unmap
        void* mapped_data;
        VK_CHECK(rhi->bvkMapMemory(context->device.logical_device, read_internal->memory, 0, size, 0, &mapped_data));
        bcopy_memory(*out_memory, mapped_data, size);
        rhi->bvkUnmapMemory(context->device.logical_device, read_internal->memory);
    ...
    else
    {
        ...
        void* data_ptr;
        VK_CHECK(rhi->bvkMapMemory(context->device.logical_device, internal_buffer->memory, offset, size, 0, &data_ptr));
...
static b8 vulkan_buffer_copy_range_internal(vulkan_context* context, VkBuffer source, u64 source_offset, VkBuffer dest, u64 dest_offset, u64 size, b8 include_in_frame_workload)
{
    ...
    rhi->bvkCmdCopyBuffer(command_buffer->handle, source, dest, 1, &copy_region);
    ...
    else
    {
        ...
        rhi->bvkCmdPipelineBarrier(command_buffer->handle,
                             srcStage, dstStage,
...
b8 vulkan_buffer_draw(renderer_backend_interface* backend, renderbuffer* buffer, u64 offset, u32 element_count, b8 bind_only)
{
    ...
    {
        BERROR("Failed to bind renderbuffer. See logs for details");
        return false;
    }
    ...
    if (buffer->type == RENDERBUFFER_TYPE_VERTEX)
    {
        rhi->bvkCmdDraw(command_buffer->handle, element_count, 1, 0, 0);
    }
    else if (buffer->type == RENDERBUFFER_TYPE_INDEX)
    {
        rhi->bvkCmdDrawIndexed(command_buffer->handle, element_count, 1, 0, 0, 0);
    }
    else
    {
...
void vulkan_renderer_wait_for_idle(renderer_backend_interface* backend)
{
    if (backend)
    {
        vulkan_context* context = backend->internal_context;
        brhi_vulkan* rhi = &context->rhi;
        VK_CHECK(rhi->bvkDeviceWaitIdle(context->device.logical_device));
    }
}
...
static b8 vulkan_graphics_pipeline_create(vulkan_context* context, const vulkan_pipeline_config* config, vulkan_pipeline* out_pipeline)
{
    ...
    if (config->color_attachment_count)
    {
        ...
        color_blend_state_create_info.attachmentCount = config->color_attachment_count;
    ...
    VK_CHECK(rhi->bvkCreatePipelineLayout(
        context->device.logical_device,
    ...
    pipeline_create_info.pColorBlendState = config->color_attachment_count ? &color_blend_state_create_info : 0;
    ...
    VkResult result = rhi->bvkCreateGraphicsPipelines(
        context->device.logical_device,
        VK_NULL_HANDLE,
        1,
...
static void vulkan_pipeline_destroy(vulkan_context* context, vulkan_pipeline* pipeline)
{
    ...
    if (pipeline)
    {
        ...
        if (pipeline->handle)
        {
            rhi->bvkDestroyPipeline(context->device.logical_device, pipeline->handle, context->allocator);
            pipeline->handle = 0;
        }
        ...
        {
            rhi->bvkDestroyPipelineLayout(context->device.logical_device, pipeline->pipeline_layout, context->allocator);
...
{
    brhi_vulkan* rhi = &context->rhi;
    rhi->bvkCmdBindPipeline(command_buffer->handle, bind_point, pipeline->handle);
}
...
static b8 setup_frequency_state(renderer_backend_interface* backend, vulkan_shader* internal_shader, shader_update_frequency frequency, u32* out_frequency_id)
{
    ...
    const char* shader_name = bname_string_get(internal_shader->name);
    ...
    else
    {
        ...
        *out_frequency_id = get_new_frequency_id(frequency_states, max_frequency_count);
        ...
        {
            BERROR("setup_frequency_state failed to acquire new %s id for shader '%s', max %s count=%u", frequency_text, shader_name, frequency_text, max_frequency_count);
            return false;
        }
    ...
    {
        BERROR("per-frame frequency is somehow descriptor set index > 0");
    }
#endif
    ...
    {
        frequency_state->sampler_states = BALLOC_TYPE_CARRAY(vulkan_uniform_sampler_state, frequency_info->uniform_sampler_count);
        ...
        for (u32 ii = 0; ii < frequency_info->uniform_sampler_count; ++ii)
        {
            ...
            for (u32 d = 0; d < array_length; ++d)
            {
                ...
                bhandle default_sampler = renderer_generic_sampler_get(backend->frontend_state, SHADER_GENERIC_SAMPLER_LINEAR_REPEAT);
    ...
    {
        frequency_state->texture_states = BALLOC_TYPE_CARRAY(vulkan_uniform_texture_state, frequency_info->uniform_texture_count);
        ...
        for (u32 ii = 0; ii < frequency_info->uniform_texture_count; ++ii)
        {
            ...
            for (u32 d = 0; d < array_length; ++d)
            {
                ...
                texture_state->texture_handles[d] = renderer_default_texture_get(backend->frontend_state, RENDERER_DEFAULT_TEXTURE_BASE_COLOR);
    ...
    VkResult result = rhi->bvkAllocateDescriptorSets(context->device.logical_device, &alloc_info, frequency_state->descriptor_sets);
    if (result != VK_SUCCESS)
    {
...
static b8 release_shader_frequency_state(vulkan_context* context, vulkan_shader* internal_shader, shader_update_frequency frequency, u32 frequency_id)
{
    ...
    if (destroy_ubo)
    {
        ...
        if (frequency_info->ubo_stride != 0)
        {
            for (u32 i = 0; i < VULKAN_RESOURCE_IMAGE_COUNT; ++i)
            {
                ...
                {
                    BERROR("release_shader_frequency_state failed to free range from renderbuffer");
                }
            }
        }
    }
    ...
    // Descriptor sets
    VkResult result = rhi->bvkFreeDescriptorSets(context->device.logical_device, internal_shader->descriptor_pool, VULKAN_RESOURCE_IMAGE_COUNT, frequency_state->descriptor_sets);
    if (result != VK_SUCCESS)
        BERROR("Error freeing %s shader descriptor sets!", shader_update_frequency_to_string(frequency));
    ...
    if (frequency_state->sampler_states)
    {
        ...
        BFREE_TYPE_CARRAY(frequency_state->sampler_states, vulkan_uniform_sampler_state, frequency_info->uniform_sampler_count);
    ...
    if (frequency_state->texture_states)
    {
        ...
        BFREE_TYPE_CARRAY(frequency_state->texture_states, vulkan_uniform_texture_state, frequency_info->uniform_texture_count);
...
static void destroy_shader_frequency_states(shader_update_frequency frequency, vulkan_shader_frequency_state* states, u32 state_count, vulkan_shader_frequency_info* info)
{
    ...
    for (u32 i = 0; i < state_count; ++i)
    {
        ...
        bzero_memory(frequency_state->descriptor_sets, sizeof(VkDescriptorSet) * VULKAN_RESOURCE_IMAGE_COUNT);
        if (frequency_state->sampler_states)
        {
            bfree(frequency_state->sampler_states, sizeof(vulkan_uniform_sampler_state) * info->uniform_sampler_count, MEMORY_TAG_ARRAY);
            frequency_state->sampler_states = 0;
        }
        if (frequency_state->texture_states)
        {
            bfree(frequency_state->texture_states, sizeof(vulkan_uniform_texture_state) * info->uniform_texture_count, MEMORY_TAG_ARRAY);
            frequency_state->texture_states = 0;
        }
    }
}
...
static b8 shader_create_modules_and_pipelines(renderer_backend_interface* backend, vulkan_shader* internal_shader, u8 stage_count, shader_stage_config* stage_configs)
{
    ...
    for (u32 i = 0; i < pipeline_count; ++i)
    {
        ...
        if ((internal_shader->flags & SHADER_FLAG_COLOR_READ_BIT) || (internal_shader->flags & SHADER_FLAG_COLOR_WRITE_BIT))
        {
            ...
            pipeline_config.color_attachment_formats = &color_attachment_format;
        }
        else
        {
        ...
        if (!pipeline_result)
        {
            BERROR("Failed to load graphics pipeline for shader: '%s'", bname_string_get(internal_shader->name));
            has_error = true;
            break;
        }
    }
    ...
    if (has_error)
    {
        ...
        for (u32 i = 0; i < internal_shader->stage_count; ++i)
        {
            rhi->bvkDestroyShaderModule(context->device.logical_device, new_stages[i].handle, context->allocator);
        }
        goto shader_module_pipeline_cleanup;
    }
...
static void setup_frequency_descriptors(b8 do_ubo, vulkan_shader_frequency_info* frequency_info, vulkan_descriptor_set_config* set_config, const bresource_shader* config)
{
    ...
    if (!set_config->binding_count)
        return;
    set_config->bindings = BALLOC_TYPE_CARRAY(VkDescriptorSetLayoutBinding, set_config->binding_count);
    ...
    if (sampler_and_image_count)
    {
        ...
        for (u32 i = 0; i < sampler_and_image_count; ++i)
        {
            ...
            set_config->bindings[frequency_binding_index].binding = frequency_binding_index;
            set_config->bindings[frequency_binding_index].descriptorCount = BMAX(u->array_length, 1); // Either treat as an array or a single texture, depending on what is passed in
    ...
    u32 descriptor_set_index)
{
    ...
    bzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * max_desc_write_count);
    ...
    if (info->uniform_count > 0)
    {
        ...
        if ((*freq_gen) == INVALID_ID_U16 || (*freq_gen) != renderer_frame_number)
        {
            ...
            BASSERT_MSG((frequency_state->offset % context->device.properties.limits.minUniformBufferOffsetAlignment) == 0, "Ubo offset must be a multiple of device.properties.limits.minUniformBufferOffsetAlignment");
    ...
    if (sampler_and_image_count)
    {
        ...
        for (u32 i = 0; i < sampler_and_image_count; ++i)
        {
            ...
            {
                bhandle resource_handle = bhandle_invalid();
                ...
                if (is_texture)
                {
                    ...
                    {
                        BERROR("Invalid texture handle found while trying to update/bind descriptor set");
                        return false;
                    }
                ...
                else
                {
                    ...
                    if (bhandle_is_invalid(resource_handle))
                    {
                        BERROR("Invalid sampler handle found while trying to update/bind descriptor set");
                        return false;
                    }
    ...
    if (descriptor_write_count > 0)
        rhi->bvkUpdateDescriptorSets(context->device.logical_device, descriptor_write_count, descriptor_writes, 0, 0);
    ...
    rhi->bvkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
