...
#define RHI_FUNCTION(name) rhi->b##name = (PFN_##name)platform_dynamic_library_load_function(#name, &rhi->vulkan_lib)
#define RHI_INSTANCE_FUNCTION(name) rhi->b##name = (PFN_##name)rhi->bvkGetInstanceProcAddr(rhi->instance, #name)
#define RHI_DEVICE_FUNCTION(name) rhi->b##name = (PFN_##name)rhi->bvkGetDeviceProcAddr(rhi->device, #name)
...
b8 vulkan_loader_load_core(brhi_vulkan* rhi)
{
    ...
    return rhi->bvkGetInstanceProcAddr != 0;
}
