...
// A lookup table of vulkan image view types indexed Bismuth's texture types
...
STATIC_ASSERT(BRESOURCE_TEXTURE_TYPE_COUNT == (sizeof(vulkan_view_types) / sizeof(*vulkan_view_types)), "Texture type count does not match Vulkan image view lookup table count");
    ...
    vulkan_image* out_image)
{
    ...
    // Creation info
    bzero_memory(&out_image->image_create_info, sizeof(VkImageCreateInfo));
    out_image->image_create_info.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    switch (type)
    {
        default:
        case BRESOURCE_TEXTURE_TYPE_2D:
        case BRESOURCE_TEXTURE_TYPE_CUBE:         // Intentional, there is no cube image type
        case BRESOURCE_TEXTURE_TYPE_2D_ARRAY:     // Intentional, there is no 2d_array image type
    ...
    if (type == BRESOURCE_TEXTURE_TYPE_CUBE)
    ...
    if (!vulkan_result_is_success(allocate_result))
    {
        ...
        BERROR("Failed to allocate memory for image with the following error: '%s'", err_str);
        return;
    }
    ...
    ballocate_report(out_image->memory_requirements.size, is_device_memory ? MEMORY_TAG_GPU_LOCAL : MEMORY_TAG_VULKAN);
    ...
    if (create_view)
    {
        ...
        if (layer_count > 1)
        {
            ...
            out_image->layer_views = ballocate(sizeof(VkImageView) * layer_count, MEMORY_TAG_ARRAY);
            out_image->layer_view_subresource_ranges = ballocate(sizeof(VkImageSubresourceRange) * layer_count, MEMORY_TAG_ARRAY);
            out_image->layer_view_create_infos = ballocate(sizeof(VkImageCreateInfo) * layer_count, MEMORY_TAG_ARRAY);
            bresource_texture_type view_type = type;
            if (type == BRESOURCE_TEXTURE_TYPE_CUBE || type == BRESOURCE_TEXTURE_TYPE_CUBE_ARRAY)
            {
                // NOTE: for individual sampling of cubemap/cubemap array layers, the view type needs to be 2d
                view_type = BRESOURCE_TEXTURE_TYPE_2D;
            }
            if (type == BRESOURCE_TEXTURE_TYPE_2D_ARRAY)
            {
...
void vulkan_image_destroy(vulkan_context* context, vulkan_image* image)
{
    ...
    {
        bfree(image->layer_view_subresource_ranges, sizeof(VkImageSubresourceRange) * image->layer_count, MEMORY_TAG_ARRAY);
        image->layer_view_subresource_ranges = 0;
    }
    if (image->layer_view_create_infos)
    {
        bfree(image->layer_view_create_infos, sizeof(VkImageCreateInfo) * image->layer_count, MEMORY_TAG_ARRAY);
    ...
    if (image->name)
    {
        bfree(image->name, string_length(image->name) + 1, MEMORY_TAG_STRING);
        image->name = 0;
    }
    ...
    bzero_memory(&image->memory_requirements, sizeof(VkMemoryRequirements));
}
    ...
    VkImageLayout new_layout)
{
    ...
    {
        // Transitioning from transfer source layout to shader-readonly layout
    ...
    else if (old_layout == VK_IMAGE_LAYOUT_UNDEFINED && new_layout == VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL)
    {
        ...
        // Don't care what stage pipeline is in at the start
        source_stage = VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT;
...
b8 vulkan_image_mipmaps_generate(vulkan_context* context, vulkan_image* image, vulkan_command_buffer* command_buffer)
{
    ...
    {
        BWARN("Texture image format does not support linear blitting! Mipmaps cannot be created");
        return false;
    }

    // Same barrier can be used for all mip levels with some modifications for each one
    VkImageMemoryBarrier barrier = {0};
    barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    barrier.image = image->handle;
    barrier.srcQueueFamilyIndex = context->device.graphics_queue_index;
    ...
    for (u32 i = 1; i < image->mip_levels; ++i)
    {
        ...
        // Destination extents are now half the width/height of the previous, unless that previous was already 1
        blit.dstOffsets[1] = (VkOffset3D){mip_width > 1 ? mip_width / 2 : 1, mip_height > 1 ? mip_height / 2 : 1, 1};
        blit.dstSubresource.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
        // Destination is the current mip level
        ...
        // Split width and height in half for the next level, if there is one
        if (mip_width > 1)
            mip_width /= 2;
        if (mip_height > 1)
            mip_height /= 2;
    ...
    // Transition last mipmap level to a shader-readable layout
    // This would not have been handled in the above loop since that always transitions the previous layer
