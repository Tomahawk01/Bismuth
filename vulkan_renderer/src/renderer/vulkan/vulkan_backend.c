...
b8 vulkan_descriptorset_update_and_bind(renderer_plugin* plugin, frame_data* p_frame_data, shader* s, VkDescriptorSet descriptor_set,
                                        u32 descriptor_set_index, vulkan_descriptor_state* descriptor_state, u64 ubo_offset, u64 ubo_stride,
                                        u32 uniform_count, vulkan_uniform_sampler_state* samplers, u32 sampler_count, b8 needs_update)
{
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
                ...
                for (u32 d = 0; d < binding_descriptor_count; ++d)
                {
                    ...
                    if (t->generation == INVALID_ID)
                    {
                        ...
                        if (!texture_system_is_default_texture(t))
                        {
                            ...
                            {
                                BERROR("Vulkan renderer failed to determine texture type while applying sampler. Falling back to 2D");
                                t = texture_system_get_default_texture();
                            }
                        }
...
b8 vulkan_renderer_shader_apply_globals(renderer_plugin* plugin, shader* s, b8 needs_update, struct frame_data* p_frame_data)
{
    ...
    if (!vulkan_descriptorset_update_and_bind(
            ...
            needs_update))
    {
        BERROR("Failed to update/bind global descriptor");
        return false;
    }
...
b8 vulkan_renderer_shader_apply_instance(renderer_plugin* plugin, shader* s, b8 needs_update, frame_data* p_frame_data)
{
    ...
    // Determine the descriptor set index which will be first. If there are no globals, this will be 0. If there are globals, will be 1
    b8 has_global = s->global_uniform_count > 0 || s->global_uniform_sampler_count > 0;
    ...
    if (!vulkan_descriptorset_update_and_bind(
            ...
            needs_update))
    {
        BERROR("Failed to update/bind instance descriptor");
        return false;
    }
