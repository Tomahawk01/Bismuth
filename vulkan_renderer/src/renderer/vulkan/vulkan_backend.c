...
b8 vulkan_renderer_backend_initialize(renderer_plugin* plugin, const renderer_backend_config* config, u8* out_window_render_target_count)
{
    ...
    plugin->internal_context = ballocate(plugin->internal_context_size, MEMORY_TAG_RENDERER);
    ...
    context->allocator = 0;
#endif
    // Set some default values for framebuffer. Needed for swapchain creation
...
void vulkan_renderer_backend_shutdown(renderer_plugin* plugin)
{
    ...
    BDEBUG("Destroying Vulkan surface...");
    if (context->surface)
    {
    ...
    {
        bfree(plugin->internal_context, plugin->internal_context_size, MEMORY_TAG_RENDERER);
...
void vulkan_renderer_begin_debug_label(renderer_plugin* plugin, const char* label_text, vec3 color)
{
#ifdef _DEBUG
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
...
b8 vulkan_renderer_renderpass_begin(renderer_plugin* plugin, renderpass* pass, render_target* target)
{
    ...
    b8 do_clear_color = (pass->clear_flags & RENDERPASS_CLEAR_COLOR_BUFFER_FLAG) != 0;
    if (do_clear_color)
    {
        bcopy_memory(clear_values[begin_info.clearValueCount].color.float32, pass->clear_color.elements, sizeof(f32) * 4);
        begin_info.clearValueCount++;
    }
    else
    {
        if (target->attachments[0].type == RENDER_TARGET_ATTACHMENT_TYPE_COLOR)
            begin_info.clearValueCount++;
    }
    ...
    else
    {
        for (u32 i = 0; i < target->attachment_count; ++i)
        {
            ...
            {
                // If there is depth/stencil attachment make sure to add the clear count but don't copying data
                begin_info.clearValueCount++;
            }
        }
    }
...
#ifdef _DEBUG
    vec3 rgba = (vec3){bfrandom_in_range(0.0f, 1.0f), bfrandom_in_range(0.0f, 1.0f), bfrandom_in_range(0.0f, 1.0f)};
...
void vulkan_renderer_texture_create(renderer_plugin* plugin, const u8* pixels, texture* t)
{
    ...
    u32 size = t->width * t->height * t->channel_count * (t->type == TEXTURE_TYPE_CUBE ? 6 : t->array_size);
...
void vulkan_renderer_texture_destroy(renderer_plugin* plugin, struct texture* texture)
{
    ...
    if (image)
    {
        ...
        bzero_memory(image, sizeof(vulkan_image));

        bfree(texture->internal_data, sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
    }
...
void vulkan_renderer_texture_resize(renderer_plugin* plugin, texture* t, u32 new_width, u32 new_height)
{
    ...
    {
        // Resizing is just destroying old image and creating new one
        // Data is not preserved because there's no reliable way to map old data to new since the amount of data differs
        vulkan_image* image = (vulkan_image*)t->internal_data;
        vulkan_image_destroy(context, image);
...
void vulkan_renderer_texture_write_data(renderer_plugin* plugin, texture* t, u32 offset, u32 size, const u8* pixels, b8 include_in_frame_workload)
{
    ...
    // Transition layout from whatever it is currently to optimal for recieving data
...
void vulkan_renderer_texture_read_data(renderer_plugin* plugin, texture* t, u32 offset, u32 size, void** out_memory)
{
    ...
    // Transition layout from whatever it is currently to optimal for handing out data
...
b8 vulkan_renderer_shader_create(renderer_plugin* plugin, shader* s, const shader_config* config, renderpass* pass)
{
    ...
    internal_shader->local_push_constant_block = ballocate(128, MEMORY_TAG_RENDERER);
    ...
    if (has_global)
    {
        ...
        if (s->global_uniform_sampler_count > 0)
        {
            for (u32 i = 0; i < s->global_uniform_sampler_count; ++i)
            {
                ...
                set_config->bindings[global_binding_index].binding = global_binding_index;
                set_config->bindings[global_binding_index].descriptorCount = BMAX(u->array_length, 1);  // Either treat as an array or a single texture, depending on what is passed in
    ...
    // If using instance uniforms, add UBO descriptor set
    if (has_instance)
    {
        vulkan_descriptor_set_config* set_config = &internal_shader->descriptor_sets[internal_shader->descriptor_set_count];
        ...
        if (s->instance_uniform_sampler_count > 0)
        {
            for (u32 i = 0; i < s->instance_uniform_sampler_count; ++i)
            {
                ...
                set_config->bindings[instance_binding_index].binding = instance_binding_index;
                set_config->bindings[instance_binding_index].descriptorCount = BMAX(u->array_length, 1);
    ...
    internal_shader->instance_states = ballocate(sizeof(vulkan_shader_instance_state) * internal_shader->max_instances, MEMORY_TAG_ARRAY);
...
b8 vulkan_renderer_shader_bind_instance(renderer_plugin* plugin, shader* s, u32 instance_id)
{
    if (!s)
    {
        BERROR("vulkan_shader_bind_instance requires a valid pointer to a shader");
        return false;
    }
    if (instance_id == INVALID_ID)
    {
        BERROR("Cannot bind instance INVALID_ID");
        return false;
    }
    vulkan_shader* internal = s->internal_data;
...
b8 vulkan_renderer_shader_bind_local(renderer_plugin* plugin, shader* s)
{
    ...
    // NOTE: This is intentionally blank, and does nothing
    return true;
}
...
b8 vulkan_renderer_shader_apply_globals(renderer_plugin* plugin, shader* s, b8 needs_update, struct frame_data* p_frame_data)
{
    ...
    if (needs_update)
    {
        ...
        bzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * (1 + VULKAN_SHADER_MAX_GLOBAL_TEXTURES));
        ...
        if (s->global_uniform_count > 0)
        {
            ...
            BASSERT_MSG((s->global_ubo_offset % context->device.properties.limits.minUniformBufferOffsetAlignment) == 0, "global ubo offset must be a multiple of device.properties.limits.minUniformBufferOffsetAlignment");
        ...
        if (s->global_uniform_sampler_count > 0)
        {
            ...
            for (u32 sb = 0; sb < s->global_uniform_sampler_count; ++sb)
            {
                ...
                for (u32 d = 0; d < binding_descriptor_count; ++d)
                {
                    ...
                    // Instead of a flat list of texture maps, the instance state should have a list of uniform samplers, each with their own list of texture maps associated with them
                    ...
                    if (t->generation == INVALID_ID)
                    {
                        ...
                        {
                            // If not using one, grab the default. This is only here as a failsafe and to be used while assets are loading
    ...
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_array[internal->bound_pipeline_index]->pipeline_layout, 0, 1, &global_descriptor_set, 0, 0);
    return true;
}
...
b8 vulkan_renderer_shader_apply_instance(renderer_plugin* plugin, shader* s, b8 needs_update, frame_data* p_frame_data)
{
    ...
    {
        BERROR("This shader does not use instances");
        return false;
    }
    vulkan_shader* internal = s->internal_data;
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
    ...
    if (needs_update)
    {
        ...
        bzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) * (1 + VULKAN_SHADER_MAX_INSTANCE_TEXTURES));
        ...
        if (s->instance_uniform_count > 0)
        {
            ...
            {
                ubo_buffer_info.buffer = ((vulkan_buffer*)internal->uniform_buffer.internal_data)->handle;
                BASSERT_MSG((instance_state->offset % context->device.properties.limits.minUniformBufferOffsetAlignment) == 0, "Instance ubo offset must be a multiple of device.properties.limits.minUniformBufferOffsetAlignment");
                ...
                for (u32 d = 0; d < binding_descriptor_count; ++d)
                {
                    ...
                    if (t->generation == INVALID_ID)
                    {
                        ...
                        if (!texture_system_is_default_texture(t))
                        {
                            ...
                            {
                                // TODO: Should acquire a new default texture with the corresponding number of layers instead
                            ...
                            {
                                BERROR("Vulkan renderer failed to determine texture type while applying instance. Falling back to 2D");
                                t = texture_system_get_default_texture();
                            }
                        }
                        // If using the default texture, invalidate the map's generation so it's updated next run
    ...
    // Determine the descriptor set index which will be first. If there are no globals, for example this will be 0. If there are globals, this will be 1
    u32 first_set = 1;
    b8 has_global = s->global_uniform_count > 0 || s->global_uniform_sampler_count > 0;
    if (!has_global)
        first_set = 0;
    ...
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline_array[internal->bound_pipeline_index]->pipeline_layout, first_set, 1, &instance_descriptor_set, 0, 0);
    return true;
}
...
static VkSamplerAddressMode convert_repeat_type(const char* axis, texture_repeat repeat)
{
    switch (repeat)
    {
        ...
        default:
            BWARN("convert_repeat_type(axis='%s') Type '%x' not supported, defaulting to repeat", axis, repeat);
            return VK_SAMPLER_ADDRESS_MODE_REPEAT;
    }
}
...
static VkFilter convert_filter_type(const char* op, texture_filter filter)
{
    switch (filter)
    {
        ...
        default:
            BWARN("convert_filter_type(op='%s'): Unsupported filter type '%x', defaulting to linear", op, filter);
            return VK_FILTER_LINEAR;
    }
}
...
b8 vulkan_renderer_texture_map_resources_acquire(renderer_plugin* plugin, texture_map* map)
{
    ...
    {
        // Push empty entry into the array
...
b8 vulkan_renderer_shader_instance_resources_acquire(renderer_plugin* plugin, struct shader* s, const shader_instance_resource_config* config, u32* out_instance_id)
{
    ...
    *out_instance_id = INVALID_ID;
    ...
    {
        BERROR("vulkan_shader_acquire_instance_resources failed to acquire new id for shader '%s', max instances=%u", s->name, internal->max_instances);
        return false;
    }
    ...
    {
        instance_state->sampler_uniforms = ballocate(sizeof(vulkan_uniform_sampler_state) * s->instance_uniform_sampler_count, MEMORY_TAG_ARRAY);
        ...
        for (u32 ii = 0; ii < s->instance_uniform_sampler_count; ++ii)
        {
            ...
            sampler_state->uniform_texture_maps = ballocate(sizeof(texture_map*) * array_length, MEMORY_TAG_ARRAY);
            // Setup descriptor states
            sampler_state->descriptor_states = ballocate(sizeof(vulkan_descriptor_state) * array_length, MEMORY_TAG_ARRAY);
            // Per descriptor
    ...
    if (result != VK_SUCCESS)
    {
        BERROR("Error allocating instance descriptor sets in shader: '%s'", vulkan_result_string(result, true));
        return false;
    }
...
b8 vulkan_renderer_shader_instance_resources_release(renderer_plugin* plugin, shader* s, u32 instance_id)
{
    ...
    // Wait for any pending operations using descriptor set to finish
    vkDeviceWaitIdle(context->device.logical_device);
    ...
    if (result != VK_SUCCESS)
        BERROR("Error freeing object shader descriptor sets");
    ...
    for (u32 a = 0; a < s->instance_uniform_sampler_count; ++a)
    {
        ...
        u32 array_length = BMAX(sampler_state->uniform->array_length, 1);
        bfree(sampler_state->descriptor_states, sizeof(vulkan_descriptor_state) * array_length, MEMORY_TAG_ARRAY);
        sampler_state->descriptor_states = 0;
        bfree(sampler_state->uniform_texture_maps, sizeof(texture_map*) * array_length, MEMORY_TAG_ARRAY);
...
b8 vulkan_renderer_uniform_set(renderer_plugin* plugin, shader* s, shader_uniform* uniform, u32 array_index, const void* value)
{
    ...
    else
    {
        ...
        {
            // Map the appropriate memory location and copy data over
...
{
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
    out_renderpass->internal_data = ballocate(sizeof(vulkan_renderpass), MEMORY_TAG_RENDERER);
    ...
    VkAttachmentDescription* color_attachment_descs = darray_create(VkAttachmentDescription);
    VkAttachmentDescription* depth_attachment_descs = darray_create(VkAttachmentDescription);

    // Can always just look at first target since they are all the same (one per frame)
    for (u32 i = 0; i < config->target.attachment_count; ++i)
    {
        ...
        if (attachment_config->type == RENDER_TARGET_ATTACHMENT_TYPE_COLOR)
        {
            ...
            {
                // If we don't care, the only other thing that needs checking is if attachment is being cleared
                attachment_desc.loadOp = do_clear_color ? VK_ATTACHMENT_LOAD_OP_CLEAR : VK_ATTACHMENT_LOAD_OP_DONT_CARE;
            }
            else
            {
                ...
                if (attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_LOAD)
                {
                    if (do_clear_color)
                    {
                        BWARN("Color attachment load operation set to load, but is also set to clear. This combination is invalid, and will err toward clearing. Verify attachment configuration");
                ...
                {
                    BFATAL("Invalid and unsupported combination of load operation (0x%x) and clear flags (0x%x) for color attachment", attachment_desc.loadOp, out_renderpass->clear_flags);
                    return false;
                }
            }
            ...
            {
                BFATAL("Invalid store operation (0x%x) set for depth attachment. Check configuration", attachment_config->store_operation);
                return false;
            }
            ...
            // If this is last pass writing to this attachment, present after should be set to true
            attachment_desc.finalLayout = attachment_config->present_after ? VK_IMAGE_LAYOUT_PRESENT_SRC_KHR : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
            attachment_desc.flags = 0;

            // Push to color attachments array
            darray_push(color_attachment_descs, attachment_desc);
        }
        else if (attachment_config->type & RENDER_TARGET_ATTACHMENT_TYPE_DEPTH || attachment_config->type & RENDER_TARGET_ATTACHMENT_TYPE_STENCIL)
        {
            ...
            else
            {
                ...
                if (attachment_config->load_operation == RENDER_TARGET_ATTACHMENT_LOAD_OPERATION_LOAD)
                {
                    ...
                    if (do_clear_depth)
                    {
                        BWARN("Depth attachment load operation set to load, but is also set to clear. This combination is invalid, and will err toward clearing. Verify attachment configuration");
                        attachment_desc.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR;
                    }
                    else
                    {
                    ...
                    if (do_clear_stencil)
                    {
                        BWARN("Stencil attachment load operation set to load, but is also set to clear. This combination is invalid. Verify attachment configuration");
                ...
                {
                    BFATAL("Invalid and unsupported combination of load operation (0x%x) and clear flags (0x%x) for depth attachment", attachment_desc.loadOp, out_renderpass->clear_flags);
                    return false;
                }
            }
            ...
            attachment_desc.finalLayout = attachment_config->present_after ? VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
    ...
    VkAttachmentReference* color_attachment_references = 0;
    u32 color_attachment_count = darray_length(color_attachment_descs);
    if (color_attachment_count > 0)
    {
        color_attachment_references = ballocate(sizeof(VkAttachmentReference) * color_attachment_count, MEMORY_TAG_ARRAY);
        for (u32 i = 0; i < color_attachment_count; ++i)
        {
            color_attachment_references[i].attachment = attachments_added;
    ...
    {
        BASSERT_MSG(depth_attachment_count == 1, "Multiple depth/stencil attachments not supported");
        depth_attachment_references = ballocate(sizeof(VkAttachmentReference) * depth_attachment_count, MEMORY_TAG_ARRAY);
        for (u32 i = 0; i < depth_attachment_count; ++i)
        {
            depth_attachment_references[i].attachment = attachments_added;
    ...
    if (color_attachment_references)
        bfree(color_attachment_references, sizeof(VkAttachmentReference) * color_attachment_count, MEMORY_TAG_ARRAY);

    if (depth_attachment_descs)
        darray_destroy(depth_attachment_descs);
    if (depth_attachment_references)
        bfree(depth_attachment_references, sizeof(VkAttachmentReference) * depth_attachment_count, MEMORY_TAG_ARRAY);
...
void vulkan_renderpass_destroy(renderer_plugin* plugin, renderpass* pass)
{
    ...
    if (pass && pass->internal_data)
    {
        ...
        bfree(internal_data, sizeof(vulkan_renderpass), MEMORY_TAG_RENDERER);
        pass->internal_data = 0;
    }
}

b8 vulkan_renderer_render_target_create(renderer_plugin* plugin, u8 attachment_count, render_target_attachment* attachments, renderpass* pass, u32 width, u32 height, u16 layer_index, render_target* out_target)
{
    vulkan_context* context = (vulkan_context*)plugin->internal_context;
    ...
    bcopy_memory(out_target->attachments, attachments, sizeof(render_target_attachment) * attachment_count);
    ...
    BTRACE("Created framebuffer ' %s' at 0x%x", formatted_name, out_target->internal_framebuffer);
    return true;
}
...
void vulkan_renderer_render_target_destroy(renderer_plugin* plugin, render_target* target, b8 free_internal_memory)
{
    ...
    if (target && target->internal_framebuffer)
    {
        ...
        {
            bfree(target->attachments, sizeof(render_target_attachment) * target->attachment_count, MEMORY_TAG_ARRAY);
...
texture* vulkan_renderer_window_attachment_get(renderer_plugin* plugin, u8 index)
{
    ...
    {
        BFATAL("Attempting to get color attachment index out of range: %d. Attachment count: %d", index, context->swapchain.image_count);
        return 0;
    }
...
texture* vulkan_renderer_depth_attachment_get(renderer_plugin* plugin, u8 index)
{
    ...
    if (index >= context->swapchain.image_count)
    {
        BFATAL("Attempting to get depth attachment index out of range: %d. Attachment count: %d", index, context->swapchain.image_count);
        return 0;
    }
