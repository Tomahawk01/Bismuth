...
b8 vulkan_renderer_shader_create(renderer_plugin* plugin, shader* s, const shader_config* config, renderpass* pass, u8 stage_count, const char** stage_filenames, shader_stage* stages)
{
    ...
    // Take copy of the pointer to the context
    ...
    // Shader stages. Parse out flags
    bzero_memory(internal_shader->config.stages, sizeof(vulkan_shader_stage_config) * VULKAN_SHADER_MAX_STAGES);
    internal_shader->config.stage_count = 0;
    // Iterate provided stages
    for (u32 i = 0; i < stage_count; i++)
    {
        // Make sure there is room enough to add stage
        if (internal_shader->config.stage_count + 1 > VULKAN_SHADER_MAX_STAGES)
        {
            BERROR("Shaders may have a maximum of %d stages", VULKAN_SHADER_MAX_STAGES);
            return false;
        }

        // Make sure stage is supported
        ...
        switch (stages[i])
        {
            ...
            default:
                // Go to the next type
                BERROR("vulkan_shader_create: Unsupported shader stage flagged: %d. Stage ignored", stages[i]);
                continue;
        }

        // Set stage and bump the counter
    ...
    bzero_memory(internal_shader->config.descriptor_sets, sizeof(vulkan_descriptor_set_config) * 2);
    ...
    // Attributes array
    bzero_memory(internal_shader->config.attributes, sizeof(VkVertexInputAttributeDescription) * VULKAN_SHADER_MAX_ATTRIBUTES);
    ...
    if (internal_shader->global_uniform_count > 0 || internal_shader->global_uniform_sampler_count > 0)
    {
        ...
        // Add binding for samplers if used
    ...
    {
        // In that set, add binding for UBO if used
        vulkan_descriptor_set_config* set_config = &internal_shader->config.descriptor_sets[internal_shader->config.descriptor_set_count];
...
void vulkan_renderer_shader_destroy(renderer_plugin* plugin, shader* s)
{
    ...
    if (s && s->internal_data)
    {
        ...
        bzero_memory(&shader->config, sizeof(vulkan_shader_config));
...
b8 vulkan_renderer_shader_initialize(renderer_plugin* plugin, shader* s)
{
    ...
    bzero_memory(internal_shader->descriptor_set_layouts, internal_shader->config.descriptor_set_count);
...
b8 vulkan_renderer_shader_apply_instance(renderer_plugin* plugin, shader* s, b8 needs_update)
{
    ...
    if (needs_update)
    {
        VkWriteDescriptorSet descriptor_writes[2];  // Always max of 2 descriptor sets
        ...
        if (internal->instance_uniform_sampler_count > 0)
        {
            ...
            for (u32 i = 0; i < total_sampler_count; ++i)
            {
                ...
                image_infos[i].sampler = context->samplers[map->internal_id];

                // TODO: change descriptor state to handle this properly
...
b8 vulkan_renderer_shader_instance_resources_acquire(renderer_plugin* plugin, shader* s, u32 texture_map_count, texture_map** maps, u32* out_instance_id)
{
    ...
    *out_instance_id = INVALID_ID;
    ...
    {
        BERROR("vulkan_shader_acquire_instance_resources failed to acquire new id");
        return false;
    }
    ...
    bzero_memory(set_state->descriptor_states, sizeof(vulkan_descriptor_state) * VULKAN_SHADER_MAX_BINDINGS);
...
b8 vulkan_renderer_shader_instance_resources_release(renderer_plugin* plugin, shader* s, u32 instance_id)
{
    ...
    bzero_memory(instance_state->descriptor_set_state.descriptor_states, sizeof(vulkan_descriptor_state) * VULKAN_SHADER_MAX_BINDINGS);

    if (instance_state->instance_texture_maps)
    {
        bfree(instance_state->instance_texture_maps, sizeof(texture_map*) * s->instance_texture_count, MEMORY_TAG_ARRAY);
...
b8 vulkan_renderer_uniform_set(renderer_plugin* plugin, shader* s, shader_uniform* uniform, const void* value)
{
    ...
    else
    {
        if (uniform->scope == SHADER_SCOPE_LOCAL)
        {
            ...
            vkCmdPushConstants(command_buffer, internal->pipelines[internal->bound_pipeline_index]->pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, uniform->offset, uniform->size, value);
        }
        else
        {
...
b8 vulkan_renderpass_create(renderer_plugin* plugin, const renderpass_config* config, renderpass* out_renderpass)
{
    ...
    for (u32 i = 0; i < config->target.attachment_count; ++i)
    {
        ...
        else if (attachment_config->type & RENDER_TARGET_ATTACHMENT_TYPE_DEPTH || attachment_config->type & RENDER_TARGET_ATTACHMENT_TYPE_STENCIL)
        {
            ...
            attachment_desc.finalLayout = attachment_config->present_after ? VK_IMAGE_LAYOUT_DEPTH_STENCIL_READ_ONLY_OPTIMAL : VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL;
