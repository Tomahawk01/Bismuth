...
static bpackage_result asset_get_data(const bpackage* package, b8 is_binary, bname name, b8 get_source, u64* out_size, const void** out_data)
{
    ...
    else
    {
        ...
        if (read_size < original_file_size)
        {
            ...
            void* temp = ballocate(read_size + (is_binary ? 0 : 1), MEMORY_TAG_ASSET);
        ...
        else
        {
            ...
            for (u32 i = 0; i < asset_array_count; ++i)
            {
                ...
                for (u32 a = 0; a < asset_count; ++a)
                {
                    ...
                    {
                        // A collision exists. This makes the manifest invalid, and should fail the process completely
                        BERROR("Failed to process asset manifest for package '%s'. An asset named '%s' already exists", bname_string_get(out_manifest->name), asset_name);
                        return false;
                    }
                }
