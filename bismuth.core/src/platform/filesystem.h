 ...
 * NOTE: Calling file must #include "logger.h"
 ...
 */
BAPI b8 filesystem_exists(const char* path);
 ...
 */
BAPI b8 filesystem_open(const char* path, file_modes mode, b8 binary, file_handle* out_handle);
 ...
 */
BAPI void filesystem_close(file_handle* handle);
 ...
 * @return BAPI 
 */
BAPI b8 filesystem_size(file_handle* handle, u64* out_size);
 ...
 */
BAPI b8 filesystem_read_line(file_handle* handle, u64 max_length, char** line_buf, u64* out_line_length);
 ...
 * @returns True if successful; otherwise false.
 */
BAPI b8 filesystem_write_line(file_handle* handle, const char* text);
 ...
 * @returns True if successful; otherwise false.
 */
BAPI b8 filesystem_read(file_handle* handle, u64 data_size, void* out_data, u64* out_bytes_read);
 ...
 * @returns True if successful; otherwise false.
 */
BAPI b8 filesystem_read_all_bytes(file_handle* handle, u8* out_bytes, u64* out_bytes_read);
 ...
 * @returns True if successful; otherwise false.
 */
BAPI b8 filesystem_read_all_text(file_handle* handle, char* out_text, u64* out_bytes_read);
 ...
 * @returns True if successful; otherwise false.
 */
BAPI b8 filesystem_write(file_handle* handle, u64 data_size, const void* data, u64* out_bytes_written);

/**
 * @brief Opens and reads all text content of the file at the provided path.
 * No file handle required. File is closed automatically. Memory is dynamically allocated (tagged as MEMORY_TAG_STRING) and should be freed by the caller
 ...
 */
BAPI const char* filesystem_read_entire_text_file(const char* filepath);

/**
 * @brief Opens and reads all content of the file at the provided path.
 * No file handle required. File is closed automatically. Memory is dynamically allocated (tagged as MEMORY_TAG_ARRAY) and should be freed by the caller
 *
 * @param filepath The path to the file to read.
 * @param out_size A pointer to hold the size of the file read in.
