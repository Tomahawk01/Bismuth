...
// Maximum length of a geometry name
#define GEOMETRY_NAME_MAX_LENGTH 256

/**
 * @brief Indicates a geometry type, typically used to infer things like vertex and index sizes
 */
typedef enum bgeometry_type
{
    /** @brief Unknown and invalid type of geometry. This being set generally indicates an error in code */
    BGEOMETRY_TYPE_UNKNOWN = 0x00,
    /** @brief Used for 2d geometry that doesn't change */
    BGEOMETRY_TYPE_2D_STATIC = 0x01,
...
/**
 * @brief Represents geometry to be used for various purposes (rendering objects in the world, physics/collision, etc.)
 */
typedef struct bgeometry
{
    ...
    u32 vertex_count;
    /** @brief The size of each vertex. Ignored unless type is BGEOMETRY_TYPE_CUSTOM */
    ...
    u32 index_count;
    /** @brief The size of each index. Ignored unless type is BGEOMETRY_TYPE_CUSTOM */
...
{
    /** @brief A grid that lies "flat" in the world along the ground plane (y-plane). This is the default configuration */
 ...
 */
BAPI void geometry_generate_normals(u32 vertex_count, vertex_3d* vertices, u32 index_count, u32* indices);
 ...
 * @param indices An array of vertices.
 */
BAPI void geometry_generate_tangents(u32 vertex_count, vertex_3d* vertices, u32 index_count, u32* indices);
 ...
 */
BAPI void geometry_deduplicate_vertices(u32 vertex_count, vertex_3d* vertices, u32 index_count, u32* indices, u32* out_vertex_count, vertex_3d** out_vertices);
 ...
 */
BAPI void generate_uvs_from_image_coords(u32 img_width, u32 img_height, u32 px_x, u32 px_y, f32* out_tx, f32* out_ty);
 ...
 */
BAPI bgeometry geometry_generate_quad(f32 width, f32 height, f32 tx_min, f32 tx_max, f32 ty_min, f32 ty_max, bname name);
 ...
 * @returns The newly-created geometry
 */
BAPI bgeometry geometry_generate_line2d(vec2 point_0, vec2 point_1, bname name);
 ...
 * @returns The newly-created geometry
 */
BAPI bgeometry geometry_generate_plane(f32 width, f32 height, u32 x_segment_count, u32 y_segment_count, f32 tile_x, f32 tile_y, bname name);
 ...
 */
BAPI void geometry_recalculate_line_box3d_by_points(bgeometry* geometry, vec3 points[8]);
 ...
 */
BAPI void geometry_recalculate_line_box3d_by_extents(bgeometry* geometry, extents_3d extents);
 ...
 * @returns The newly-created geometry
 */
BAPI bgeometry geometry_generate_line_box3d(vec3 size, bname name);

/**
 * @brief Generates a three-dimensional cube of geometry. Note that memory for the
 * vertex and index arrays are dynamically allocated, so this should be cleaned up with geometry_destroy()
 *
 ...
 * @returns The newly-created geometry
 */
BAPI bgeometry geometry_generate_cube(f32 width, f32 height, f32 depth, f32 tile_x, f32 tile_y, bname name);
 ...
 * @returns The newly-created geometry
 */
BAPI bgeometry geometry_generate_grid(grid_orientation orientation, u32 segment_count_dim_0, u32 segment_count_dim_1, f32 segment_size, b8 use_third_axis, bname name);
