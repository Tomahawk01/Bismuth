...
void geometry_recalculate_line_box3d_by_points(bgeometry* geometry, vec3 points[8])
{
    ...
    {
        // top
        ((color_vertex_3d*)geometry->vertices)[0].position = vec4_from_vec3(points[2], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[1].position = vec4_from_vec3(points[3], 1.0f);
        // right
        ((color_vertex_3d*)geometry->vertices)[2].position = vec4_from_vec3(points[1], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[3].position = vec4_from_vec3(points[2], 1.0f);
        // bottom
        ((color_vertex_3d*)geometry->vertices)[4].position = vec4_from_vec3(points[0], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[5].position = vec4_from_vec3(points[1], 1.0f);
        // left
        ((color_vertex_3d*)geometry->vertices)[6].position = vec4_from_vec3(points[3], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[7].position = vec4_from_vec3(points[0], 1.0f);
    }
    // back lines
    {
        // top
        ((color_vertex_3d*)geometry->vertices)[8].position = vec4_from_vec3(points[6], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[9].position = vec4_from_vec3(points[7], 1.0f);
        // right
        ((color_vertex_3d*)geometry->vertices)[10].position = vec4_from_vec3(points[5], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[11].position = vec4_from_vec3(points[6], 1.0f);
        // bottom
        ((color_vertex_3d*)geometry->vertices)[12].position = vec4_from_vec3(points[4], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[13].position = vec4_from_vec3(points[5], 1.0f);
        // left
        ((color_vertex_3d*)geometry->vertices)[14].position = vec4_from_vec3(points[7], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[15].position = vec4_from_vec3(points[4], 1.0f);
    }

    // top connecting lines
    {
        // left
        ((color_vertex_3d*)geometry->vertices)[16].position = vec4_from_vec3(points[3], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[17].position = vec4_from_vec3(points[7], 1.0f);
        // right
        ((color_vertex_3d*)geometry->vertices)[18].position = vec4_from_vec3(points[2], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[19].position = vec4_from_vec3(points[6], 1.0f);
    }
    // bottom connecting lines
    {
        // left
        ((color_vertex_3d*)geometry->vertices)[20].position = vec4_from_vec3(points[0], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[21].position = vec4_from_vec3(points[4], 1.0f);
        // right
        ((color_vertex_3d*)geometry->vertices)[22].position = vec4_from_vec3(points[1], 1.0f);
        ((color_vertex_3d*)geometry->vertices)[23].position = vec4_from_vec3(points[5], 1.0f);
    }
}
...
void geometry_recalculate_line_box3d_by_extents(bgeometry* geometry, extents_3d extents)
{
    ...
    {
        // top
        ((color_vertex_3d*)geometry->vertices)[0].position = (vec4){extents.min.x, extents.min.y, extents.min.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[1].position = (vec4){extents.max.x, extents.min.y, extents.min.z, 1.0f};
        // right
        ((color_vertex_3d*)geometry->vertices)[2].position = (vec4){extents.max.x, extents.min.y, extents.min.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[3].position = (vec4){extents.max.x, extents.max.y, extents.min.z, 1.0f};
        // bottom
        ((color_vertex_3d*)geometry->vertices)[4].position = (vec4){extents.max.x, extents.max.y, extents.min.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[5].position = (vec4){extents.min.x, extents.max.y, extents.min.z, 1.0f};
        // left
        ((color_vertex_3d*)geometry->vertices)[6].position = (vec4){extents.min.x, extents.min.y, extents.min.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[7].position = (vec4){extents.min.x, extents.max.y, extents.min.z, 1.0f};
    }
    // back lines
    {
        // top
        ((color_vertex_3d*)geometry->vertices)[8].position = (vec4){extents.min.x, extents.min.y, extents.max.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[9].position = (vec4){extents.max.x, extents.min.y, extents.max.z, 1.0f};
        // right
        ((color_vertex_3d*)geometry->vertices)[10].position = (vec4){extents.max.x, extents.min.y, extents.max.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[11].position = (vec4){extents.max.x, extents.max.y, extents.max.z, 1.0f};
        // bottom
        ((color_vertex_3d*)geometry->vertices)[12].position = (vec4){extents.max.x, extents.max.y, extents.max.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[13].position = (vec4){extents.min.x, extents.max.y, extents.max.z, 1.0f};
        // left
        ((color_vertex_3d*)geometry->vertices)[14].position = (vec4){extents.min.x, extents.min.y, extents.max.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[15].position = (vec4){extents.min.x, extents.max.y, extents.max.z, 1.0f};
    }

    // top connecting lines
    {
        // left
        ((color_vertex_3d*)geometry->vertices)[16].position = (vec4){extents.min.x, extents.min.y, extents.min.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[17].position = (vec4){extents.min.x, extents.min.y, extents.max.z, 1.0f};
        // right
        ((color_vertex_3d*)geometry->vertices)[18].position = (vec4){extents.max.x, extents.min.y, extents.min.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[19].position = (vec4){extents.max.x, extents.min.y, extents.max.z, 1.0f};
    }
    // bottom connecting lines
    {
        // left
        ((color_vertex_3d*)geometry->vertices)[20].position = (vec4){extents.min.x, extents.max.y, extents.min.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[21].position = (vec4){extents.min.x, extents.max.y, extents.max.z, 1.0f};
        // right
        ((color_vertex_3d*)geometry->vertices)[22].position = (vec4){extents.max.x, extents.max.y, extents.min.z, 1.0f};
        ((color_vertex_3d*)geometry->vertices)[23].position = (vec4){extents.max.x, extents.max.y, extents.max.z, 1.0f};
    }
}
...
b8 geometry_calculate_triangles(bgeometry* geometry)
{
    if (!geometry)
        return false;
    ...
    {
        BERROR("Invalid index count that is not divisible by 3, thus a incomplete/degenerate triangle exists");
        return false;
    }

    // NOTE: This assumes a triangle list. A strip would not work here
    geometry->triangle_count = geometry->index_count / 3;
    geometry->tris = BALLOC_TYPE_CARRAY(triangle_3d, geometry->triangle_count);
