...
f32 vec4_dot_f32(f32 a0, f32 a1, f32 a2, f32 a3, f32 b0, f32 b1, f32 b2, f32 b3)
{
    f32 p;
...
{
    f32 sin_p = bsin(euler_rotation_radians.x);
    f32 cos_p = bcos(euler_rotation_radians.x);
    f32 sin_y = bsin(euler_rotation_radians.y);
    f32 cos_y = bcos(euler_rotation_radians.y);
    f32 sin_r = bsin(euler_rotation_radians.z);
    f32 cos_r = bcos(euler_rotation_radians.z);
...
vec3 quat_to_euler_radians(quat q)
{
    ...
    if (babs(sin_p) >= 1.0f)
        out_euler.x = copysign(B_PI / 2.0f, sin_p);
    else
        out_euler.x = basin(sin_p);
...
quat quat_lookat(vec3 from, vec3 to)
{
    ...
    else if (dot <= -0.9999f)
    {
        ...
        vec3 up = vec3_up();
        return quat_from_axis_angle(up, B_PI, false);
    }
...
quat quat_slerp(quat q_0, quat q_1, f32 percentage)
{
    ...
    f32 s0 = bcos(theta) - dot * sin_theta / sin_theta_0; // == sin(theta_0 - theta) / sin(theta_0)
...
{
    f32 r = extents->x * babs(p->normal.x) +
            extents->y * babs(p->normal.y) +
            extents->z * babs(p->normal.z);
...
f32 oriented_bounding_box_project(const oriented_bounding_box* obb, vec3 axis)
{
    ...
    return babs(vec3_dot(axis, right)) * obb->half_extents.x +
           babs(vec3_dot(axis, up)) * obb->half_extents.y +
           babs(vec3_dot(axis, forward)) * obb->half_extents.z;
}
