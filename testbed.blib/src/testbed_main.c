                ...
                if (point_in_rect_2d((vec2){(f32)x, (f32)y}, v->rect))
                {
                    ...
                    if (scene_raycast(&state->main_scene, &r, &r_result))
                    {
                        ...
                        for (u32 i = 0; i < hit_count; ++i)
                        {
                            ...
                            test_box.parent_xform = bhandle_invalid();
...
b8 application_boot(struct application* game_inst)
{
    ...
    // Allocate game state
    game_inst->state = ballocate(sizeof(application_state), MEMORY_TAG_GAME);
...
static b8 load_main_scene(struct application* game_inst)
{
    ...
    asset->asset_name = bname_create("test_scene");
    asset->package_name = bname_create("Testbed");
    ...
    if (!scene_resource)
    {
        BERROR("Failed to request scene resource. See logs for details");
        return false;
    }
