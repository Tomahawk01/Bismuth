...
b8 editor_gizmo_rendergraph_node_execute(struct rendergraph_node* self, struct frame_data* p_frame_data)
{
    ...
    {
        renderer_begin_rendering(internal_data->renderer, p_frame_data, internal_data->vp.rect, 1, &internal_data->colorbuffer_texture->renderer_texture_handle, b_handle_invalid(), 0);
        ...
        // Set model matrix
        shader_system_uniform_set_by_location(internal_data->color_shader_id, internal_data->debug_locations.model, &model);
