...
b8 editor_gizmo_create(editor_gizmo* out_gizmo)
{
    ...
    out_gizmo->selected_xform_handle = bhandle_invalid();
    // Default orientation
...
b8 editor_gizmo_load(editor_gizmo* gizmo)
{
    ...
    for (u32 i = 0; i < EDITOR_GIZMO_MODE_MAX + 1; ++i)
    {
        bgeometry* g = &gizmo->mode_data[i].geo;
        editor_gizmo_mode_data* mode = &gizmo->mode_data[i];

        g->vertex_count = mode->vertex_count;
        g->vertex_element_size = sizeof(color_vertex_3d);
        g->vertex_buffer_offset = 0;
        g->vertices = BALLOC_TYPE_CARRAY(color_vertex_3d, g->vertex_count);
        g->index_count = 0;
...
void editor_gizmo_refresh(editor_gizmo* gizmo)
{
    if (gizmo)
    {
        if (!bhandle_is_invalid(gizmo->selected_xform_handle))
        {
            // Set position
        ...
        {
            BINFO("refreshing gizmo with defaults");
            // Reset
...
void editor_gizmo_selected_transform_set(editor_gizmo* gizmo, bhandle xform_handle, bhandle parent_xform_handle)
{
    if (gizmo)
    {
...
void editor_gizmo_mode_set(editor_gizmo* gizmo, editor_gizmo_mode mode)
{
    if (gizmo)
    {
        ...
        case EDITOR_GIZMO_MODE_NONE:
            BTRACE("Gizmo mode set to 'none'");
            break;
        case EDITOR_GIZMO_MODE_MOVE:
            BTRACE("Gizmo mode set to 'move'");
            break;
        case EDITOR_GIZMO_MODE_ROTATE:
            BTRACE("Gizmo mode set to 'rotate'");
            break;
        case EDITOR_GIZMO_MODE_SCALE:
            BTRACE("Gizmo mode set to 'scale'");
            break;
        }
#endif
    }
    ...
    if (gizmo->mode == EDITOR_GIZMO_MODE_MOVE)
    {
        if (interaction_type == EDITOR_GIZMO_INTERACTION_TYPE_MOUSE_DRAG)
        {
            ...
            if (!bhandle_is_invalid(gizmo->selected_xform_handle))
            {
                ...
                if (!bhandle_is_invalid(gizmo->selected_xform_parent_handle))
                {
