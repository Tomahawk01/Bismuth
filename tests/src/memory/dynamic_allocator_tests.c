...
u8 dynamic_allocator_multi_allocation_all_space()
{
    ...
    // Allocate last part of the block
...
u8 dynamic_allocator_multi_allocation_over_allocate()
{
    ...
    BDEBUG("The following warning and errors are intentionally caused by this test");
...
u8 util_free(dynamic_allocator* allocator, alloc_data* data, u64* currently_allocated, u64 header_size, u64 total_allocator_size)
{
    ...
    {
        BERROR("util_free, dynamic_allocator_free_aligned failed");
        return false;
    }
    data->block = 0;
    currently_allocated -= (data->size + header_size + data->alignment);
...
u8 dynamic_allocator_multiple_alloc_aligned_different_alignments_random()
{
    ...
    {
        u32 index = (u32)brandom_in_range(0, alloc_data_count);
        if (alloc_datas[index].block == 0)
        {
            ...
            {
                BERROR("util_allocate failed on index: %u");
                return false;
            }
            alloc_count++;
        }
    ...
    BTRACE("Randomly allocated %u times. Freeing randomly...", alloc_count);
