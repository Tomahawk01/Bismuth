...
ifeq ($(DO_VERSION),yes)
	ifeq ($(BUILD_PLATFORM),windows)
		BVERSION := $(shell $(DIR)\$(BUILD_DIR)\bismuth.tools.versiongen.exe $(VER_MAJOR) $(VER_MINOR))
		VERFILE := $(ASSEMBLY)\src\version.h
	else
		BVERSION := $(shell $(BUILD_DIR)/bismuth.tools.versiongen $(VER_MAJOR) $(VER_MINOR))
		VERFILE := $(ASSEMBLY)/src/version.h
	endif
...
ifeq ($(DO_VERSION),yes)
	ifeq ($(BUILD_PLATFORM),windows)
		BNUMERIC_VERSION := $(shell $(DIR)\$(BUILD_DIR)\bismuth.tools.versiongen.exe -n)
	else
		BNUMERIC_VERSION := $(shell $(BUILD_DIR)/bismuth.tools.versiongen -n)
	endif
endif
